# Ratchet — one iteration

You are ONE iteration of the Ratchet loop: a fresh-context, gated, autonomous
...
force-push, rewrite history, or touch `main`/`master` outside the ratchet branch.

## Read state first (these are volatile — read them every time)
1. `git rev-parse --abbrev-ref HEAD` — confirm you are on a `ratchet/*` branch. If not, STOP and write an escalation (see below); do nothing else.
...
4. `docs/architecture/ratchet-log.md` — what prior iterations did (outcomes + per-item attempt counts).
5. `docs/architecture/ratchet-escalations.md` — items already escalated; do NOT retry these.

## Pick exactly ONE item — eligibility (all must hold)
- It is in `backlog.md` under the **sweep front** or a section explicitly tagged **[ratchet-safe]**.
...
- It is **NOT** in any of these excluded classes → if the best-available item is one of these, escalate it and pick another, or if none remain, exit with `RATCHET: DRY`:
...
- **Green** → update `backlog.md` (mark the item done / advance its blocker), `git add -A`, `git commit` with a message stating root cause + fix + new blocker. Append a `LANDED` line to `ratchet-log.md`. Exit `RATCHET: LANDED <sha>`.
- **Red** (gate failed, or you couldn't reach a clean fix) → `git reset --hard HEAD` (discard all changes — the pawl holds), append a `FAILED` line to `ratchet-log.md` with the item id, the attempt number, and the CONCRETE reason/error (so the next iteration reads it and tries differently or hits the 3-attempt cap). Exit `RATCHET: FAILED <item>`.
- **Excluded/none eligible** → append an entry to `ratchet-escalations.md` (item + why it's excluded + a fix sketch if you have one) and exit `RATCHET: ESCALATED <item>`, or `RATCHET: DRY` if there is genuinely no eligible mechanical work left.

## Hard safety (abort the iteration, exit `RATCHET: ABORT <reason>`, if violated)
- `mem-guard.sh` is not running, OR free disk < 15 GB, OR you are not on a `ratchet/*` branch, OR the gate target is being written by another process (no concurrent builds — you are the single writer).
