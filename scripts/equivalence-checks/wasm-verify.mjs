#!/usr/bin/env node
// scripts/equivalence-checks/wasm-verify.mjs — browser interaction driver for
// `--target wasm` SPA examples. Serves the emitted `www/` tree with a local
// HTTP server (no binary to boot — the WASM runs entirely in the browser),
// launches headless Chromium, runs the named scenario, and exits 0/1.
//
// Usage:
//   node wasm-verify.mjs <www-dir> <scenario> [port]
//
// <www-dir>   — path to the `sky-out/rust/www/` directory (must contain
//               index.html + pkg/sky_app.js + pkg/sky_app_bg.wasm)
// <scenario>  — scenario key from verify-scenarios.mjs, or "smoke"
...
async function runScenario(page, baseUrl, name) {
    // Import the shared scenario module (same file the server-app driver uses).
