...
//
// PORTED VERBATIM from ../sky/scripts/verify-scenarios.mjs — this file is
// SHAPE-AGNOSTIC (it drives a browser against whatever's listening on
// `baseUrl`), so it applies unchanged to skyc-emitted Rust binaries AND the
// Go-oracle reference binary alike. See docs/architecture/
...
async function expectSkyEventAfter(opts, fn, label) {
    if (!opts || !Array.isArray(opts.skyEventPosts)) {
        ...
        // structural HTML check in web-verify.mjs.
        await fn();
        return;
    }
...
export const scenarios = {
    ...
    async skyvote(page, { baseUrl }) {
        ...
        await fillByName(page, 'description', 'Auto-generated by web-verify.mjs to exercise full Sky.Live CRUD path.', 200);
