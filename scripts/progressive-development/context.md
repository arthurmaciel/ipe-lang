...
4. **No shortcuts (PRINCIPLES.md §0)** — removing/skipping the file, example, test, or line that TRIGGERS a bug is NOT a fix. Never edit a reference example/fixture/`.sky` to dodge a compiler gap, weaken a gate, or `#[allow]` a real violation. Two acceptable outcomes only: root-cause it, or report it as an honest tracked blocker. A green obtained by deleting the red is a FAILURE. "Make the sweep green" means make the programs actually compile+run, not make red rows disappear.
