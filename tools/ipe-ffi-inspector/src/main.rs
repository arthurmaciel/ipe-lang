//! ipe-ffi-inspector — Rust crate inspector for Ipe FFI bindings.
//!
...
//!
//! Output JSON matches the `PkgInfo` schema that `ipe_ffi::pkginfo` consumes.
//! The output contract is unchanged from v0.1 — only the inspection
//! backend changed.
//!
//! Usage:
//!   ipe-ffi-inspector uuid                          # single crate
//!   ipe-ffi-inspector uuid chrono                   # multi → JSON array
//!   ipe-ffi-inspector --features v4 uuid            # with features
...
// ── Output schema (ipe_ffi::pkginfo PkgInfo) ─────────────────────────────────
...
fn main() {
    ...
    while i < raw_args.len() {
        match raw_args[i].as_str() {
            ...
            "--rev" => {
                i += 1;
                if i < raw_args.len() {
                    git_rev = Some(raw_args[i].clone());
                }
            }
            "--branch" => {
                i += 1;
                if i < raw_args.len() {
                    git_branch = Some(raw_args[i].clone());
                }
            }
            "--tag" => {
                i += 1;
                if i < raw_args.len() {
                    git_tag = Some(raw_args[i].clone());
                }
            }
            _ => {
    ...
    // A `--manifest` run ALWAYS emits a JSON array (the generator caller decodes
...
fn emit_tail_audit_report(crate_args: &[String], results: &[PkgInfo]) {
    ...
    let drops: Vec<(String, bool, String)> = TAIL_AUDIT_DROPS.with(|v| v.borrow().clone());
    ...
    for reason in [
        ...
        // suppressed because the field is narrower than / wider than Ipe's i64.
        "setter_narrowing",
...
fn parse_manifest(path: &str) -> Result<Vec<CrateSpec>, String> {
    ...
    for (i, e) in arr.iter().enumerate() {
        let name = e
            .get("name")
            .and_then(|n| n.as_str())
            .ok_or_else(|| format!("manifest entry {i} missing string `name`"))?
            .to_string();
        let features = e
            .get("features")
...
fn inspect_crate_source(
    ...
    // clean-name slug).
    let clean_name = crate_name.split_once('@').map_or(crate_name, |(n, _)| n);
        ...
        Ok((json_content, version, transitive_deps, effective_features)) => {
            match serde_json::from_str::<serde_json::Value>(&json_content) {
                Ok(doc) => {
                    ...
                    // `#[define_in_ipe]` is added to the exposed set, so a
...
fn run_rustdoc_source(
    ...
    {
        return Err(format!(
            "invalid crate name {crate_name:?}: only [A-Za-z0-9_-] are allowed"
        ));
    }
    ...
    {
        return Err(format!(
            "invalid version requirement {v:?} for crate {crate_name:?}: only semver chars allowed"
        ));
    }
    ...
    let (json_content, version, effective_features) = match run_rustdoc_package(
        ...
        Ok((j, v)) if auto_injected && verify_stable => {
            match injected_stable_check(&manifest_str, &dir.join("target-verify")) {
                ...
                // the next `ipe install` once the check can run. NOT
                // mislabelled as nightly (distinct message + reason).
                StableCheck::Unverifiable(why) => {
                    eprintln!(
                        "[ipe-ffi] could not verify feature set [{}] on stable ({}); \
...
fn run_rustdoc_package(
    ...
    let output = cmd
        ...
        .map_err(|e| {
            format!(
                "cargo +nightly rustdoc failed: {e}.\n\
...
fn find_rustdoc_json(doc_dir: &PathBuf, safe_name: &str) -> Option<PathBuf> {
    // Direct match first
    let direct = doc_dir.join(format!("{safe_name}.json"));
    if direct.exists() {
        return Some(direct);
    }
...
fn build_consent_decision(
    ...
    if allow_build_scripts {
        return Ok(Some(format!(
            "[ipe-ffi] WARNING: --allow-build-scripts is set — the following \
    ...
    match git {
        None => {
            ...
            match feats_field {
                ...
                Some(f) => format!("{crate_name} = {{ version = \"{vr}\", {f} }}"),
            }
        }
        Some(g) => {
            let mut fields = vec![format!("git = \"{}\"", toml_escape(&g.url))];
            if let Some(r) = &g.rev {
                fields.push(format!("rev = \"{}\"", toml_escape(r)));
            }
            if let Some(b) = &g.branch {
                fields.push(format!("branch = \"{}\"", toml_escape(b)));
            }
            if let Some(t) = &g.tag {
                fields.push(format!("tag = \"{}\"", toml_escape(t)));
            }
            if let Some(f) = feats_field {
                fields.push(f);
            }
...
fn parse_rustdoc(doc: &serde_json::Value, crate_name: &str, version: &str) -> PkgInfo {
    ...
    let Some(index) = doc["index"].as_object() else {
        return pkg_error(crate_name, "rustdoc JSON missing 'index' field");
    };
    ...
    for (item_id, item) in index {
        ...
        if let Some(impl_data) = inner.get("impl") {
            ...
            let self_mono_subst =
                for_val_raw.and_then(|fv| impl_self_mono_subst(fv, impl_generics));
            let for_val_owned: Option<serde_json::Value> = match (&self_mono_subst, for_val_raw) {
                (Some(map), Some(fv)) => Some(subst_generic_json(fv, map, BoundPos::Param)),
                _ => None,
            };
            let for_val = for_val_owned.as_ref().or(for_val_raw);
            let self_ipe = for_val
                .map(|v| rustdoc_type_to_ipe(v, &aliases))
                .unwrap_or_default();
            ...
            let bridge_tag = impl_data.get("trait").and_then(std_trait_tag);
            if bridge_tag == Some("Display") {
                // key = Ipe type name; value = full Rust type (may have generics)
                display_types
                    .entry(self_ipe.clone())
                    .or_insert_with(|| self_rust.clone());
            }
                ...
                for method_id in items {
                    ...
                    if let Some(method_item) = index.get(&id) {
                        ...
                        if let Some(fn_data_raw) = method_item["inner"].get("function") {
                            ...
                            let self_mono_owned: Option<serde_json::Value> = self_mono_subst
                                .as_ref()
                                .map(|map| subst_generic_json(fn_data_raw, map, BoundPos::Param));
                            ...
                            let generic_bearing =
                                method_is_generic_bearing(fn_data, impl_generics, struct_generics);
                            ...
                            let is_concrete_trait_method = !is_inherent_impl && trait_self_concrete;
                            let take_parametric = ((generic_bearing && is_inherent_impl)
                    ...
                    project_trait_default_methods(trait_node, impl_data, for_val, index)
            {
                ...
                let trait_has_send_supertrait =
                    trait_node.and_then(|tn| tn.get("id")).is_some_and(|id| {
                ...
                for (pm_name, pm_fn_data) in projected {
                    ...
                    match try_parametric_stub(
                        ...
                        Ok(generic) => {
                            record_generic_bound();
                            if let Some(f) = parametric_function(
                                &pm_name, fn_data, &self_ipe, &self_rust, generic,
                            ) {
                                functions.push(f);
                            }
                        }
        ...
        // closed-set-eligible field. The owning struct's Ipe / Rust types come
        ...
        if let Some(struct_data) = inner.get("struct") {
            ...
            // Receiver Ipe / Rust types from a synthetic resolved_path to this
            ...
            let recv_ipe = rustdoc_type_to_ipe(&recv_path, &aliases);
            let recv_rust = rustdoc_type_to_rust_str(&recv_path);
            if recv_ipe.is_empty() || recv_rust.is_empty() {
                continue;
            }
            ...
            for field_id in field_ids {
                ...
                let field_ipe = rustdoc_type_to_ipe(field_ty, &aliases);
                let field_rust = rustdoc_type_to_rust_str(field_ty);
                if field_ipe.is_empty() || field_rust.is_empty() {
                    continue;
                }

                // C2: bake a `_field` discriminator into the Ipe-visible NAME so
                // a `pub id` field and an `id()` method on the same struct can't
                // collide in the dedup / kernel-json / `.ipei` name key. The
                ...
                functions.push(Function {
                    ...
                    params: vec![Param {
                        name: "self".into(),
                        ty: recv_ipe.clone(),
                        ipe_type: recv_ipe.clone(),
                        rust_type: recv_rust.clone(),
                    }],
                    results: vec![Param {
                        name: String::new(),
                        ty: field_ipe.clone(),
                        ipe_type: field_ipe.clone(),
                ...
                // is the receiver type. `wrapperIpeType` renders this as
                // `FieldTy -> Recv -> Result Error Recv`; the Rust-local
                // `fieldIpeType` strips the `Result Error` to the INFALLIBLE
                // (C6) `FieldTy -> Recv -> Recv`. The setter returns a NEW
                // receiver with the field replaced (Ipe immutable-update value
        ...
        // accessor, and single-field payload extractors. NEVER lowered to a Ipe
    ...
    // the receiver type.
    // Functions with `generic: Some(_)` are routed to `ipe_ffi_generics.rs` by
    // the generator's FfiGen — they are NEVER emitted by `emitRustFnSimple` in
    ...
    functions.retain(|f| {
        ...
        let is_instance = f.params.first().is_some_and(|p| p.name == "self");
        // [#58 WALL 2] Trait-mono'd methods carry a dummy `<M>` suffix on
        // `recv_rust_type` to trigger the generator's `hasGenericRecvParam` skip in
    ...
    if functions.is_empty() && !errors.is_empty() {
        ...
        errors.insert(0, format!("{crate_name}: 0 functions extracted"));
    }
...
/// a note naming them so the user knows to `ipe add` the sub-crates directly.
///
...
fn facade_guidance(crate_name: &str, fn_count: usize, doc: &serde_json::Value) -> Vec<String> {
    ...
    let add_cmds: Vec<String> = names.iter().map(|n| format!("ipe add {n}")).collect();
        ...
        .is_some_and(|arr| {
            arr.iter().any(|a| {
                ...
                let s = a
                    .as_str()
                    .map(str::to_string)
                    .or_else(|| a.get("other").and_then(|o| o.as_str()).map(str::to_string));
                s.is_some_and(|t| t.contains("doc(hidden)") || t.contains("DocHidden"))
            })
        })
}
...
fn collect_from_string_defpaths(
    ...
    for (_id, item) in index {
        ...
        let target_local = index
            .get(&target_id)
            .is_some_and(|it| it["crate_id"].as_u64().unwrap_or(1) == 0);
        if !target_local {
            continue;
        }
...
fn collect_trait_concrete_impls(
    ...
    for (_id, item) in index {
        ...
        let Some(trait_id) = trait_node.get("id") else {
            continue;
        };
        ...
        let reachable = for_val
            .get("resolved_path")
            .and_then(|rp| rp.get("id"))
            .is_some_and(|id| reachable_local_path(id).is_some());
        if !reachable {
            continue;
        }
        ...
        if !entry
            .iter()
            .any(|e| rustdoc_type_to_rust_str(e) == rendered)
        {
...
fn mirror_into_global_xc_index(doc: &serde_json::Value) {
    ...
    for (_id, item) in index {
        ...
        let Some(canon) = canon_path_of_id(paths, &trait_key) else {
            continue;
        };
        let for_val = impl_data.get("for").or_else(|| impl_data.get("for_"));
        let Some(for_val) = for_val else { continue };
        ...
        // canonical path (`client_crate::RealClient`) — exactly what the ipe-app
        ...
        if self_public_path.is_empty() || self_public_path.contains("__ipe_xc_path") {
            continue;
        }
        ...
        let synthetic = serde_json::json!({
            "resolved_path": { "__ipe_xc_path": self_public_path }
        });
...
fn collect_send_supertrait_trait_ids(
    ...
    for (id, item) in index {
        ...
        let Some(trait_data) = item["inner"].get("trait") else {
            continue;
        };
        let Some(bounds) = trait_data.get("bounds").and_then(|b| b.as_array()) else {
            continue;
        };
        let has_send = bounds
            .iter()
            .any(|b| trait_bound_name(b).as_deref() == Some("Send"));
        if has_send {
            out.insert(id.clone());
        }
    }
...
// Arms kept separate to document each primitive's distinct admission rationale.
...
fn field_type_eligible(
    ...
    if let Some(prim) = ty.get("primitive").and_then(|p| p.as_str()) {
        return match prim {
            "i8" | "i16" | "i32" | "i64" | "u8" | "u16" | "u32" | "f32" | "f64" | "bool" => Ok(()),
            ...
            // inbound Ipe i64 can never silently truncate/reinterpret into a
            // wider/narrower-signedness field. Asymmetric by design: lossless to
            // READ, lossy to WRITE.
            "u64" | "u128" | "i128" | "usize" | "isize" => Ok(()),
            // `char` is Copy and maps cleanly end-to-end: `type_str_to_ipe` now
            // has a `char -> Char` arm (S2), the Rust TypeRenderer already lowers
            // Ipe `Char -> char` (`Can.TType _ "Char" []`), and `char_kernel`
...
/// True when a field's Rust type can LOSSLESSLY RECEIVE a Ipe scalar in a
/// setter's inbound assignment. Ipe `Int` is `i64` and Ipe `Float` is `f64`, so
...
/// setter to `…map(|x| x as u32)` — both reinterpret/truncate the Ipe i64 just
...
/// and containers thereof) receives a Ipe value of the same shape with no
...
fn setter_receives_losslessly(rust_ty: &str) -> bool {
    ...
    let narrowing = rust_ty
        ...
        .any(|tok| {
            matches!(
                tok,
                // Narrower than the Ipe scalar — assignment truncates.
                "i8" | "i16" | "i32" | "u8" | "u16" | "u32" | "f32"
                // Wider-than-i64 ints — a Ipe i64 over/under-shoots or a negative
        ...
        .is_none_or(|arr| {
            arr.iter().any(|a| {
                let s = a
                    ...
                    .map(str::to_string)
                    .or_else(|| a.get("other").and_then(|o| o.as_str()).map(str::to_string));
                s.is_some_and(|t| t.contains("non_exhaustive"))
            })
        })
}

/// The exact sentinel the `#[define_in_ipe]` marker attribute (the `ipe_bindgen`
/// companion proc-macro) attaches to an item as a `#[doc = "…"]` string.
///
/// `#[define_in_ipe]` is an INERT attribute: it re-emits the annotated item
...
const IPE_DEFINE_MARKER: &str = "ipe-ffi-define-marker";

/// True when a rustdoc item carries the `#[define_in_ipe]` marker — i.e. one of
/// its `docs` lines is EXACTLY the [`IPE_DEFINE_MARKER`] sentinel.
///
...
fn ipe_define_marked(item: &serde_json::Value) -> bool {
    item.get("docs")
        .and_then(|d| d.as_str())
        .is_some_and(|docs| docs.lines().any(|line| line.trim() == IPE_DEFINE_MARKER))
}

/// The NAMES of every crate-local item carrying the `#[define_in_ipe]` marker.
///
...
fn ipe_define_marked_names(doc: &serde_json::Value) -> std::collections::HashSet<String> {
...
/// Whether a foreign type STRING names one of the `#[define_in_ipe]`-marked
...
fn variant_kind(variant_inner: &serde_json::Value) -> Option<(&'static str, Vec<String>)> {
    ...
    if let Some(tup) = kind.get("tuple") {
        ...
        let ids: Vec<String> = tup.as_array()?.iter().map(item_id_to_str).collect();
...
/// Resolve a variant's field-type ids to `(field_name, type_json, type_ipe,
...
fn resolve_variant_fields(
    ...
    for fid in field_ids {
        ...
        let ipe = rustdoc_type_to_ipe(ty, aliases);
...
fn emit_enum_bindings(
    ...
    let enum_ipe = rustdoc_type_to_ipe(&recv_path, aliases);
    let enum_rust = rustdoc_type_to_rust_str(&recv_path);
    if enum_ipe.is_empty() || enum_rust.is_empty() {
        return;
    }
    ...
    for vid in &variant_ids {
        ...
        // wide-int gate). Ipe has no clean FFI tuple, so a multi-field variant
    ...
    functions.push(Function {
        ...
        params: vec![Param {
            name: "e".into(),
            ty: enum_ipe.clone(),
            ipe_type: enum_ipe.clone(),
            rust_type: enum_rust.clone(),
        }],
        results: vec![Param {
            name: String::new(),
            ty: "String".into(),
            ipe_type: "String".into(),
            rust_type: "String".into(),
        }],
        effect: "pure".into(),
        exported: true,
        // E5: the tag's Ipe name is `tag_of_<E>` — it ALREADY embeds the enum
        // name, so it must NOT get the `_from_<E>` disambiguation suffix the
        // dedup/.ipei/kernel.json naming derives from `recv_type`. Leave
...
/// `(name, type_json, ipe, rust)` tuples in declaration order; params take the
/// field Ipe/Rust types, the result is the opaque enum. INFALLIBLE (no Result).
fn make_enum_ctor(
    variant_name: &str,
    kind: &str,
    ...
    Function {
        ...
        results: vec![Param {
            name: String::new(),
            ty: enum_ipe.to_string(),
            ipe_type: enum_ipe.to_string(),
            rust_type: enum_rust.to_string(),
        }],
        effect: "pure".into(),
        exported: true,
        recv_type: enum_ipe.to_string(),
...
fn make_enum_extract(
    ...
    // `wrapperRefName` → kernel.json / .ipei / dedup identically.
    let infix = if single {
        String::new()
    } else if kind == "struct" {
        format!("_{fname}")
    ...
    // Tiny helper kept adjacent to its sole use for locality.
...
/// per-symbol markdown report to `.ipe/cache/ffi/rust/<crate>.coverage.md` when
/// the crate name validates (#6). `--audit` is NOT required.
fn emit_generic_coverage(crate_name: &str) {
    let bound = GENERIC_BOUND.with(std::cell::Cell::get);
    ...
    let report_path =
        safe_crate_name(crate_name).map(|c| format!(".ipe/cache/ffi/rust/{c}.coverage.md"));
    let report_note = report_path.as_deref().map_or_else(
        || " — report: (skipped: unsafe crate name)".to_string(),
        |p| format!(" — report: {p}"),
    );
    if drops.is_empty() {
    ...
    if let Some(path) = report_path {
        ...
        let _ = writeln!(body, "# FFI generic coverage — {crate_name}\n");
        let _ = writeln!(
            body,
        ...
        if !drops.is_empty() {
            ...
            for (sym, d) in &drops {
                let _ = writeln!(body, "| `{}` | {} | {} |", sym, d.reason(), d.detail());
            }
            body.push('\n');
        }
...
fn tail_audit_enabled() -> bool {
    TAIL_AUDIT_ENABLED.with(std::cell::Cell::get)
}
...
fn drop_is_valuable(name: &str, recv: Option<(&str, &str)>) -> bool {
    recv.is_none()
        || matches!(
            name,
            "new" | "default" | "parse" | "build" | "open" | "create"
        )
...
///
/// `recv` is `Some((ipe_type, rust_type))` for methods, `None` for free fns.
// Large rustdoc-JSON walker; the single dispatch reads clearer whole than split.
#[allow(clippy::too_many_lines)]
fn parse_fn_item(
    name: &str,
    ...
    // Ipe would bypass the very invariants `unsafe` exists to protect.
    //
    ...
    // concrete Ipe-representable type. `None` => unresolvable => drop the fn,
    ...
    let inputs_ok = sig["inputs"].as_array().is_none_or(|ins| {
        ins.iter()
            .all(|i| impl_traits_resolvable(i, BoundPos::Param))
    });
        ...
        if !output_ok && contains_iterator_impl_trait(output) {
            record_generic_drop(
                ...
                GenericDrop::IteratorDrop(
                    ...
                    "return-position impl IntoIterator/Iterator (undecidable finiteness)"
                        .to_string(),
                ),
            );
        }
    ...
    {
        let input_dyn = inputs
            .iter()
            .find(|i| is_dyn_trait_object_including_fn(&i[1]));
        let output_dyn = if is_dyn_trait_object_including_fn(output) {
            Some(output)
        } else {
            None
        };
    ...
    if let Some((ipe_ty, rust_ty)) = recv
        && has_self_in_inputs
    {
        params.push(Param {
            name: "self".into(),
            ty: ipe_ty.to_string(),
            ipe_type: ipe_ty.to_string(),
    ...
    for input in inputs {
        ...
        let ipe = rustdoc_type_to_ipe(&type_json, aliases);
        ...
        let (recv_ipe, recv_rust) = recv.unwrap_or(("", ""));
        let ipe = subst_self(&ipe, recv_ipe);
        let rust = subst_self(&rust, recv_rust);
        params.push(Param {
    ...
    if !output.is_null() {
        ...
        let ipe = rustdoc_type_to_ipe(&out_json, aliases);
    ...
    let self_returning = {
        let recv_rust = recv.map_or("", |(_, r)| r);
        let self_is_mut = !is_async
            && inputs
                .first()
                .filter(|i| i[0].as_str() == Some("self"))
                .is_some_and(|i| rustdoc_type_to_rust_str(&i[1]) == "&mut Self");
    ...
    // normalize the result to the OWNED receiver type. This makes the Ipe
    ...
    if self_returning {
        ...
        results = vec![Param {
            name: String::new(),
            ty: recv_ipe.to_string(),
            ipe_type: recv_ipe.to_string(),
            rust_type: recv_rust.to_string(),
        }];
    }
    ...
    // These are iterator / borrowed-view types that aren't usable from Ipe
    ...
    // Skip fixed-size arrays / slices (`[u8; 16]`, `&[u8]`, `&[u8; 6]`).  Ipe's
    ...
    // `is_ipe_coercible_elem`'s clone-opaque arm admits `Clone + !Send` structs
    ...
    // the E slot is the IpeError conversion path (always String-Debug → Send).
    //
    // Void async (`results` empty, Ipe binding `Task Error ()`) passes — () is Send.
    //
    ...
    if effect == "effectful" {
        ...
        let non_send_param = params
            ...
            .skip(usize::from(
                recv.is_some() && params.first().is_some_and(|p| p.name == "self"),
            ))
        ...
        let is_instance_method = recv.is_some() && params.first().is_some_and(|p| p.name == "self");
        if is_instance_method {
            let recv_rt = recv.map_or("", |(_, r)| r);
    ...
    Some(Function {
        ...
        effect,
        exported: true,
        recv_type: recv_ipe.to_string(),
        recv_rust_type: recv_rust.to_string(),
        method_name: if recv.is_some() {
            name.to_string()
        } else {
            String::new()
        },
...
thread_local! {
    ...
    // type renderers (`rustdoc_type_to_ipe` / `rustdoc_type_to_rust_str`) expand a
    ...
    // array/slice filter drops constructors that can't take a Ipe Vec<u8>.
    ...
    // `for_node` is a SYNTHETIC `{"resolved_path":{"__ipe_xc_path": "<public path>"}}`
    ...
    // generated ipe-app Cargo.toml resolves it). A non-nameable / non-reachable / not-
    ...
    // Ipe-coercible closed set (primitive/String/Vec/Option/clone-opaque) — all of
...
/// named field passes `field_type_eligible` (the Ipe-coercible closed set, every
...
fn collect_all_fields_send_type_ids(
    ...
    for (id, item) in index {
        ...
        let Some(struct_data) = item["inner"].get("struct") else {
            continue;
        };
        ...
        let Some(plain) = kind.and_then(|k| k.get("plain")) else {
            continue;
        };
        ...
        for fid in field_ids {
            ...
            let Some(field_item) = index.get(&key) else {
                all_send = false;
                break;
            };
            let Some(fty) = field_item["inner"].get("struct_field") else {
                all_send = false;
                break;
            };
...
fn collect_provably_send_recv_names(
    ...
    for (_id, item) in index {
        ...
        let Some(trait_id) = trait_node.get("id").map(item_id_to_str) else {
            continue;
        };
...
fn recv_provably_async_send(recv_rust: &str) -> bool {
    ...
    let in_recv_set =
        PROVABLY_SEND_RECV_NAMES.with(|c| c.borrow().contains(base) || c.borrow().contains(last));
...
fn collect_external_trait_paths(doc: &serde_json::Value) -> HashMap<String, String> {
    ...
    for (id, entry) in paths {
        ...
        if !joined.is_empty() {
            ...
            let public_path = if let Some(r) = public_path.strip_prefix("core::str::traits::") {
                format!("core::str::{r}")
...
fn collect_generic_result_aliases(
    ...
    for (id, item) in index {
        ...
        let Some(body) = alias.get("type").or_else(|| alias.get("type_")) else {
            continue;
        };
...
fn generic_alias_body_is_result_or_option(
    ...
    let Some(rp) = body.get("resolved_path") else {
        return false;
    };
...
fn expand_generic_alias(rp: &serde_json::Value) -> Option<serde_json::Value> {
    let id = rp.get("id")?;
    let key = item_id_to_str(id);
    let (params, body) = GENERIC_ALIAS_MAP.with(|c| c.borrow().get(&key).cloned())?;
...
fn subst_generic_params(
    ...
    match ty {
        ...
        serde_json::Value::Array(arr) => {
            serde_json::Value::Array(arr.iter().map(|v| subst_generic_params(v, subst)).collect())
        }
        other => other.clone(),
    }
}
...
const ALWAYS_NAMEABLE: &[&str] = &[
    "Option", "Result", "Vec", "Box", "Rc", "Arc", "Mutex", "RwLock", "Cell", "RefCell", "HashMap",
    "BTreeMap", "HashSet", "BTreeSet", "VecDeque", "Cow", "String",
];
...
fn walk_module_with_path(
    ...
    let Some(module) = index.get(mid).and_then(|it| it["inner"].get("module")) else {
        return;
    };
    let Some(items) = module["items"].as_array() else {
        return;
    };
    for child in items {
        let cid = item_id_to_str(child);
        let Some(it) = index.get(&cid) else { continue };
        ...
        if let Some(u) = it["inner"].get("use") {
            ...
            if u["is_glob"].as_bool().unwrap_or(false) {
                ...
                if index.get(tids).is_some_and(item_is_type) {
                    insert_shorter_path(out, tids.clone(), format!("{mp}::{n}"));
                } else if index
                    .get(tids)
                    .is_some_and(|t| t["inner"].get("function").is_some() && is_public(t))
                {
...
/// True if a rust-type token is a single recognised Ipe-coercible element:
...
fn is_ipe_coercible_elem(s: &str) -> bool {
    let s = s.trim();
    if s.is_empty() {
        return false;
    }
    ...
    // Closed-set numerics (each soundly mapped to Ipe Int/Float by the
    ...
    // `OsString`/`PathBuf` (not Ipe-closed), and any dep-crate / non-Clone
...
/// Async-output variant of `is_ipe_coercible_elem`.
///
/// `tokio::task::spawn` requires `Output: Send + 'static`.  `is_ipe_coercible_elem`
...
/// clone-opaque arm.  Sync-FFI callers continue to use `is_ipe_coercible_elem`
...
fn is_async_send_seq(rt: &str) -> bool {
    ...
    let (is_ref, body) = if let Some(b) = t.strip_prefix('&') {
        (true, b.trim())
    } else {
        (false, t)
    };
...
/// True if `rt` is a sequence shape whose element is Ipe-coercible:
/// `&[T]` / `Vec<T>` / `[T; N]` / `&[T; N]` where T passes `is_ipe_coercible_elem`.
fn is_coercible_seq(rt: &str) -> bool {
    ...
    if !is_ref
        ...
        && let Some(elem) = rest.strip_suffix('>')
    {
        return is_ipe_coercible_elem(elem.trim());
    }
    ...
    if let Some(inner) = body.strip_prefix('[').and_then(|s| s.strip_suffix(']')) {
        ...
        return is_ref && is_ipe_coercible_elem(inner.trim());
    }
    false
}
...
fn classify_effect(output: &serde_json::Value, _params: &[Param], is_async: bool) -> String {
    if is_async {
    ...
    let ipe = rustdoc_type_to_ipe(output, &HashMap::new());
    if ipe.starts_with("Result ") {
        return "fallible".into();
    }
    if ipe.starts_with("Task ") {
        return "effectful".into();
    }
    "pure".into()
}
...
fn emit_display_fromstr_bridges(
    ...
    for (ipe_ty, rust_ty) in display_types {
    ...
    for (ipe_ty, rust_ty) in fromstr_types {
        functions.push(Function {
...
// ── rustdoc JSON type → Ipe type string ───────────────────────────────
//
...
fn rustdoc_type_to_ipe(val: &serde_json::Value, aliases: &HashMap<String, String>) -> String {
    if val.is_null() {
        return "()".to_string();
    }

    // [#47(a)] Sentinel for a serde-bound generic reduced to serde_json::Value.
    // Ipe surface type is `String` (the JSON text).
    if val.get("__ipe_serde_value").is_some() {
        return "String".to_string();
    }

    // { "primitive": "bool" | "usize" | "str" | … }
    if let Some(prim) = val.get("primitive") {
        return type_str_to_ipe(prim.as_str().unwrap_or(""), aliases);
    }

    // { "borrowed_ref": { "type": … } }  →  strip the reference
    if let Some(inner) = val.get("borrowed_ref") {
        return rustdoc_type_to_ipe(inner_type(inner), aliases);
    }
    ...
    if let Some(rp) = val.get("resolved_path") {
        // [WALL-G #84] A synthetic cross-crate concrete node — the Ipe surface type is
        ...
        // unify at the Ipe call site. Must mirror the rust-string path's verbatim emit.
        if let Some(xc) = rp.get("__ipe_xc_path").and_then(|p| p.as_str()) {
            let seg = xc.rsplit("::").next().unwrap_or(xc);
            return type_str_to_ipe(seg, aliases);
        }
        // [WALL 5 / #63] See through a crate-local GENERIC Result/Option alias
        // identically to the rust-string path above, so the Ipe surface and the
        ...
        if let Some(_g) = AliasDepthGuard::enter() {
            if let Some(expanded) = expand_generic_alias(rp) {
                return rustdoc_type_to_ipe(&expanded, aliases);
            }
            // See through a crate-local non-generic alias (uuid::Bytes -> [u8;16]).
            if let Some(aliased) = rp.get("id").and_then(resolve_alias) {
                return rustdoc_type_to_ipe(&aliased, aliases);
            }
        }
        return resolve_path_to_ipe(rp, aliases);
    }
    ...
    if let Some(items) = val.get("tuple") {
        let parts: Vec<String> = items
            ...
            .map(|arr| {
                arr.iter()
                    .map(|t| rustdoc_type_to_ipe(t, aliases))
                    .collect()
            })
            .unwrap_or_default();
    ...
    if let Some(inner) = val.get("slice") {
        return format!("List {}", rustdoc_type_to_ipe(inner, aliases));
    }

    // { "array": { "type": …, "len": "N" } }
    if let Some(arr) = val.get("array") {
        let inner = rustdoc_type_to_ipe(inner_type(arr), aliases);
...
fn resolve_path_to_ipe(rp: &serde_json::Value, aliases: &HashMap<String, String>) -> String {
    ...
    let match_name = name.rsplit_once("::").map_or(name, |(_, last)| last);

    // Ipe type application is left-associative, so a multi-word generic arg
    ...
    match match_name {
        ...
        "Box" | "Arc" | "Rc" | "Mutex" | "RwLock" | "Cell" | "RefCell" => args
            .first()
            .cloned()
            .unwrap_or_else(|| match_name.to_string()),
        ...
        "Result" => {
            // Rust Result<T, E>  →  Ipe Result E T
        ...
        _ => {
            ...
            let base = name.rsplit_once("::").map_or(name, |(_, last)| last);
            if let Some(alias) = aliases.get(base) {
                return alias.clone();
            }
            // Disambiguate same-named types in different submodules by deriving
            // the Ipe name from the type's QUALIFIED public path (the same
            ...
            match rp.get("id").and_then(reachable_local_path) {
                Some(full) => ipe_name_from_path(&full),
                None => base.to_string(),
            }
        }
    }
...
/// A Ipe type name that the codegen's `isKnownIpe` resolves to a BUILTIN Rust
/// type (`ipeTypeToRust`), so an opaque crate type with this bare name would be
/// mis-resolved (`bytes::Bytes` → `Vec<u8>`). Matches `isKnownIpe`'s atoms.
fn collides_with_ipe_builtin(n: &str) -> bool {
    matches!(n, "Bytes" | "String" | "Int" | "Float" | "Bool")
}

/// Derive a Ipe type name from a qualified crate path, CamelCase-joining any
/// submodule segments so same-named types in different submodules get distinct
/// Ipe names: `regex::Regex` → `Regex`, `regex::bytes::Regex` → `BytesRegex`,
/// `chrono::format::Parsed` → `FormatParsed`. The crate segment is dropped —
/// EXCEPT when the result would collide with a Ipe builtin (`bytes::Bytes` →
/// `BytesBytes`, not `Bytes`), which the codegen would otherwise resolve to the
/// builtin Rust type instead of the crate type.
fn ipe_name_from_path(path: &str) -> String {
    ...
    if collides_with_ipe_builtin(&joined) && !segs.is_empty() {
...
fn extract_angle_type_args(
    ...
    args.and_then(|a| a.get("angle_bracketed"))
        ...
        .map(|arr| {
            arr.iter()
                .filter_map(|arg| arg.get("type").map(|t| rustdoc_type_to_ipe(t, aliases)))
                .collect()
        })
        .unwrap_or_default()
}
...
// ipe_ffi::pkginfo reads for coercion decisions.
...
fn rustdoc_type_to_rust_str(val: &serde_json::Value) -> String {
    ...
    if let Some(inner) = val.get("borrowed_ref") {
        ...
        return format!(
            ...
            lt,
            m,
            rustdoc_type_to_rust_str(inner_type(inner))
        );
    }
    ...
    if let Some(rp) = val.get("resolved_path") {
        ...
        // ipe-app crate.  External types whose canonical path runs through a
...
fn type_str_to_ipe(s: &str, aliases: &HashMap<String, String>) -> String {
    let s = s.trim();

    if let Some(ipe) = aliases.get(s) {
        return ipe.clone();
    }
    ...
    match s {
        ...
        | "usize" => "Int".into(),
        "f32" | "f64" => "Float".into(),
        // A Rust `char` is one Unicode scalar value = Ipe `Char` (the Rust
        // TypeRenderer lowers Ipe `Char -> char`; char_kernel round-trips it).
        "char" => "Char".into(),
        "IpeError" | "Error" => "Error".into(),
        _ => {
            // Strip module path
            let base = s.rsplit_once("::").map_or(s, |(_, l)| l);
...
fn inner_type(wrapper: &serde_json::Value) -> &serde_json::Value {
    let t = &wrapper["type"];
    if t.is_null() { &wrapper["type_"] } else { t }
}
...
// A generic param bound maps to a concrete, Ipe-representable type that we
...
// already coerces from Ipe List Int / String.
...
fn string_node() -> serde_json::Value {
    // Use {"primitive":"str"} so type_to_typeref → TRPrim "String" → generator
...
// `fn f(x: serde_json::Value)`. Ipe's surface type is `String` (JSON text).
//
...
/// — the wrapper deserialises the Ipe String to an owned `serde_json::Value`
/// local and passes `&local` (a borrow of an owned value living for the call).
/// A `&mut T` STAYS inadmissible (the host could mutate a value the Ipe side
...
fn serde_occurrence_admissible_in(
    ...
    if let Some(items) = val.get("tuple").and_then(|v| v.as_array()) {
        return items.iter().all(|item| {
            serde_occurrence_admissible_in(item, name, OccurrenceCtx::Inadmissible, false)
        });
    }
    ...
    if let Some(rp) = val.get("resolved_path") {
        ...
        let type_args: Vec<&serde_json::Value> = rp
            .get("args")
            .and_then(|a| a.get("angle_bracketed"))
            .and_then(|ab| ab.get("args"))
            .and_then(|v| v.as_array())
        ...
        match leaf {
            ...
            "Result" | "Option" | "Vec" => {
                ...
                return type_args
                    .iter()
                    .all(|t| serde_occurrence_admissible_in(t, name, ctx, false));
            }
            // HashMap / BTreeMap / IndexMap: key (arg[0]) is fine; value
            // (arg[1]) is INADMISSIBLE for T (no Ipe coercion for map values).
            "HashMap" | "BTreeMap" | "IndexMap" | "AHashMap" => {
    ...
    if let Some(inner) = val
        .get("slice")
        .or_else(|| val.get("array").and_then(|a| a.get("type")))
    {
...
fn fn_serde_param_all_admissible_ext(
    ...
    let inputs_ok = sig["inputs"].as_array().is_none_or(|inputs| {
        inputs.iter().all(|inp| {
            ...
            serde_occurrence_admissible_in(
                ty,
                name,
                OccurrenceCtx::Admissible,
                allow_serialize_input_ref,
            )
        })
    });
    if !inputs_ok {
...
/// to its canonical Ipe-coercible concrete type-JSON node, or None if X isn't
/// representable in Ipe. Recursive: handles primitives, paths, slices.
fn concrete_for_inner_type(t: &serde_json::Value) -> Option<serde_json::Value> {
    ...
    // resolved_path: known std/prelude types map to Ipe concretes
    if let Some(rp) = t.get("resolved_path") {
        ...
        return match leaf {
            "String" | "OsString" | "PathBuf" | "Path" | "OsStr" => Some(string_node()),
            "Vec" => {
                let inner = rp
                    .get("args")
                    .and_then(|a| a.get("angle_bracketed"))
...
fn bound_to_concrete(bound: &serde_json::Value, pos: BoundPos) -> Option<serde_json::Value> {
    ...
    let args: Vec<&serde_json::Value> = tr
        .get("args")
        .and_then(|a| a.get("angle_bracketed"))
        .and_then(|ab| ab.get("args"))
        .and_then(|v| v.as_array())
    ...
    match name {
        ...
        "Display" | "ToString" => match std_trait_tag(tr) {
            Some("Display" | "ToString") => Some(string_node()),
            _ => None,
        },
        ...
        "AsRef" | "Borrow" | "Into" | "From" => {
            ...
            // pre-#73 bind emitted `-> IpeResult<IpeError, str>` (unsized) = E0277.
            ...
            if let Some(p) = arg.get("primitive").and_then(|p| p.as_str()) {
                ...
                // runtime-owned Ipe value (String/Vec/…) can lend a borrow but can
...
/// Auto/marker traits that don't constrain the Ipe-facing type — ignored when
...
/// a stable coverage tag string. The first three MIRROR the generator
/// `ipe_ffi::instance.closureDropReason` taxonomy (`closure-mut-slot` /
...
enum ClosureDropTag {
    ...
    /// reconstruct a `&mut` from an owned Ipe value).
    MutSlot,
    /// The closure RETURNS a function/closure (`-> impl Fn`) — the closed
    /// Ipe↔Rust set has no function arm to marshal back.
    HoReturn,
...
fn classify_param_bound(bound: &serde_json::Value) -> Result<Option<String>, GenericDrop> {
    ...
    let Some(name) = trait_bound_name(bound) else {
        return Ok(None);
    };
    ...
    if let Some(std_tag) = std_trait_tag(
        bound
            .get("trait_bound")
    ...
    if MARKER_TRAITS.contains(&name.as_str()) && !is_modellable_5(&name) {
        ...
        // NOT one of the modellable-5 contributes no Ipe-facing `<T: …>` bound.
...
fn check_defaulted_params(
    ...
    for p in params {
        ...
        let has_bound = ty
            .and_then(|t| t.get("bounds"))
            .and_then(|b| b.as_array())
            .is_some_and(|bs| bs.iter().any(|b| !is_marker_bound(b)));
...
fn has_const_generic(generics: &serde_json::Value) -> bool {
    generics
        ...
        .is_some_and(|arr| {
            arr.iter()
                .any(|p| p.get("kind").and_then(|k| k.get("const")).is_some())
        })
}
...
fn type_to_typeref(
    ...
    // call-AST node (Ipe surface String; Rust `serde_json::Value`). MUST precede
    ...
    if let Some(p) = val.get("primitive").and_then(|p| p.as_str()) {
        // Closed-set primitive mapping (Ipe Int=i64, Float=f64).
        let r = match p {
    ...
    if let Some(rp) = val.get("resolved_path") {
        ...
        let name: String = if let Some(full) = qualified.or(ext_path).or(xc_path) {
            full
        } else {
            let last = raw_name.rsplit("::").next().unwrap_or(raw_name);
            if ALWAYS_NAMEABLE.contains(&last) {
                ...
                return Err(GenericDrop::NotBindable(format!(
                    "external type `{raw_name}` has no fully-qualified path \
                     in doc[\"paths\"] — bare `::{last}` is unresolvable without \
                     an explicit `use`"
                )));
            }
        };
        ...
        // normalized to IpeError at codegen (#32/#34) — the wrapper consumes it via
        ...
        // best-effort, substituting the IpeError-normalized sentinel (`String`, the
    ...
    // OsString / PathBuf} → lower to `String` on the Ipe side; the call site
    ...
    if let Some(br) = val.get("borrowed_ref") {
        if !is_mutable(br) {
            ...
            // [WALL 3a-&I / #65] `&{__ipe_serde_value}` (non-mut) — a `&T`
            ...
            // substitution). Emit `SerdeValueRef`: Ipe `String` in, `&sv_j` at
            // the call site. MUST precede the str/Path arm (distinct sentinel).
            if inner.get("__ipe_serde_value").is_some() {
                return Ok(TypeRef::SerdeValueRef);
            }
            let is_str_prim = inner.get("primitive").and_then(|p| p.as_str()) == Some("str");
            let is_str_path = inner
                ...
                .is_some_and(|name| {
                    let last = name.rsplit("::").next().unwrap_or(name);
                    matches!(last, "String" | "Path" | "OsStr" | "OsString" | "PathBuf")
                });
    ...
    // `TRCtor("()", [])`: `ipe_of_typeref` yields `"()"` (so `parametric_function`
    // emits no result Param → Ipe `Task Error ()`) and `renderTypeRef` yields
    // the Rust `()`. A NON-empty tuple stays unbindable (no Ipe product-coercion).
...
/// (`usize`/`u32`/`f32`/…) instead of collapsing to the Ipe carrier (i64/f64) the
/// way the recursive `type_to_typeref` does — so the generator projected emitter
...
/// bridge — `closure-by-ref-noclone`). Mirrors the generator `concreteNotClone`
...
/// Clone.
// Arms kept separate to document why each variant is provably Clone.
#[allow(clippy::match_same_arms)]
...
fn classify_closure_param(
        ...
        for input in inputs {
            if let Some(br) = input.get("borrowed_ref") {
                ...
                let inner = br.get("type_").or_else(|| br.get("type")).ok_or_else(|| {
                    GenericDrop::NotBindable(
                        "borrowed closure input without inner type".to_string(),
                    )
                })?;
    ...
    // function arm in the closed Ipe↔Rust set) → closure-ho-return.
...
/// Whether a CLOSURE-companion bound is one a Ipe-synthesized closure can
/// actually satisfy. A generated Ipe closure wrapper is `Sized` by construction
...
/// `recv` is `Some((ipe, rust, struct_generics, impl_generics))` for a struct
...
fn try_parametric_stub(
        ...
        Some(tc) if !tc.assoc_bindings.is_empty() || contains_qualified_path(sig) => {
            ...
            if contains_qualified_path(&sig_owned) {
                return Err(GenericDrop::TraitMethodDrop(
                    ...
                    format!(
                        "unresolved associated-type projection in `{method_name}` \
    ...
    if let Some(inputs) = sig["inputs"].as_array() {
        for input in inputs {
            ...
            // concrete receiver, NOT a Ipe tyvar. Collecting it would make `Self`
    ...
    // trait (`Fn`/`FnMut`/`FnOnce`) is NOT a Ipe tyvar — it is CONSUMED into a
    ...
    // trait (`IntoIterator`/`Iterator`) is NOT a Ipe tyvar — it is CONSUMED into
    ...
    // The Ipe-facing TYVARS = used params MINUS the consumed closure/iterator
    // params.
    ...
    // the {param:i} index and the Ipe tyvar order (round-trips with the <A: …>
            ...
            if let Some(concrete) = resolve_param_bounds(bounds_for, BoundPos::Param) {
                ...
                if !fn_serde_param_all_admissible(fn_data, tv) {
                    return Err(GenericDrop::UnmodellableBound(format!(
                        "mono-inadmissible: param `{tv}` occurs in a non-owned position"
                    )));
                }
                ...
                // T → serde_json::Value, surfaced as a Ipe JSON String.
                //
                ...
                // Ipe String → an owned `serde_json::Value` local and passes
                ...
                // arm only for the exact non-mut `&{__ipe_serde_value}` shape; a
    ...
    let param_idx: HashMap<String, usize> = order
        .iter()
        .enumerate()
        .map(|(i, n)| (n.clone(), i))
        .collect();
    ...
    // bounds are consumed into their argType, NOT reduced to a Ipe `<F: …>`
    ...
    // for a TRAIT-impl method, every Ipe-facing tyvar MUST carry a resolved
    ...
    if trait_ctx.is_some() {
        for tv in &tyvars {
            let has_bound = bounds_map.get(tv).is_some_and(|b| !b.is_empty());
            if !has_bound {
                return Err(GenericDrop::TraitMethodDrop(
                    TraitMethodDropTag::BoundCrossImpl,
                    format!(
                        "trait-impl method tyvar `{tv}` has no resolvable bound \
    ...
    let path: Vec<String> = {
        ...
        match segs.split_first() {
            Some((first, rest)) => {
                ...
                v.extend(rest.iter().map(std::string::ToString::to_string));
                v
            }
            None => vec![base.clone()],
        }
    ...
    // SEPARATELY via `_call_receiver` (the generator `renderCall` prepends it,
    ...
    // projected emitter widens it to the Ipe carrier (saturating).
    ...
    // omitted from the wire ⇒ the generator renderer emits no method turbofish.
    //
    // Per-generic resolution when a turbofish is needed:
    //   * mono'd / serde-reduced → its concrete from `mono_map` (`type_to_typeref`
    //     maps the serde `{__ipe_serde_value}` sentinel to `SerdeValue` and a
    ...
    let needs_method_turbofish = method_own_generics.iter().any(|g| mono_map.contains_key(g));
    ...
    Ok(Generic {
        ...
        call: Call {
            ...
            // own generic). EMPTY ⇒ omitted ⇒ generator keeps the single-serde path.
            method_turbofish,
        },
...
fn struct_generics_of<'a>(
    ...
    item["inner"]
        .get("struct")
        .map(|_| &item["inner"]["struct"]["generics"])
}
...
fn impl_assoc_bindings(
    ...
    for member_id in items {
        ...
        let Some(member) = index.get(&mid) else {
            continue;
        };
...
/// for any other shape (a generic / tuple / ref / a synthetic `__ipe_xc_path` node
...
fn resolved_path_type_id(node: &serde_json::Value) -> Option<String> {
    ...
    if rp.get("__ipe_xc_path").is_some() {
        return None;
    }
...
fn sibling_impl_assoc(
    ...
    for item in index.values() {
        let Some(im) = item.get("inner").and_then(|i| i.get("impl")) else {
            continue;
        };
        ...
        let Some(for_t) = im.get("for").or_else(|| im.get("for_")) else {
            continue;
        };
...
fn subst_self_projections(
    ...
    match val {
        serde_json::Value::Object(o) => serde_json::Value::Object(
            o.iter()
                .map(|(k, v)| {
                    (
                        k.clone(),
                        subst_self_projections(v, self_id, index, changed, depth + 1),
                    )
                })
                .collect(),
        ),
        serde_json::Value::Array(a) => serde_json::Value::Array(
            a.iter()
                .map(|v| subst_self_projections(v, self_id, index, changed, depth + 1))
                .collect(),
        ),
        other => other.clone(),
    }
...
fn self_is_closed_monomorphic(for_val: &serde_json::Value) -> bool {
    ...
    for arg in ab_args {
        if let Some(t) = arg.get("type") {
            ...
            if rendered.len() == 1 && rendered.chars().next().is_some_and(char::is_uppercase) {
                return false;
            }
        }
        // Lifetime args: skip.
...
fn self_path_with_concrete_args(for_val: &serde_json::Value) -> Option<String> {
            ...
            arr.iter()
                .filter_map(|arg| {
                    if let Some(t) = arg.get("type") {
                        ...
                        if s.is_empty() || !type_is_nameable(&s) {
                            None
                        } else {
                            Some(s)
                        }
...
fn method_is_generic_bearing(
    ...
    let Some(sig) = fn_data.get("sig").or_else(|| fn_data.get("decl")) else {
        return false;
    };
    let mut used: HashSet<String> = HashSet::new();
    if let Some(inputs) = sig["inputs"].as_array() {
...
/// Build the FFI `Function` for a parametric generic stub: the Ipe-facing sig
/// uses lowercased tyvars (`k`/`v`), the receiver renders as the Wall-#1
/// parametric Ipe type (`IndexMap k v`), and the `generic` block carries the
...
fn parametric_function(
    ...
    // Ipe tyvar names = the lowercased stub params (Wall #1 round-trip: Ipe `k`
    // ↔ Rust `<K>`). The receiver Ipe type is the parametric foreign type with
    // its tyvar args, e.g. `IndexMap k v`.
    let tyvars: Vec<String> = generic.params.iter().map(|p| lower_first(p)).collect();
    let recv_ipe_parametric = parametric_ipe_type(self_ipe, &tyvars);

    // Render a Ipe type for a `TypeRef` (used for the value-arg + ret Ipe sides).
    let ipe_of = |tr: &TypeRef| ipe_of_typeref(tr, &tyvars, self_ipe);
    ...
    if has_self {
        params.push(Param {
            name: "self".into(),
            ty: recv_ipe_parametric.clone(),
            ipe_type: recv_ipe_parametric.clone(),
            rust_type: self_rust.to_string(),
        });
        at += 1;
    }
    // Value args (non-self inputs), Ipe types from the call's argTypes.
    for input in inputs {
        if input[0].as_str() == Some("self") {
            continue;
        }
    ...
    // Result.
    let mut results: Vec<Param> = Vec::new();
    let ret_ipe = ipe_of(&generic.call.ret);
    if ret_ipe != "()" {
        results.push(Param {
            name: String::new(),
            ty: ret_ipe.clone(),
    ...
    // trip the Ipe compiler's NON-generic inherent-method path (`arg0.m()`),
    ...
    // to avoid E0659 (duplication between _bindings.rs and ipe_ffi_generics.rs)
    // and E0425 (wrong free-function call from _bindings.rs for trait methods):
    //
    // INHERENT mono: drop `generic` entirely.  The generator emitter sees
    //   `generic = None` → no ipe_ffi_generics.rs entry at all.  The regular
    ...
    //
    // TRAIT mono: keep `generic` so ipe_ffi_generics.rs emits the UFCS call
    //   (`<Self as Trait>::method(&arg0, arg)`).  Set `recv_rust_type` to a
    //   dummy `<M>`-suffixed form to trigger `hasGenericRecvParam=true` in
    //   the generator's `emitRustFnSimple`.  Also keep `method_name` non-empty so
    ...
    // [WALL 3a / #59] An async host method surfaces in Ipe as `Task Error _`
    ...
    let mono_effect = if generic.call.is_async {
        "effectful"
    } else {
        "pure"
    };
    if is_wall2_mono {
        if is_trait_method {
            // Trait mono: keep generic for ipe_ffi_generics.rs UFCS, but use a
            ...
            return Some(Function {
                ...
                exported: true,
                recv_type: recv_ipe_parametric.clone(),
                recv_rust_type: format!("{self_rust}<M>"),
    ...
    // `isInstance` nor `isStaticFn` fires in the generator wrapper generator (the
    ...
    // MUST leave `recv_type` / `method_name` EMPTY so the Ipe compiler routes it
    // ONLY through `ipe_ffi_generics.rs` (UFCS) and does NOT also emit an
    ...
    // skip gate in generator).  An inherent method (no qualifier) KEEPS them.
    let (out_recv_type, out_recv_rust, out_method_name) = if is_trait_method {
        (String::new(), String::new(), String::new())
    } else {
        (
            recv_ipe_parametric.clone(),
            self_rust.to_string(),
    ...
    // `recv_type=""` (C-4 above) so `wrapperRefName` in generator can't distinguish
...
/// Render the Wall-#1 parametric Ipe type for a receiver: the bare Ipe type name
/// (the receiver's `self_ipe` with any existing tyvar suffix stripped) followed
/// by the stub's lowercased tyvars, e.g. `IndexMap` + `[k,v]` → `IndexMap k v`.
fn parametric_ipe_type(self_ipe: &str, tyvars: &[String]) -> String {
    // self_ipe may already be `IndexMap k v` (rustdoc_type_to_ipe renders the
    // generic args). Take only the head constructor token.
    let head = self_ipe.split_whitespace().next().unwrap_or(self_ipe);
    if tyvars.is_empty() {
        head.to_string()
    } else {
...
/// [WALL-I] Last path segments that BOTH Ipe-rendering paths keep type args for —
/// i.e. they are Ipe-PARAMETRIC and their args become real Ipe type application,
...
/// the receiver path (`resolve_path_to_ipe`'s `_` arm drops its args, treating the
/// type as an opaque nominal), so the parametric-RETURN path (`ipe_of_typeref`)
...
/// consistent): `ipe_of_typeref` keeps the literal last segment (`Vec t`,
/// `HashMap k v`) while `resolve_path_to_ipe` remaps (`List t`, `Dict String v`).
...
/// job is to never collapse a real Ipe-container's args — keep it a superset of
/// every with-args arm of `resolve_path_to_ipe`.
fn is_ipe_container_head(last: &str) -> bool {
    matches!(
        last,
        "Vec"
            | "Option"
...
/// Render a `TypeRef` as a Ipe type string (parametric — tyvars, not concretes).
// Serde arms kept separate to document their distinct #59/#65 provenance.
#[allow(clippy::match_same_arms)]
fn ipe_of_typeref(tr: &TypeRef, tyvars: &[String], self_ipe: &str) -> String {
    match tr {
        TypeRef::Param(i) => tyvars.get(*i).cloned().unwrap_or_else(|| "a".to_string()),
        TypeRef::Prim(p) => match p.as_str() {
            // [#95] Map EVERY numeric width to its Ipe carrier — the IPE-FACING
            // type is always `Int`/`Float`, never a foreign width. #95 made the
            // call-AST `TypeRef` PRESERVE the foreign width (`usize`/`u32`/`f32`)
            // so the generator codegen can coerce it; but this renderer builds the
            // `.ipei` Ipe surface type, which must stay `Int`/`Float` (a leaked
            // `u32`/`usize` would be an undefined Ipe type — wrong in `ipe doc`
            // and against any importer's `Int` annotation). Codegen reads the
            // call-AST TypeRef directly (generator `renderTypeRef`), NOT this fn.
        ...
        TypeRef::Ctor(name, args) => {
            // The receiver/foreign ctor: use the Ipe head from self_ipe when the
            // path matches the receiver; otherwise the last `::` segment.
            let last = name.rsplit("::").next().unwrap_or(name);
            // [WALL 3a / #59] A `Result<T, E>` host return (the firestore
            // `get_obj`/`put_obj -> Result<T, String>` shape) maps to Ipe
            // `Result E T` (error-FIRST), matching `rustdoc_type_to_ipe` (4666).
            // Rendering it positionally as `Result T E` would put the OK type in
            // the error slot — `wrapperIpeType`'s `stripResultLayer` then keeps
            ...
            if last == "Result" && args.len() == 2 {
                let ok = {
                    let s = ipe_of_typeref(&args[0], tyvars, self_ipe);
                    if s.contains(' ') { format!("({s})") } else { s }
                };
                let err = {
                    let s = ipe_of_typeref(&args[1], tyvars, self_ipe);
                    if s.contains(' ') { format!("({s})") } else { s }
                };
                return format!("Result {err} {ok}");
            }
            let head = {
                // Prefer the receiver's Ipe name for the receiver ctor.
                if self_ipe.split_whitespace().next() == Some(last) {
                    self_ipe
                        .split_whitespace()
                        .next()
                        .unwrap_or(last)
            ...
            if args.is_empty() {
                head
            } else if !is_ipe_container_head(last) {
                // [WALL-I] A crate-local OPAQUE generic struct (e.g. the stripe
                // `Customizable<Resp>` / `CustomizableStripeRequest<Customer>`).
                // The RECEIVER path (`resolve_path_to_ipe`'s `_` arm) renders such
                // a type BARE — the Ipe surface treats it as an opaque nominal
                // (its type arg is an internal Rust detail, never a Ipe tyvar).
                ...
                // receiver renders bare `Customizable`, Ipe HM can't unify them
                ...
                // keep their type application — they ARE Ipe-parametric.
                head
            } else {
                let parts: Vec<String> = args
                    .iter()
                    .map(|a| {
                        let s = ipe_of_typeref(a, tyvars, self_ipe);
                        if s.contains(' ') { format!("({s})") } else { s }
                    })
                    .collect();
                // [#90] Map the known-container HEAD to its Ipe SEMANTIC name so the
                // projected/UFCS `.ipei` surface AGREES with the inherent path
                // (`resolve_path_to_ipe`): a producer's `List t` must unify with a
                // consumer's `List t` receiver — pre-#90 this path emitted the RAW
                // Rust head (`Vec t` / `Option t` / `Dict`'s `HashMap k v`), an
                // undefined Ipe type that also broke cross-path HM unification.
                // Bounded to the three unambiguous containers the divergence names;
                // `Dict` drops the key arg (Ipe Dict is always String-keyed, exactly
                // as `resolve_path_to_ipe`). Surface-only — codegen reads the
                // call-AST TypeRef, never this string (the #95 invariant).
                match last {
                    // `unwrap_or_else(|| "String")` fallback matches `resolve_path_to_ipe`
                    // exactly (degenerate/malformed-arity input only — well-formed
                    // rustdoc always supplies the args).
                    "Vec" => format!(
                        "List {}",
                        parts.first().cloned().unwrap_or_else(|| "String".into())
                    ),
                    "Option" => format!(
                        "Maybe {}",
                        parts.first().cloned().unwrap_or_else(|| "String".into())
                    ),
                    "HashMap" | "BTreeMap" | "IndexMap" | "AHashMap" => format!(
                        "Dict String {}",
                        parts.get(1).cloned().unwrap_or_else(|| "String".into())
                    ),
                    _ => format!("{head} {}", parts.join(" ")),
                }
            }
        }
        // A closure argType renders as a Ipe function type. `byRef` is invisible
        // to Ipe (the borrow never escapes — B4); the Ipe side only sees the
        // owned arg/ret types. Zero-arg ⇒ `(() -> R)` (the `run_once` shape).
        TypeRef::Closure { arg_types, ret, .. } => {
            let ret_ipe = ipe_of_typeref(ret, tyvars, self_ipe);
            let arrow = if arg_types.is_empty() {
                ...
                let ins: Vec<String> = arg_types
                    ...
                    .collect();
                format!("{} -> {ret_ipe}", ins.join(" -> "))
            };
            format!("({arrow})")
        }
        // [WALL 3a / #59] A serde-reduced param/return surfaces in Ipe as
        // `String` (the JSON text), exactly as the #47 inherent arm does
        // (`rustdoc_type_to_ipe`'s `__ipe_serde_value` → "String").
        TypeRef::SerdeValue => "String".to_string(),
        // [WALL 3a-&I / #65] A `&T` serde-Serialize INPUT also surfaces in Ipe as
        // `String` — identical Ipe surface to `SerdeValue`. The reference is an
        // internal codegen detail (`&sv_j` at the call site); Ipe never sees it.
    ...
    for b in bounds {
        ...
        if !local {
            ...
            let is_std = TRAIT_ID_CANON_PATH.with(|c| {
                c.borrow().get(&key).is_some_and(|p| {
...
// Large rustdoc-JSON walker; the single dispatch reads clearer whole than split.
// Outer Option = param applies?; inner Option = present-but-null — both meaningful.
...
fn monomorphize_concrete_impl_params(
    ...
    for p in params {
        ...
        let mut bounds: Vec<serde_json::Value> = p["kind"]["type"]["bounds"]
            ...
            .cloned()
            .unwrap_or_default();
        if let Some(extra) = where_bounds.get(name) {
        ...
        if let Some(concrete) = concrete_for_unique_impl(&trait_key) {
            ...
            record_generic_drop(
                ...
                GenericDrop::TraitBoundedParamAmbiguous(
                    trait_bound_name_of_key(&trait_key).unwrap_or_else(|| trait_key.clone()),
                ),
            );
    ...
    for (param_name, trait_key) in &matched_send_keys {
        ...
        if let Some(xc) = xc_unique_for_trait_key(trait_key) {
            ...
            let same_crate_unique = TRAIT_CONCRETE_IMPLS
                .with(|c| c.borrow().get(trait_key).is_some_and(|v| v.len() == 1));
        ...
        let Some(concrete) = concrete_for_unique_impl(trait_key) else {
            continue;
        };
        ...
        let via_explicit_impl = concrete_id
            .as_ref()
            .is_some_and(|id| EXPLICIT_SEND_TYPE_IDS.with(|s| s.borrow().contains(id)));
        let via_all_fields = concrete_id
            .as_ref()
            .is_some_and(|id| concrete_type_all_fields_send(id));
...
fn register_self_mono_send(trait_key: &str, concrete: &serde_json::Value) {
    if let Some(xc) = xc_unique_for_trait_key(trait_key) {
        let same_crate_unique =
            TRAIT_CONCRETE_IMPLS.with(|c| c.borrow().get(trait_key).is_some_and(|v| v.len() == 1));
        if !same_crate_unique {
            if xc.send_ok && !xc.self_public_path.is_empty() {
                PROVABLY_SEND_OPAQUE_NAMES.with(|c| {
    ...
    let Some(id) = concrete.get("resolved_path").and_then(|rp| rp.get("id")) else {
        return;
    };
...
/// primitive that the PARAMETRIC wrapper path cannot coerce at the call site. Ipe
...
fn project_trait_default_methods(
    ...
    if let Some(items) = trait_def.get("items").and_then(|i| i.as_array()) {
        for id in items {
            let Some(mi) = index.get(&item_id_to_str(id)) else {
                continue;
            };
            let Some(fnd) = mi.get("inner").and_then(|i| i.get("function")) else {
                continue;
            };
            ...
            if projected_method_has_unverifiable_where(fnd, &proj) {
                record_generic_drop(
                    ...
                    GenericDrop::TraitMethodDrop(
                        TraitMethodDropTag::DefaultMethodWhereUnsatisfied,
                        format!(
                            "default method `{name}` has a where-bound on a monomorphized trait param"
                        ),
                    ),
                );
                continue;
            ...
            let mut projected =
                subst_assoc_json(&subst_generic_json(fnd, &proj, BoundPos::Param), &assoc);
                ...
                .and_then(|w| w.as_array_mut())
            {
                wps.retain(|wp| {
                    wp.get("bound_predicate")
                        .and_then(|bp| bp.get("type"))
                        .is_none_or(|t| t.get("generic").is_some())
                });
            }
            ...
            let has_bad_param = projected
                ...
                .is_some_and(|ins| {
                    ins.iter()
                        .any(|inp| inp.get(1).is_some_and(type_has_uncoercible_numeric))
                });
            if has_bad_param {
                record_generic_drop(
                    name,
...
fn subst_generic_json(
    ...
    match val {
        serde_json::Value::Object(obj) => serde_json::Value::Object(
            obj.iter()
                .map(|(k, v)| (k.clone(), subst_generic_json(v, map, pos)))
                .collect(),
        ),
        serde_json::Value::Array(arr) => serde_json::Value::Array(
            arr.iter()
                .map(|v| subst_generic_json(v, map, pos))
                .collect(),
        ),
        other => other.clone(),
    }
...
fn resolve_generics(
    ...
    let Some(params) = fn_data["generics"]["params"].as_array() else {
    ...
    for p in params {
        ...
        if kind.get("lifetime").is_some() {
            continue;
        } // lifetime param -> ignore
        let name = p.get("name").and_then(|n| n.as_str()).unwrap_or("");
        let mut bounds: Vec<serde_json::Value> = kind
            .get("type")
            .and_then(|t| t.get("bounds"))
            .and_then(|b| b.as_array())
            .cloned()
        ...
        match resolve_param_bounds(&bounds, BoundPos::Param) {
            Some(concrete) => {
                map.insert(name.to_string(), concrete);
            }
            None => {
...
/// `"String"` fallback in `rustdoc_type_to_ipe`, which is unsound — so the
...
/// satisfiable Ipe fn-value, owned by the CLOSURE seam (`try_parametric_stub`) — it
...
fn dyn_trait_object_present(val: &serde_json::Value, include_fn: bool) -> bool {
    if val.get("dyn_trait").is_some() {
        let is_fn = dyn_trait_principal_name(val)
            .as_deref()
            .is_some_and(is_fn_family_trait_name);
    ...
    match val {
        serde_json::Value::Object(obj) => obj
            .values()
            .any(|v| dyn_trait_object_present(v, include_fn)),
        serde_json::Value::Array(arr) => {
...
mod tests {
    ...
    #[test]
    fn the_define_marker_is_read_from_the_docs_field() {
        // rustdoc folds a `#[doc = "…"]` into `docs` (trimmed), so the marker
        // that the inert `#[define_in_ipe]` macro emits lands there, not in attrs.
        let marked = serde_json::json!({ "docs": IPE_DEFINE_MARKER });
        assert!(ipe_define_marked(&marked));
        // A marker line among ordinary prose still matches (whole-line).
        let mixed = serde_json::json!({
            "docs": format!("Widget doc.\n{IPE_DEFINE_MARKER}\nMore prose.")
        });
        assert!(ipe_define_marked(&mixed));
    }
    ...
    fn ordinary_prose_never_forges_the_marker() {
        ...
        let prose = serde_json::json!({
            "docs": format!("see {IPE_DEFINE_MARKER} for details")
        });
        assert!(!ipe_define_marked(&prose));
        let unmarked = serde_json::json!({ "docs": "just a normal doc comment" });
        assert!(!ipe_define_marked(&unmarked));
        let no_docs = serde_json::json!({ "attrs": ["non_exhaustive"] });
        assert!(!ipe_define_marked(&no_docs));
    }
    ...
    fn marked_names_collects_only_crate_local_marked_items() {
        let doc = serde_json::json!({
            "index": {
                "1": { "name": "Widget", "crate_id": 0, "docs": IPE_DEFINE_MARKER },
                ...
                "3": { "name": "Foreign", "crate_id": 1, "docs": IPE_DEFINE_MARKER }
            }
        });
        let names = ipe_define_marked_names(&doc);
    ...
    fn mk_xc(path: &str) -> XcImpl {
        XcImpl {
            for_node: serde_json::json!({ "resolved_path": { "__ipe_xc_path": path } }),
    ...
    fn wallh_generic_instantiation_send_requires_base_and_all_args() {
        ...
        assert!(is_generic_instantiation_send(
            "req_crate::Customizable<client_crate::Resp>"
        ));
        // primitive arg → Send.
        assert!(is_generic_instantiation_send(
            "req_crate::Customizable<u32>"
        ));
        // arg NOT proven Send (absent from every set) → NOT Send.
        assert!(!is_generic_instantiation_send(
            "req_crate::Customizable<other::NotSend>"
        ));
    ...
    fn wallg_xc_unique_only_when_exactly_one_impl() {
        ...
        GLOBAL_XC_IMPLS.with(|c| {
            c.borrow_mut().insert(
                ...
                vec![mk_xc("client_crate::RealClient")],
            );
        });
        let got = xc_unique_for_canon("crate_a::Wire").expect("unique impl resolves");
        assert_eq!(
            rustdoc_type_to_rust_str(&got.for_node),
            "client_crate::RealClient"
        );
        assert!(got.send_ok);
        ...
        GLOBAL_XC_IMPLS.with(|c| {
            c.borrow_mut().insert(
                "crate_a::Wire".to_string(),
                vec![
                    mk_xc("client_crate::RealClient"),
                    mk_xc("other_crate::OtherClient"),
                ],
            );
        });
        assert!(xc_unique_for_canon("crate_a::Wire").is_none());
    ...
    // Fixture builder: owned JSON args mirror serde_json::json! ergonomics.
    ...
    #[test]
    fn test_primitives() {
        assert_eq!(ipe(&prim("u64")), "Int");
        assert_eq!(ipe(&prim("usize")), "Int");
        assert_eq!(ipe(&prim("i32")), "Int");
        assert_eq!(ipe(&prim("f32")), "Float");
        assert_eq!(ipe(&prim("bool")), "Bool");
        assert_eq!(ipe(&prim("str")), "String");
    }

    #[test]
    fn test_string_path() {
        assert_eq!(ipe(&path("String")), "String");
        assert_eq!(ipe(&path("OsString")), "String");
    }
    ...
    fn owned_string_vs_borrowed_str_rust_type_distinct() {
        ...
        // SAME Ipe surface (`String`), so the only carrier of the distinction is
        ...
        assert_eq!(
            rustdoc_type_to_rust_str(&borrowed(path("String"))),
            "&String"
        );
        // Both collapse to the Ipe `String` surface (proving the Ipe type alone
        // CANNOT distinguish owned from borrowed — `rust_type` must).
        assert_eq!(ipe(&path("String")), "String");
        assert_eq!(ipe(&borrowed(prim("str"))), "String");
    }

    #[test]
    fn test_generic_containers() {
        assert_eq!(ipe(&path_with_args("Vec", vec![prim("u64")])), "List Int");
        assert_eq!(
            ipe(&path_with_args("Option", vec![path("String")])),
            "Maybe String"
        );
        assert_eq!(
            ipe(&path_with_args(
                "HashMap",
                vec![path("String"), prim("u64")]
            )),
            "Dict String Int"
        );
    }
    ...
    fn test_path_qualified_and_nested_containers() {
        ...
        assert_eq!(
            ipe(&path_with_args("Vec", vec![inner])),
        ...
        assert_eq!(
            ipe(&path_with_args("Result", vec![ok, path("String")])),
            "Result String (Dict String String)"
        );
    }

    #[test]
    fn test_result_and_ipe_result() {
        // Rust Result<String, IpeError>  →  Ipe Result Error String
        let r = path_with_args("Result", vec![path("String"), path("IpeError")]);
        assert_eq!(ipe(&r), "Result Error String");

        // IpeResult<IpeError, String>  →  Ipe Result Error String
        let sr = path_with_args("IpeResult", vec![path("IpeError"), path("String")]);
        assert_eq!(ipe(&sr), "Result Error String");
    }
    ...
    fn test_borrowed_ref_stripped() {
        ...
        // &Vec<u64> → List Int
        assert_eq!(
            ipe(&borrowed(path_with_args("Vec", vec![prim("u64")]))),
            "List Int"
        );
    }
    ...
    fn test_setter_receives_losslessly() {
        ...
        // Narrower-than-Ipe-scalar numerics → setter would truncate → dropped.
        ...
        // reinterpreting/truncating the Ipe i64 — must stay dropped.
    ...
    fn test_coercible_elem_gate_tightened() {
        ...
        for t in [
            ...
            assert!(is_ipe_coercible_elem(t), "should admit {t}");
        }

        // DROP — str (unsized: no Vec<str>), OsString/PathBuf (not Ipe-closed),
        ...
        for t in [
            ...
            assert!(!is_ipe_coercible_elem(t), "should DROP {t}");
        }

        // Markers still reject.
        assert!(!is_ipe_coercible_elem("&u8"));
        assert!(!is_ipe_coercible_elem("Vec<u8>"));
        assert!(!is_ipe_coercible_elem(""));
    }
    ...
    fn test_bound_to_concrete() {
        ...
        let asref_u8 = trait_bound(
            "AsRef",
            vec![serde_json::json!({ "slice": { "primitive": "u8" } })],
        );
        assert_eq!(
            bound_to_concrete(&asref_u8, BoundPos::Param),
            Some(vec_u8_node())
        );
        // AsRef<str> / Borrow<str>    -> String (PARAM: call site does `.as_ref()`)
        assert_eq!(
            bound_to_concrete(&trait_bound("AsRef", vec![prim("str")]), BoundPos::Param),
            Some(string_node())
        );
        ...
        assert_eq!(
            bound_to_concrete(&trait_bound("AsRef", vec![prim("str")]), BoundPos::Return),
            None
        );
        assert_eq!(
            bound_to_concrete(&trait_bound("Borrow", vec![prim("str")]), BoundPos::Return),
            None
        );
        ...
        assert_eq!(
            bound_to_concrete(&trait_bound("Into", vec![path("String")]), BoundPos::Return),
            Some(string_node())
        );
        // Display / ToString (no arg) -> String
        assert_eq!(
            bound_to_concrete(&trait_bound("Display", vec![]), BoundPos::Param),
            Some(string_node())
        );
        assert_eq!(
            bound_to_concrete(&trait_bound("ToString", vec![]), BoundPos::Param),
            Some(string_node())
        );
        // Into<Vec<u8>> -> Vec<u8> ; Into<String> -> String
        assert_eq!(
            bound_to_concrete(
                &trait_bound("Into", vec![path_with_args("Vec", vec![prim("u8")])]),
                BoundPos::Param
            ),
            Some(vec_u8_node())
        );
        assert_eq!(
            bound_to_concrete(&trait_bound("Into", vec![path("String")]), BoundPos::Param),
            Some(string_node())
        );
        // Unknown / unsupported -> None
        assert_eq!(
            bound_to_concrete(&trait_bound("FromStr", vec![]), BoundPos::Param),
            None
        );
        ...
        assert_eq!(
            bound_to_concrete(&trait_bound("AsRef", vec![prim("u16")]), BoundPos::Param),
            None
        );
        ...
        assert_eq!(
            bound_to_concrete(
                &trait_bound(
                    "AsRef",
                    vec![
                        serde_json::json!({ "resolved_path": { "name": "SomeWeirdType", "path": "SomeWeirdType", "id": 0, "args": null } })
                    ]
                ),
                BoundPos::Param
            ),
        ...
        // [#78 WALL-E] Into<&'static str> / From<&str> drop — a runtime owned Ipe
        ...
        assert_eq!(
            bound_to_concrete(
                &trait_bound("Into", vec![static_str.clone()]),
                BoundPos::Param
            ),
            None,
        ...
        assert_eq!(
            bound_to_concrete(&trait_bound("From", vec![elided_str]), BoundPos::Param),
            None,
        ...
        assert_eq!(
            bound_to_concrete(&trait_bound("AsRef", vec![static_str]), BoundPos::Param),
            Some(string_node()),
    ...
    fn test_resolve_param_bounds() {
        ...
        assert_eq!(
            resolve_param_bounds(std::slice::from_ref(&asref_u8), BoundPos::Param),
            Some(vec_u8_node())
        );
        // shape + marker bounds -> markers ignored, resolves
        assert_eq!(
            resolve_param_bounds(&[asref_u8.clone(), clone.clone(), send], BoundPos::Param),
            Some(vec_u8_node())
        );
        ...
        assert_eq!(
            resolve_param_bounds(&[asref_u8.clone(), foo], BoundPos::Param),
            None
        );
        // two conflicting shape bounds -> None
        assert_eq!(
            resolve_param_bounds(&[asref_u8, asref_str], BoundPos::Param),
            None
        );
    }
    ...
    fn test_resolve_generics() {
        ...
        assert_eq!(
            resolve_generics(&fn_with_generics(vec![], vec![])),
            Some(std::collections::HashMap::new())
        );

        // <T: AsRef<[u8]>> -> {T: Vec<u8>}
        let r = resolve_generics(&fn_with_generics(
            vec![type_param("T", vec![asref_u8.clone()])],
            vec![],
        ))
        .unwrap();
        ...
        let r2 = resolve_generics(&fn_with_generics(
            vec![
                type_param("T", vec![asref_u8.clone()]),
                type_param("U", vec![display]),
            ],
            vec![],
        ))
        .unwrap();
        ...
        assert_eq!(
            resolve_generics(&fn_with_generics(
                vec![type_param("T", vec![trait_bound("FromStr", vec![])])],
                vec![]
            )),
            None
        );
        ...
        assert_eq!(
            resolve_generics(&fn_with_generics(vec![const_p], vec![])),
            None
        );
        ...
        let r3 =
            resolve_generics(&fn_with_generics(vec![type_param("T", vec![])], vec![wp])).unwrap();
    ...
    fn test_subst_generic_json() {
        ...
        // Nested multi-word generic arg is parenthesised so Ipe's
        // left-associative type application parses it as one argument.
        assert_eq!(ipe(&got), "Maybe (List Int)");
    ...
    fn test_parse_fn_item_monomorphises() {
        ...
        assert_eq!(f.params[0].ipe_type, "List Int"); // T -> Vec<u8> -> List Int
        assert_eq!(f.params[0].rust_type, "Vec<u8>");
        assert_eq!(f.results[0].ipe_type, "String"); // -> str -> String
    }
    ...
    fn test_parse_fn_item_nongeneric_unchanged() {
        ...
        assert_eq!(f.params[0].ipe_type, "String");
        assert_eq!(f.results[0].ipe_type, "Int");
    }
    ...
    fn test_impl_trait_arg() {
        ...
        assert_eq!(f.params[0].ipe_type, "List Int");
        assert_eq!(f.params[0].rust_type, "Vec<u8>");
    ...
    fn test_return_position_concrete_vec_still_binds() {
        ...
        assert_eq!(f.results[0].ipe_type, "List Int");
    }
    ...
    fn test_bound_to_concrete_bare_iterator_param_drops() {
        ...
        assert_eq!(
            bound_to_concrete(&it, BoundPos::Param),
            None,
        ...
        assert_eq!(
            ipe(&bound_to_concrete(&into, BoundPos::Param).unwrap()),
            "List Int"
        );
    }
    ...
    fn test_bound_to_concrete_v2() {
        ...
        let asref_path = trait_bound(
            "AsRef",
            vec![
                serde_json::json!({ "resolved_path": { "name": "Path", "path": "Path", "id": 0, "args": null } }),
            ],
        );
        assert_eq!(
            bound_to_concrete(&asref_path, BoundPos::Param),
            Some(string_node())
        );
        let asref_osstr = trait_bound(
            "AsRef",
            vec![
                serde_json::json!({ "resolved_path": { "name": "OsStr", "path": "OsStr", "id": 0, "args": null } }),
            ],
        );
        assert_eq!(
            bound_to_concrete(&asref_osstr, BoundPos::Param),
            Some(string_node())
        );
        let into_pb = trait_bound(
            "Into",
            vec![
                serde_json::json!({ "resolved_path": { "name": "PathBuf", "path": "PathBuf", "id": 0, "args": null } }),
            ],
        );
        assert_eq!(
            bound_to_concrete(&into_pb, BoundPos::Param),
            Some(string_node())
        );
        ...
        assert_eq!(
            ipe(&bound_to_concrete(
                &trait_bound("Into", vec![primitive("i64")]),
                BoundPos::Param
            )
            .unwrap()),
            "Int"
        ...
        assert_eq!(
            ipe(&bound_to_concrete(
                &trait_bound("Into", vec![primitive("f64")]),
                BoundPos::Param
            )
            .unwrap()),
            "Float"
        ...
        for name in ["i32", "u32", "u64", "usize", "isize", "f32"] {
            ...
            assert_eq!(
                bound_to_concrete(&b, BoundPos::Param),
                None,
                "Into<{name}> must drop (i64/f64 unsound substitution)"
            );
        }
        ...
        assert_eq!(
            ipe(&bound_to_concrete(&int_b, BoundPos::Param).unwrap()),
            "Int"
        );
        ...
        assert_eq!(
            ipe(&bound_to_concrete(&float_b, BoundPos::Param).unwrap()),
            "Float"
        );
        ...
        let got =
            bound_to_concrete(&into_iter_str, BoundPos::Param).expect("IntoIterator<Item=&str>");
        assert_eq!(ipe(&got), "List String");
    ...
    fn test_concrete_for_inner_type() {
        ...
        assert_eq!(
            concrete_for_inner_type(&primitive("str")),
            Some(string_node())
        );
        ...
        for name in [
            ...
            assert_eq!(ipe(&r), "Int", "bad ipe for {name}");
        }
        for name in ["f32", "f64"] {
            let r = concrete_for_inner_type(&primitive(name))
                .unwrap_or_else(|| panic!("missing: {name}"));
            assert_eq!(ipe(&r), "Float", "bad ipe for {name}");
        }
    ...
    fn test_parse_fn_item_v2_slice_array() {
        ...
        assert_eq!(f.params[0].ipe_type, "List String");
        ...
        assert_eq!(f2.params[0].ipe_type, "List Float");
        ...
        assert_eq!(f4.params[0].ipe_type, "List Int");
        assert_eq!(f4.params[0].rust_type, "&[u8]");
        assert_eq!(f4.results[0].ipe_type, "List Int");
    }
    ...
    fn test_modellable_5_matches() {
        ...
        // {Hash,Eq,Ord,Clone,Default}. The generator side asserts the same set in
    ...
    fn with_doc_provenance<T>(doc: &serde_json::Value, f: impl FnOnce() -> T) -> T {
        ...
        EXTERNAL_TYPE_PATH_BY_ID.with(|c| *c.borrow_mut() = collect_external_type_paths(doc));
        EXTERNAL_DEFPATH_BY_ID.with(|c| *c.borrow_mut() = collect_external_defpaths(doc));
        let (reach_paths, reach_fn_paths) = collect_reachable_paths(doc);
        REACHABLE_PATHS.with(|c| *c.borrow_mut() = reach_paths);
        REACHABLE_FN_PATHS.with(|c| *c.borrow_mut() = reach_fn_paths);
        let out = f();
    ...
    fn wall6_private_module_named_reexport_resolves_public_path() {
        ...
        assert_eq!(
            reach.get("48").map(std::string::String::as_str),
            Some("fluentcrate::Db"),
            "Db should resolve to its root path"
        );
        assert_eq!(
            reach.get("2").map(std::string::String::as_str),
            Some("fluentcrate::FluentSelect"),
            "a private-module type re-exported by a NAMED `pub use` at root must resolve \
             to the public re-export path (not drop); got {:?}",
            reach.get("2")
        );
    }
    ...
    #[test]
    fn wall90_ipe_of_typeref_container_heads_use_ipe_names() {
        // [#90] The projected/UFCS renderer must agree with the inherent path
        // (`resolve_path_to_ipe`): Vec→List, Option→Maybe, HashMap-family→
        // `Dict String <v>` (key arg dropped). Pre-#90 it emitted raw `Vec t` /
        // `Option t` / `HashMap k v` — undefined Ipe types that broke cross-path
        // HM unification.
        let tv = vec!["t".to_string()];
        assert_eq!(
            ipe_of_typeref(
                &TypeRef::Ctor("Vec".into(), vec![TypeRef::Param(0)]),
                &tv,
                ""
            ),
            "List t"
        ...
        assert_eq!(
            ipe_of_typeref(
                &TypeRef::Ctor("Option".into(), vec![TypeRef::Prim("i64".into())]),
                &[],
                ""
            ),
            "Maybe Int"
    ...
    fn test_classify_param_bound_crate_local_clone_drops() {
        ...
        with_doc_provenance(&doc, || {
            ...
            match classify_param_bound(&local_clone) {
                ...
                other => {
                    panic!("crate-local `Clone` must drop as UnmodellableBound, got {other:?}")
                }
            }
        });
    }
    ...
    fn test_classify_param_bound_std_clone_admitted() {
        ...
        with_doc_provenance(&doc, || {
            ...
            assert_eq!(
                classify_param_bound(&std_clone),
                Ok(Some("Clone".to_string()))
            );
        });
    }
    ...
    fn test_is_marker_bound_crate_local_not_marker() {
        ...
        with_doc_provenance(&doc, || {
            let local_clone = trait_bound_id("Clone", 10);
            assert!(
                !is_marker_bound(&local_clone),
                "crate-local Clone is not a marker"
            );
        });
    ...
    fn test_bound_to_concrete_crate_local_display_drops() {
        ...
        with_doc_provenance(&doc, || {
            ...
            assert_eq!(
                bound_to_concrete(&std_d, BoundPos::Param),
                Some(string_node())
            );
        });
    }
    ...
    fn test_c1_full_union_from_impl_and_struct() {
        ...
        let impl_g = generics_obj(
            vec![
                type_param(
                    "K",
                    vec![trait_bound("Hash", vec![]), trait_bound("Eq", vec![])],
                ),
                type_param("V", vec![]),
            ],
            vec![],
        ...
        let struct_g = generics_obj(
            vec![type_param("K", vec![]), type_param("V", vec![])],
            vec![],
        );
        let used: HashSet<String> = ["K", "V"]
            .iter()
            .map(std::string::ToString::to_string)
            .collect();
        let union =
            full_union_bounds(&method_g, Some(&impl_g), Some(&struct_g), None, &used).unwrap();
        assert_eq!(
            union.get("K"),
            Some(&vec!["Eq".to_string(), "Hash".to_string()])
        );
    ...
    fn test_c1_union_via_where_predicate() {
        ...
        let used: HashSet<String> = ["K"].iter().map(std::string::ToString::to_string).collect();
    ...
    fn test_c3_unmodellable_bound_drops() {
        ...
        let method_g = generics_obj(
            vec![type_param("T", vec![trait_bound("Serialize", vec![])])],
            vec![],
        );
        let used: HashSet<String> = ["T"].iter().map(std::string::ToString::to_string).collect();
    ...
    fn test_c3_modellable_super_closure_ok() {
        ...
        let method_g = generics_obj(
            vec![type_param("T", vec![trait_bound("Ord", vec![])])],
            vec![],
        );
        let used: HashSet<String> = ["T"].iter().map(std::string::ToString::to_string).collect();
    ...
    fn test_c4_defaulted_bound_param_drops_when_used() {
        ...
        let used_with_s: HashSet<String> = ["K", "S"]
            .iter()
            .map(std::string::ToString::to_string)
            .collect();
        ...
        let used_no_s: HashSet<String> =
            ["K"].iter().map(std::string::ToString::to_string).collect();
    ...
    fn test_try_parametric_stub_indexmap_shape() {
        ...
        REACHABLE_PATHS.with(|c| {
            c.borrow_mut()
                .insert("7".to_string(), "indexmap::IndexMap".to_string());
        });
        ...
        assert_eq!(
            stub.call.path,
            vec!["::indexmap".to_string(), "IndexMap".to_string()]
        );
    ...
    fn test_parametric_stub_inherent_closure_emits_closure_argtype() {
        ...
        // F is consumed: the Ipe tyvars are exactly [A, B], in declaration order.
        assert_eq!(stub.params, vec!["A".to_string(), "B".to_string()]);
        // No `<F: Fn>` Ipe bound leaks (the Fn bound went into the closure shape).
        assert!(
            stub.bounds.is_empty(),
            "no Fn bound should survive as a Ipe bound"
        );
        ...
        assert_eq!(
            stub.call.ret,
            TypeRef::Ctor("::Vec".to_string(), vec![TypeRef::Param(1)])
        );
    }
    ...
    fn test_parametric_stub_closure_with_send_companion_drops() {
        ...
        // Ipe closure can't honour `Send`, so binding it would emit a wrapper whose
    ...
    fn test_parametric_stub_closure_with_lifetime_companion_binds() {
        ...
        // owned-capture Ipe closure → still binds. Confirms the gate admits
        ...
        assert!(
            stub.call
                .arg_types
                .iter()
                .any(|t| matches!(t, TypeRef::Closure { .. }))
        );
    }
    ...
    fn classify_iterator_param_emits_vec_item_and_kind() {
        ...
        let (kind, item) =
            classify_iterator_param(IterKind::IntoIter, &bound, &HashMap::new(), true)
    ...
    fn parametric_stub_into_iterator_param_emits_vec_direct() {
        ...
        assert!(stub.params.is_empty(), "I is consumed → no Ipe tyvar");
        assert!(
            stub.bounds.is_empty(),
            "no iterator bound survives as a Ipe bound"
        );
        assert_eq!(
            stub.call.arg_types,
            vec![TypeRef::Ctor(
                "::Vec".into(),
                vec![TypeRef::Prim("i64".into())]
            )]
        );
        assert!(
            stub.call.iter_adapters.is_empty(),
            "IntoIterator is direct (no .into_iter())"
        );
    ...
    fn parametric_stub_iterator_param_records_into_iter_adapter() {
        ...
        let stub =
            try_parametric_stub("count", &fd, Some(("Foo", "::iter::Foo")), None, None, None)
        ...
        assert_eq!(
            stub.call.iter_adapters,
            vec![0],
            "Iterator arg needs .into_iter()"
        );
    }
    ...
    fn parametric_stub_shared_iterator_tyvar_routes_both_slots() {
        ...
        assert!(
            ...
            "the shared I is consumed in BOTH slots"
        );
    ...
    fn test_generic_serde_shape() {
        // The emitted `generic` JSON must match the wire keys the generator
        // FromJSON reads (drift 4b — Rust emission side).
        let g = Generic {
            params: vec!["K".into(), "V".into()],
            bounds: [("K".to_string(), vec!["Hash".to_string()])]
                .into_iter()
                .collect(),
            call: Call {
                kind: "function".into(),
        ...
        assert_eq!(
            v["call"]["ret"],
            serde_json::json!({ "ctor": "::c::T", "args": [{ "param": 0 }] })
        );
    ...
    fn test_method_turbofish_wire_shape() {
        ...
        // `methodTurbofish` list the generator renderer reads — a concrete PER
    ...
    fn ab_idx() -> HashMap<String, usize> {
        [("a".to_string(), 0usize), ("b".to_string(), 1usize)]
            .into_iter()
            .collect()
    }
    ...
    fn closure_matches_hand_stub_key_order() {
        ...
        // single `closure` key — the shape the generator `FromJSON TypeRef` reads
    ...
    fn closure_drop_tags_are_distinct_and_stable() {
        ...
        // generator `closureDropReason` taxonomy + the two inspector-only tags).
        let tags = [
    ...
    fn test_self_is_concrete_named() {
        ...
        assert!(!self_is_concrete_named(&path_with_args(
            "Pair",
            vec![prim("i64")]
        )));
        ...
        assert!(!self_is_concrete_named(&path_with_args(
            "Pair",
            vec![path_with_args("Vec", vec![prim("i64")])]
        )));
    ...
    fn trait_ctx_with(trait_def: Option<&serde_json::Value>) -> TraitCtx<'_> {
        TraitCtx {
    ...
    fn trait_ctx_assoc<'a>(binds: &[(&str, serde_json::Value)]) -> TraitCtx<'a> {
        TraitCtx {
            trait_def_generics: None,
            qualifier: ("::tm::Circle".to_string(), "::tm::Pair".to_string()),
            assoc_bindings: binds
                .iter()
    ...
    fn test_q2a_bare_restated_bound_pre_drops_post_binds() {
        ...
        let pre = try_parametric_stub(
            "keyed",
            &fd,
            Some(("Circle", "::tm::Circle")),
            None,
            None,
            Some(&trait_ctx_with(None)),
        );
        match pre {
        ...
        //    a modellable-5 trait, so `T` stays a Ipe tyvar and the stub EMITS
        ...
        let stub = post.expect("POST (trait-def union): `T: Ord` resolves → `keyed` must BIND");
        // T survives as a Ipe tyvar carrying the unioned `Ord` bound.
        ...
        assert_eq!(
            stub.call.receiver.as_ref().map(|r| r.by.as_str()),
            Some("ref")
        );
    }
    ...
    fn test_impl_assoc_bindings_extraction() {
        ...
        assert_eq!(
            impl_assoc_bindings(&impl2, &index2).get("B"),
            Some(&prim("bool"))
        );
    }
    ...
    fn test_display_bridge_one_to_string_zero_fmt() {
        ...
        let to_strings: Vec<&Function> = functions
            .iter()
            .filter(|f| f.method_name == "to_string")
            .collect();
        assert_eq!(to_strings.len(), 1, "exactly one to_string bridge");
        assert!(
            functions.iter().all(|f| f.method_name != "fmt"),
    ...
    fn test_route_concrete_method_pubcrate_trait_drops_trait_unreachable() {
        ...
        REACHABLE_PATHS.with(|c| {
            c.borrow_mut()
                .insert("42".to_string(), "tm::Scale".to_string());
        });
        ...
        match ok {
            BoundSource::TraitImpl(ctx) => {
                assert_eq!(
                    ctx.qualifier,
                    ("::tm::Circle".to_string(), "::tm::Scale".to_string())
                );
            }
        ...
        assert!(
            inh.ctx().is_none(),
            "an inherent BoundSource carries no qualifier"
        );
    ...
    fn test_external_from_trait_binds_with_canonical_ufcs() {
        ...
        with_doc_provenance(&doc, || {
            ...
            let ctx = build_trait_ctx(Some(&trait_node), "from", Some(&for_val), &impl_data, index)
                .expect(
                    "external std `From` MUST resolve its canonical UFCS path \
    ...
    fn test_external_trait_display_drops_on_unbindable_arg_not_unreachable() {
        ...
        with_doc_provenance(&doc, || {
            ...
            let bs =
                route_concrete_method(Some(&trait_node), "fmt", Some(&for_val), &impl_data, index)
            ...
            match dropped {
                Err(GenericDrop::TraitMethodDrop(TraitMethodDropTag::TraitUnreachable, _)) => {
                    panic!(
                        "Display.fmt must NOT drop trait-unreachable post-#46 — \
    ...
    fn test_unconfirmable_external_trait_still_drops_unreachable() {
        ...
        with_doc_provenance(&doc, || {
            ...
            match route_concrete_method(Some(&trait_node), "go", Some(&for_val), &impl_data, index)
            {
                Err(GenericDrop::TraitMethodDrop(TraitMethodDropTag::TraitUnreachable, _)) => {}
    ...
    fn test_generic_method_on_nongeneric_struct_recv_has_no_method_tyvar() {
        ...
        let stub = try_parametric_stub(
            ...
            Some(("Circle", "::tm::Circle")),
            None,
            /* impl_generics */ None, /* struct_generics: NON-generic */
            Some(&trait_ctx_with(Some(&trait_def["generics"]))),
        )
        ...
        // `T` still appears as a Ipe tyvar (in the value args), just not the recv.
        assert_eq!(stub.params, vec!["T".to_string()]);
    }
    ...
    fn test_trait_method_parametric_fn_leaves_recv_empty() {
        ...
        // `method_name` EMPTY so the Ipe compiler routes it ONLY through the
        ...
        assert!(
            f.generic
                .as_ref()
                .is_some_and(|g| g.call.trait_qualifier.is_some())
        );
        // … and leaves recv_type / method_name EMPTY (no inherent duplicate).
        assert_eq!(
            f.recv_type, "",
            "a trait method must not set recv_type (inherent dup)"
        );
        assert_eq!(
            f.method_name, "",
            "a trait method must not set method_name (inherent dup)"
        );
    }
    ...
    fn test_inherent_generic_method_keeps_recv() {
        ...
        let stub =
            try_parametric_stub("keyed", &fd, Some(("Box1", "::cr::Box1")), None, None, None)
    ...
    fn circle_area_doc(
        method_vis: &str,
        sig_inputs: serde_json::Value,
        sig_output: serde_json::Value,
    ) -> serde_json::Value {
        let area_method = serde_json::json!({
    ...
    fn test_default_visibility_trait_method_binds() {
        ...
        let area = pkg.functions.iter().find(|f| f.name == "area").expect(
            "a `default`-visibility trait method on a public trait+type \
        ...
        let g = area
            .generic
            .as_ref()
            .expect("trait method binds as a parametric stub");
        assert_eq!(
    ...
    fn test_trait_method_pubcrate_self_drops() {
        ...
        doc["index"]["1"]["inner"]["module"]["items"] = serde_json::json!([42, 50, 60]);
    ...
    fn test_blanket_impl_trait_method_drops() {
        ...
        doc["index"]["50"]["inner"]["impl"]["for"] = serde_json::json!({ "generic": "T" });
        ...
        assert!(
            dropped,
            "blanket impl must record trait-method-generic-self"
        );
    }
    ...
    fn test_nongeneric_trait_method_on_closed_monomorphic_self_binds() {
        ...
        for recv in [
            ...
            let area_fn = pkg
                .functions
                .iter()
                .find(|f| f.name == "area_from_holder")
                .unwrap();
    ...
    fn dyn_fn_param_still_routes_to_closure_seam() {
        ...
        // `dyn Fn` is a satisfiable Ipe fn-value; the new trait-object drop must NOT
        ...
        // Fn/FnMut/FnOnce so the closure seam keeps it. We assert the rendered/ipe
    ...
    fn trait_object_drop_coverage_tag() {
        ...
        let recorded = GENERIC_DROPS.with(|v| {
            v.borrow()
                .iter()
                .any(|(sym, d)| sym == "takes_box" && d.reason() == "trait-object-unsupported")
        });
        assert!(
            recorded,
            "dyn-object drop must record the trait-object-unsupported tag"
        );
    }
    ...
    fn dyn_fn_object_drop_records_tag() {
        ...
        let recorded = GENERIC_DROPS.with(|v| {
            v.borrow()
                .iter()
                .any(|(sym, d)| sym == "takes_cb" && d.reason() == "trait-object-unsupported")
        });
        assert!(
            recorded,
            "dyn-Fn-object drop must record the trait-object-unsupported tag"
        );
    }
    ...
    fn async_send_gate_unwrap_result_outer() {
        ...
        assert_eq!(
            unwrap_result_outer_test("Result<Vec<i64>, String>"),
            "Vec<i64>"
        );
        ...
        assert_eq!(
            unwrap_result_outer_test("asyncffi72::NonSendAux"),
    ...
    fn async_send_gate_drops_clone_but_not_send_output() {
        ...
        // (`is_ipe_coercible_elem` via `is_clone_opaque_name`) because sync FFI
        ...
        // property: `is_ipe_coercible_elem` vs `is_async_send_output` for a plain
        ...
        // is_ipe_coercible_elem calls is_clone_opaque_name (empty here → false, but
    ...
    fn async_send_gate_covers_receiver() {
        ...
        let async_method = |out: &serde_json::Value| {
            serde_json::json!({
                "header": { "is_async": true, "is_unsafe": false },
                "generics": { "params": [], "where_predicates": [] },
        ...
        assert!(
            parse_fn_item(
                "m",
                &async_method(&fallible_i64),
                &HashMap::new(),
                Some(("DbHandle", "DbHandle"))
            )
            .is_none(),
        ...
        PROVABLY_SEND_RECV_NAMES.with(|c| {
            c.borrow_mut().insert("SendDb".to_string());
        });
        assert!(
            parse_fn_item(
                "m",
    ...
    fn fn_data_with_generic_bounds(
        ...
        let input_pairs: Vec<serde_json::Value> = inputs
            .into_iter()
            .enumerate()
    ...
    fn serde_serialize_bound_detected() {
        ...
        assert!(
            is_serde_trait_bound(&b),
            "serde::Serialize must be detected as a serde bound"
        );
    }
    ...
    fn serde_deserialize_owned_bound_detected() {
        ...
        assert!(
            is_serde_trait_bound(&b),
            "serde::de::DeserializeOwned must be detected"
        );
    }
    ...
    fn own_serialize_look_alike_not_detected() {
        ...
        assert!(
            !is_serde_trait_bound(&b),
            "crate-local Serialize must NOT be detected as serde"
        );
    }
    ...
    fn serialize_plus_send_is_all_serde_or_marker() {
        let bounds = vec![serde_bound("serde::Serialize"), marker_bound("Send")];
    ...
    fn empty_bounds_not_serde() {
        assert!(
            !bounds_are_all_serde_or_marker(&[]),
            "no bounds → not serde (needs at least one)"
        );
    }
    ...
    fn by_value_param_admissible() {
        ...
        assert!(serde_occurrence_admissible_in(
            &generic_node("T"),
            "T",
            OccurrenceCtx::Admissible,
            false
    ...
    fn serde_bound_mixed_admissible_and_borrow_vetoes_whole_fn() {
        ...
        // `&Value` for `b` (type-checks in `ipe`, cargo-fails E0277/E0308 — the
    ...
    fn serde_bound_result_vec_t_in_ok_and_err_admissible() {
        ...
        let result_ty = path_with_args(
            "Result",
            vec![
                path_with_args("Vec", vec![generic_node("T")]),
                generic_node("T"),
            ],
        );
        let fd = fn_data_with_generic_bounds(
    ...
    fn serialize_bound_is_serialize_only() {
        ...
        assert!(!is_serialize_bound(&serde_bound(
            "serde::de::DeserializeOwned"
        )));
    ...
    fn bounds_serialize_only_classification() {
        ...
        assert!(!bounds_are_serialize_only_or_marker(&[serde_bound(
            "serde::de::DeserializeOwned"
        ),]));
        assert!(!bounds_are_serialize_only_or_marker(&[
            serde_bound("serde::Serialize"),
        ...
        assert!(
            !bounds_are_serialize_only_or_marker(&[]),
    ...
    fn serialize_input_ref_admissible_when_flag_set() {
        ...
        assert!(
            serde_occurrence_admissible_in(&by_ref, "T", OccurrenceCtx::Admissible, true),
            "&T serialize INPUT must be admissible with the input-ref flag"
        );
        assert!(
            !serde_occurrence_admissible_in(&by_ref, "T", OccurrenceCtx::Admissible, false),
    ...
    fn mut_serialize_input_ref_stays_inadmissible() {
        ...
        // hand a Ipe String as a mutable borrow of a to-be-serialised value.
        let by_mut = mut_borrowed_generic("T");
        assert!(
            !serde_occurrence_admissible_in(&by_mut, "T", OccurrenceCtx::Admissible, true),
    ...
    fn serialize_ref_inside_container_not_admitted() {
        ...
        assert!(
            !serde_occurrence_admissible_in(&vec_of_ref, "T", OccurrenceCtx::Admissible, true),
    ...
    fn fn_ext_admits_serialize_input_ref_but_not_output_ref() {
        ...
        assert!(
            fn_serde_param_all_admissible_ext(&in_ref, "T", true),
            "&T INPUT must pass the ext census with the flag"
        );
        assert!(
            !fn_serde_param_all_admissible(&in_ref, "T"),
        ...
        assert!(
            !fn_serde_param_all_admissible_ext(&out_ref, "T", true),
    ...
    fn fn_ext_mixed_owned_and_ref_input_admissible() {
        ...
        assert!(
            fn_serde_param_all_admissible_ext(&fd, "T", true),
            "owned + &T serialize inputs must both be admissible with the flag"
        );
        assert!(
            !fn_serde_param_all_admissible(&fd, "T"),
    ...
    fn serde_ref_borrowed_node_becomes_serde_value_ref() {
        // `type_to_typeref(&{__ipe_serde_value})` (non-mut) → SerdeValueRef.
        // `&mut {__ipe_serde_value}` → NotBindable (drops). This is the codegen
    ...
    // ── rustdoc_type_to_ipe / rustdoc_type_to_rust_str sentinel ──────────────
    ...
    fn serde_core_serialize_path_detected_via_id() {
        ...
        assert!(
            result,
            "serde_core::ser::Serialize via EXTERNAL_TRAIT_PATH_BY_ID must be detected"
        );
    }
    ...
    fn serde_core_deserialize_owned_path_detected_via_id() {
        ...
        assert!(
            result,
            "serde_core::de::DeserializeOwned via EXTERNAL_TRAIT_PATH_BY_ID must be detected"
        );
    }
    ...
    #[test]
    fn serde_value_node_to_ipe_is_string() {
        assert_eq!(
            rustdoc_type_to_ipe(&serde_json_value_node(), &HashMap::new()),
            "String"
        );
    }
    ...
    fn test_wall_b_remap_alloc_to_std() {
        assert_eq!(
            remap_alloc_to_std("alloc::string::ToString"),
            "std::string::ToString"
        );
        assert_eq!(
            remap_alloc_to_std("alloc::borrow::ToOwned"),
            "std::borrow::ToOwned"
        );
        // Non-alloc paths are untouched.
        assert_eq!(
            remap_alloc_to_std("core::fmt::Display"),
            "core::fmt::Display"
        );
        assert_eq!(
            remap_alloc_to_std("std::string::ToString"),
            "std::string::ToString"
        );
        ...
        assert_eq!(
            canonical, "alloc::string::ToString",
            "table still lists the alloc:: form first (the bug's source)"
        );
        assert_eq!(
            remap_alloc_to_std(canonical),
            "std::string::ToString",
    ...
    fn test_facade_guidance_names_subcrate() {
        ...
        assert_eq!(
            notes.len(),
            1,
            "facade with 0 fns must produce exactly one note"
        );
        let note = &notes[0];
        assert!(
            note.contains("other_crate"),
            "note must name the sub-crate: {note}"
        );
        assert!(
            note.starts_with("note: 'facade_crate' re-exports its API from:"),
            "note must use the expected prefix format: {note}"
        );
        assert!(
            note.contains("ipe add other_crate"),
            "note must suggest the ipe add command: {note}"
        );
    }
    ...
    fn test_facade_guidance_supplementary_for_reexporting_crate() {
        ...
        let note = &notes[0];
        assert!(
            note.contains("also re-exports items from:"),
            "≥-threshold fn_count must use the supplementary wording: {note}"
        );
        assert!(
            note.contains("other_crate") && note.contains("ipe add other_crate"),
    ...
    #[test]
    fn wall5_result_alias_expands_rust_and_ipe() {
        ...
        // DbResult<Db>  →  Result<Db, DbError> (rust)  /  Result DbError Db (ipe)
        let use_site = alias_use(vec![path("Db")]);
        assert_eq!(rustdoc_type_to_rust_str(&use_site), "Result<Db, DbError>");
        assert_eq!(ipe(&use_site), "Result DbError Db");
    }
    ...
    fn wall5_result_alias_inner_box_dyn_survives_for_downstream_drop() {
        ...
        assert!(
            rust.starts_with("Result<"),
            "outer Result must expand: {rust}"
        );
        // The inner stays a Box (the downstream gate, not the see-through, drops it).
        assert!(
            rust.contains("Box"),
            "inner Box must survive expansion: {rust}"
        );
    }
    ...
    fn wall5_cyclic_alias_terminates_fail_closed() {
        ...
        assert!(
            rust.starts_with("Result<"),
            "outer Result must expand once: {rust}"
        );
        assert!(
            rust.contains("DbResult"),
        ...
        assert!(
            rust.len() < 4096,
            "rendered string must be bounded: len={}",
            rust.len()
        );
        assert!(
            !s.is_empty(),
            "ipe rendering must also terminate non-empty: {s}"
        );
    ...
    fn wall5_option_alias_expands() {
        ...
        let use_site = alias_use(vec![path("Db")]);
        assert_eq!(ipe(&use_site), "Maybe Db");
    ...
    fn wall5_non_result_generic_alias_not_expanded() {
        ...
        assert!(
            map.is_empty(),
            "non-Result generic alias must NOT be recorded"
        );
        GENERIC_ALIAS_MAP.with(|c| *c.borrow_mut() = map);
    ...
    fn wall5_crate_local_result_homonym_not_expanded() {
        ...
        assert!(
            map.is_empty(),
            "crate-local Result homonym must NOT be recorded"
        );
    }
    ...
    fn wall5_nested_generic_substitutes_totally() {
        ...
        assert!(
            !rust.contains('T') || rust.contains("DbError"),
    ...
    fn wall5_self_inner_preserved_for_subst_self() {
        ...
        assert_eq!(
            subst_self(&rustdoc_type_to_rust_str(&use_site), "Db"),
            "Result<Db, DbError>"
        );
    }
    ...
    fn wall4_positive_recognises_and_extracts_output_from_constraints() {
        ...
        // and NOT the `equality` wrapper. Render to the Ipe type and assert it is
        // `Result String i64` (so de-async → `Task String i64`, never `Task Error ()`).
        let ipe = rustdoc_type_to_ipe(out, &HashMap::new());
        assert_eq!(
            ipe, "Result String Int",
            "Output must be the real T, not (): {ipe}"
        );
    }
    ...
    fn wall4_de_async_clone_forces_is_async_and_rewrites_output_without_mutation() {
        ...
        assert_eq!(
            rustdoc_type_to_ipe(&clone["sig"]["output"], &HashMap::new()),
            "Result String Int"
        );
        ...
        assert_eq!(
            fn_data["sig"]["output"]["resolved_path"]["path"],
            "::core::pin::Pin"
        );
    }
    ...
    fn wall4_parse_fn_item_binds_async_trait_method_as_effectful_task() {
        ...
        // EFFECTFUL fn whose result Ipe type is `Task String i64`, with NO drop.
        let boxed = async_trait_box(
            result_i64_string(),
            "::core::future::Future",
            4,
        ...
        PROVABLY_SEND_RECV_NAMES.with(|c| {
            c.borrow_mut().insert("Db".to_string());
        });
        ...
        // The de-async'd output is the REAL `Result<i64,String>` (rendered Ipe
        ...
        // `Task Error ()`).
        assert_eq!(
            ret.ipe_type, "Result String Int",
            "Output must be the real Result<i64,String>, not (): {:?}",
            ret.ipe_type
        );
        assert_ne!(ret.ipe_type, "()", "C4: must not silently collapse to unit");
    }
    ...
    fn wall4_parse_fn_item_async_trait_with_non_send_receiver_drops() {
        ...
        assert!(
            f.is_none(),
            "a non-Send receiver async_trait method must drop"
        );
    }
    ...
    fn wall4_negative_a_non_future_dyn_object_stays_dropped() {
        ...
        assert!(
            async_trait_future_output(&boxed).is_none(),
    ...
    fn wall4_negative_b_stream_and_iterator_stay_dropped() {
        ...
        assert!(
            async_trait_future_output(&stream).is_none(),
        ...
        assert!(
            async_trait_future_output(&iter_box).is_none(),
    ...
    fn wall4_canonical_future_gate_resolves_by_id_in_real_doc() {
        ...
        with_doc_provenance(&doc, || {
            assert!(
                async_trait_future_output(&boxed).is_some(),
    ...
    fn wallf_rpitit_positive_recognises_and_extracts_output() {
        ...
        assert_eq!(
            rustdoc_type_to_ipe(out, &HashMap::new()),
            "Result String Int",
    ...
    fn wallf_rpitit_negative_no_send_drops() {
        ...
        assert!(
            impl_future_output(&imp).is_none(),
            "an RPITIT Future without +Send must NOT bind (E0277-prone)"
        );
        assert!(
            de_async_clone(&desugared_fn(imp)).is_none(),
    ...
    fn visibility_features_prefers_full_with_decode() {
        ...
        let avail = vec![
            ...
            "serialize".to_string(),
            "full".to_string(),
            "redact-generated-debug".to_string(),
        ];
        let got = choose_visibility_features(&avail);
        assert_eq!(
            got,
            vec!["full", "deserialize", "serialize"],
    ...
    fn visibility_features_full_without_decode_features() {
        ...
        assert_eq!(
            choose_visibility_features(&avail),
            vec!["full"],
    ...
    fn wallj_resolves_self_output_via_sibling_impl() {
        ...
        assert!(
            resolved.get("qualified_path").is_none(),
            "the projection must be gone — got: {resolved}"
        );
        assert_eq!(
            resolved["resolved_path"]["id"].as_u64(),
            Some(7),
