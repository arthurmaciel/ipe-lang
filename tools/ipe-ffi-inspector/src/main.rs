...
fn parse_fn_item(
    ...
    for input in inputs {
        ...
        let (ripe, rrust) = recv.unwrap_or(("", ""));
        let ipe = subst_self(&ipe, ripe);
    ...
    if effect == "effectful" {
        ...
        let non_send_param = params
            ...
            .skip(
                if recv.is_some() && params.first().map(|p| p.name == "self").unwrap_or(false) {
                    1
                } else {
                    0
                },
        ...
        let is_instance_method =
            recv.is_some() && params.first().map(|p| p.name == "self").unwrap_or(false);
        ...
        if let Some(u) = it["inner"].get("use") {
            ...
            if u["is_glob"].as_bool().unwrap_or(false) {
                ...
                if index.get(tids).map(item_is_type).unwrap_or(false) {
                    ...
                    .get(tids)
                    .map(|t| t["inner"].get("function").is_some() && is_public(t))
                    .unwrap_or(false)
                {
...
fn inner_type(wrapper: &serde_json::Value) -> &serde_json::Value {
    let t = &wrapper["type"];
    if !t.is_null() { t } else { &wrapper["type_"] }
}
...
fn self_is_closed_monomorphic(for_val: &serde_json::Value) -> bool {
    ...
    for arg in ab_args {
        if let Some(t) = arg.get("type") {
            ...
            if rendered.len() == 1 && rendered.chars().next().is_some_and(|c| c.is_uppercase()) {
                return false;
            }
        }
        // Lifetime args: skip.
...
fn monomorphize_concrete_impl_params(
    ...
    for (param_name, trait_key) in &matched_send_keys {
        ...
        if let Some(xc) = xc_unique_for_trait_key(trait_key) {
            ...
            let same_crate_unique = TRAIT_CONCRETE_IMPLS.with(|c| {
                c.borrow()
                    .get(trait_key)
                    .map(|v| v.len() == 1)
                    .unwrap_or(false)
        ...
        for id in items {
            ...
            let has_bad_param = projected
                ...
                .map(|ins| {
                    ins.iter().any(|inp| {
                        inp.get(1)
                            .map(type_has_uncoercible_numeric)
                            .unwrap_or(false)
                    })
                })
                .unwrap_or(false);
...
mod tests {
    ...
    fn test_bound_to_concrete_v2() {
        ...
        for name in ["i32", "u32", "u64", "usize", "isize", "f32"] {
            ...
            assert_eq!(
                ...
                None,
                "Into<{}> must drop (i64/f64 unsound substitution)",
                name
            );
        }
    ...
    fn wall6_private_module_named_reexport_resolves_public_path() {
        ...
        assert_eq!(
            reach.get("48").map(|s| s.as_str()),
            Some("fluentcrate::Db"),
            "Db should resolve to its root path"
        );
        assert_eq!(
            reach.get("2").map(|s| s.as_str()),
    ...
    fn test_trait_method_parametric_fn_leaves_recv_empty() {
        ...
        assert!(
            f.generic
                .as_ref()
                .map(|g| g.call.trait_qualifier.is_some())
                .unwrap_or(false)
        );
    ...
    fn test_nongeneric_trait_method_on_closed_monomorphic_self_binds() {
        ...
        for recv in [
            ...
            assert!(
                ...
                "UFCS self-path must contain `::tm::Holder<i64>`, got: {:?}",
                self_path
            );
            assert!(
                trait_path.contains("::tm::Area"),
                "UFCS trait-path must contain `::tm::Area`, got: {:?}",
                trait_path
            );
    ...
    fn test_facade_guidance_names_subcrate() {
        ...
        assert!(
            ...
            "note must name the sub-crate: {}",
            note
        );
        assert!(
            note.starts_with("note: 'facade_crate' re-exports its API from:"),
            "note must use the expected prefix format: {}",
            note
        );
        assert!(
            note.contains("ipe add other_crate"),
            "note must suggest the ipe add command: {}",
            note
        );
    }
    ...
    fn test_facade_guidance_supplementary_for_reexporting_crate() {
        ...
        assert_eq!(
            notes.len(),
            1,
            "a crate with an external glob re-export must produce a note even at 5 fns: {:?}",
            notes
        );
        let note = &notes[0];
        assert!(
            note.contains("also re-exports items from:"),
            "≥-threshold fn_count must use the supplementary wording: {}",
            note
        );
        assert!(
            note.contains("other_crate") && note.contains("ipe add other_crate"),
            "supplementary note must still name the sub-crate + add command: {}",
            note
        );
    }
    ...
    fn test_facade_guidance_absent_when_no_reexports() {
        ...
        assert!(
            notes.is_empty(),
            "crate with 0 fns but no re-exports must NOT produce guidance: {:?}",
            notes
        );
    }
    ...
    fn wall5_cyclic_alias_terminates_fail_closed() {
        ...
        assert!(
            rust.starts_with("Result<"),
            "outer Result must expand once: {}",
            rust
        );
        assert!(
            rust.contains("DbResult"),
            "inner cyclic alias must render OPAQUE at the depth bound: {}",
            rust
        );
        ...
        assert!(
            !s.is_empty(),
            "ipe rendering must also terminate non-empty: {}",
            s
        );
