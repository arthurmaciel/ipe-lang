...
fn emit_generic_coverage(crate_name: &str) {
    ...
    let report_path =
        safe_crate_name(crate_name).map(|c| format!(".skycache/ffi/rust/{c}.coverage.md"));
...
fn extract_angle_type_args(
    ...
    args.and_then(|a| a.get("angle_bracketed"))
        ...
        .map(|arr| {
            arr.iter()
                .filter_map(|arg| arg.get("type").map(|t| rustdoc_type_to_sky(t, aliases)))
                .collect()
        })
        .unwrap_or_default()
}
...
mod tests {
    ...
    fn test_bound_to_concrete_bare_iterator_param_drops() {
        ...
        assert_eq!(
            sky(&bound_to_concrete(&into, BoundPos::Param).unwrap()),
            "List Int"
        );
    }
    ...
    fn test_bound_to_concrete_v2() {
        ...
        assert_eq!(
            sky(&bound_to_concrete(&int_b, BoundPos::Param).unwrap()),
            "Int"
        );
        ...
        assert_eq!(
            sky(&bound_to_concrete(&float_b, BoundPos::Param).unwrap()),
            "Float"
        );
    ...
    fn test_parametric_stub_inherent_closure_emits_closure_argtype() {
        ...
        assert!(
            ...
            "no Fn bound should survive as a Sky bound"
        );
    ...
    fn parametric_stub_into_iterator_param_emits_vec_direct() {
        ...
        assert!(
            stub.bounds.is_empty(),
            "no iterator bound survives as a Sky bound"
        );
        assert_eq!(
            stub.call.arg_types,
            vec![TypeRef::Ctor(
    ...
    fn test_facade_guidance_names_subcrate() {
        ...
        assert!(
            note.contains("sky add other_crate"),
            "note must suggest the sky add command: {}",
            note
        );
    }
    ...
    fn test_facade_guidance_supplementary_for_reexporting_crate() {
        ...
        assert!(
            note.contains("other_crate") && note.contains("sky add other_crate"),
    ...
    fn wall5_cyclic_alias_terminates_fail_closed() {
        ...
        assert!(
            !s.is_empty(),
            "sky rendering must also terminate non-empty: {}",
            s
        );
    ...
    fn wall4_positive_recognises_and_extracts_output_from_constraints() {
        ...
        assert_eq!(
            sky, "Result String Int",
            "Output must be the real T, not (): {sky}"
        );
    }
    ...
    fn wall4_parse_fn_item_binds_async_trait_method_as_effectful_task() {
        ...
        // `Task Error ()`).
        assert_eq!(
            ret.sky_type, "Result String Int",
    ...
    fn wallf_rpitit_positive_recognises_and_extracts_output() {
        ...
        assert_eq!(
            rustdoc_type_to_sky(out, &HashMap::new()),
            "Result String Int",
