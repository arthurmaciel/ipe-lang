...
|---|---|---|---|---|---|
| | High | Sweep to green | `http_response_fields` E2E fixture fails at skyc tier — SKY-I0001 "no synthesised struct for HttpRequest fieldset" (`record_struct_by_key` miss for `let req = Http.defaultRequest "…"` with only a field read as consumer) | PRE-EXISTING on master (filed 2026-07-11 by the Medium lane; reproduced at fb32dd3 and 5cb500d compilers — not caused by that lane's work). Invisible to the default gate: the whole test body (compile assert included) is `SKY_E2E`-gated. See BACKLOG for the CLI repro. | |
