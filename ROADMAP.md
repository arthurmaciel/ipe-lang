...
| | High | Sweep to green | #90 SKY-L0114 ctor-payload-function — `Ok`/`Just` holding a function is rejected, making `Result.andMap` / `Maybe.andMap` unusable | **NEEDS A REAL DESIGN PASS, not another mechanical fix — 4 incidents same day (2026-07-10), all on the SAME sub-feature (T3, curried-`andMap` payload rejection).** Attempts 1-3 (all reverted): lambda-param reuse gate missing + `let`-bound-alias bypass; bare/re-exported-alias bypass because the check lived in the Call-node arm only. **Attempt 4** (`ffdafcf`, reverted `2a7b0d6`) implemented the revised two-tier design (Tier-2 `TyBounds::and_map_payload()` obligation + Tier-1 `lower_callee`-funnel backstop) and correctly closed all 11 prior aliasing shapes (verified by a from-scratch adversarial review writing its own fixtures, not re-running the lane's). A 12th shape broke it: an **annotated double forwarder** (`am2 x f = am1 x f` where `am1 x f = Result.andMap x f`, both explicitly typed) reaches `cargo build` as E0308. Root cause: `emitted_bound_satisfied`'s `and_map_payload` arm fails OPEN on a bare `Ty::Var` (unlike every sibling obligation, which fails closed) — the obligation escapes across the annotation-skolem boundary when `am1` is referenced inside `am2`'s body, since `check_scheme_applications` is a one-shot check, not a bound-transfer, so `am2` records no obligation of its own. A differential test against `Math.min`'s identical double-forwarder shape (which correctly rejects at both hops) confirms the "mirrors Math.min exactly" claim was false for this case. **Fix direction:** make `and_map_payload` fail closed on a non-concrete type, matching every sibling bound and the mechanism's own documented contract — open design question is whether genuine cross-binding bound propagation is needed to keep a legitimate arity-1 annotated double forwarder accepted, or fail-closed is acceptable (conservative, matching `Math.min`'s own behavior). T1/T2/T4 (blanket-rejection narrowing, lambda-param reuse gate) are independently solid per two separate reviews — a 5th attempt only needs to fix `and_map_payload`'s fail-open bug. **Do NOT rush a 5th attempt.** | `docs/architecture/ctor-payload-function-design.md`, `docs/architecture/ctor-payload-andmap-arity-gate-design.md` |
...
| | Medium | Hardening follow-ups | #129's 7 new `spawn_blocking` regression tests never run under the default `cargo nextest run --workspace` CI gate — feature-gated out, no CI job runs the runtime crate with `--features full` | Found by independent review of #129 (2026-07-10). See the matching BACKLOG.md row for detail. | `docs/architecture/class9-kernel-robustness-fix-spec-2026-07-09.md` |
| | Low | Hardening follow-ups | `system_load_env` does a synchronous `std::fs::read_to_string(".env")` not routed through #129's own `run_blocking` helper in the same file | Found by independent review of #129 (2026-07-10). Low impact — `.env` is tiny, read once at startup. | `docs/architecture/class9-kernel-robustness-fix-spec-2026-07-09.md` |
