...
| 2026-07-10 | #109/#156 `Ui.onSubmit`/`Std.Html.Events.onSubmit` dispatch via `Event::OnForm`; `Arc<dyn Any>` OnRaw removed — zero `dyn Any` in emitted-code paths; #63 `Sky.Http.Middleware.withCsrf` — double-submit CSRF for Sky.Http.Server + `ServerResponse` multi-`Set-Cookie` fix + a same-day well-formedness hardening fix from independent review; Class-8 web remainder (4 AUD-09 items); #45 kernel-scheme exhaustiveness gate. #90 ctor-payload-function lift and Boundary Scheme Promotion were both briefly landed, REVERTED after independent review reproduced real SEAL violations in each, then RE-LANDED same-day with the exact fixes closed and re-verified via real `cargo build` before/after repro against each specific failing shape (not just the gate) — see `docs/divergences-from-sky.md` B22/B23 for full detail. |
...
| 2026-07-10 | High | Sweep to green | #90 SKY-L0114 ctor-payload-function — `Ok`/`Just` holding a function is rejected, making `Result.andMap` / `Maybe.andMap` unusable | Re-landed after a same-day revert (independent review found two real SEAL violations: E0382 lambda-param reuse gap, E0277 let-bound-alias bypass of the curried-`andMap` gate). Re-fixed (T4 reuse gate now also runs over `lower_lambda`'s `ir_params`; T3 curried check moved from an AST-shape match to the kernel-call resolution boundary, closing every syntactic alias) and re-verified against BOTH exact failing shapes via real `cargo build` before/after repro, not just the gate. New blocker for the ex00 sweep: SKY-L0115 (see row below). See `docs/divergences-from-sky.md` B22. | `docs/architecture/ctor-payload-function-design.md` |
