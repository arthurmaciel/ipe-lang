...
**Golden tests** (`crates/ipe/tests/golden_*.rs`): a golden =
...
emitted project (THE SEAL: ipe-0 ⇒ cargo-0). Oracle files are regenerated
...
- **Per-step timeouts.** Any step that execs a subprocess (`ipe build` /
  `ipe run` / `ipe watch`) wraps the child in `timeout`. A step that doesn't
...
- **Monitors** in dev-loop tooling (`ipe watch`) need a heartbeat/max-wait so
...
- **`ipe build` ⇒ the emitted Rust `cargo build`s** (THE SEAL — every
