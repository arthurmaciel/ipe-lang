...
pub struct WatchOptions {
    ...
    /// The port injected as `SKY_LIVE_PORT` for the spawned child and probed
            ...
            OrchestratorEvent::CompileDone {
                ...
                match outcome {
                    ...
                    CompileOutcome::Red(msg) => {
                        eprintln!("[ipe watch] build failed (last-good binary stays up):\n{msg}");
...
///
/// Sets both `SKY_LIVE_PORT` and `SKY_SERVER_PORT` to the SAME configured
/// port — harmless (ignored) for whichever app shape didn't ask for it, and
/// what lets a `Sky.Http.Server` fixture that reads `SKY_SERVER_PORT` (the
...
/// environment already configures `SKY_LIVE_STORE`, in which case that
...
fn child_env(port: u16, out_dir: &Path) -> Vec<(String, String)> {
    let mut env = vec![
        ("SKY_LIVE_PORT".to_owned(), port.to_string()),
        ("SKY_SERVER_PORT".to_owned(), port.to_string()),
    ];
    if std::env::var("SKY_LIVE_STORE").is_err() {
        env.push(("SKY_LIVE_STORE".to_owned(), "sqlite".to_owned()));
        let db_path = out_dir
            .parent()
            .unwrap_or(out_dir)
            .join(".ipe-watch-sessions.db");
        env.push((
            "SKY_LIVE_STORE_PATH".to_owned(),
...
fn warn_if_memory_store() {
    if std::env::var("SKY_LIVE_STORE").as_deref() == Ok("memory") {
        eprintln!(
            "[ipe watch] warning: SKY_LIVE_STORE=memory is set — session state will NOT \
             survive a watch-triggered restart. Unset it (watch defaults to sqlite) or set \
             SKY_LIVE_STORE=sqlite explicitly to keep your session across rebuilds."
        );
    }
}
