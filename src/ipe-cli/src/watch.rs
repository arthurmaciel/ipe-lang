...
/// `file` (not `sqlite`) is the default because a plain `Web.app` reaches
/// no DB kernel, so the emitted crate carries no `db` feature and a
/// `sqlite` request would silently degrade to a non-persistent in-memory
/// store — defeating the Model handoff. The `file` store rides the `web`
/// feature every web build already has.
