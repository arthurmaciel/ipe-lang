...
pub const FOREIGN_ALLOWLIST: &[(&str, &str, &str)] = &[
    // `docs/reference/capabilities.md` does not exist yet — every axis is
    // "undocumented" by the file-presence check. All documented axes are
    // tracked via the capability-vocabulary source until the reference file
    // is generated.
    (
        "documented",
        "network",
        "docs/reference/capabilities.md not yet generated",
    ),
    (
        "documented",
        "filesystem",
