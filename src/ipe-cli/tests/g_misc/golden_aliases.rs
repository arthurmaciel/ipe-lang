...
//!
//! ```text
//! $ cd "$(mktemp -d)" && cp Main.ipe . && ipe run Main.ipe   # Go backend
//! 42
//! ```
//!
...
/// alias-annotated program prints `42` — the same value the Go backend produces.
...
fn end_to_end_builds_and_prints_forty_two() {
    ...
    let root = repo_root();
    let entry = example_entry(&root);
    let out = std::env::temp_dir().join("ipec_m1_aliases_e2e");
    let _ = std::fs::remove_dir_all(&out);
