...
//! a golden oracle diff; the point is the Attribute NEWTYPE identity, which the Go
...
/// `IPE_E2E=1`.
fn build_run_attribute_home_179() -> crate::support::RunOutcome {
    let root = repo_root();
    let dir = root
        .join("tests")
        .join("golden")
    ...
    crate::support::build_and_run_emitted("attribute_home_disambiguation_179", &out)
}
