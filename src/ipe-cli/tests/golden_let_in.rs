//! `let … in` gate: `skyc` must emit `main.rs` byte-identical to the
...
//!
//! ```text
//! $ sky run tests/golden/let_in/Main.sky   # Go backend
//! 22
//! ```
//!
...
fn end_to_end_builds_and_prints_twenty_two() {
    ...
    let outcome = support::build_and_run_emitted("let_in", &out);
