...
/// Repo root: `src/playground` → `src` → repo.
...
fn stage_ipe(source: &str) -> Staged {
    ...
    warm_and_seed(&scratch, &crate_dir);
    Staged { scratch }
}
...
fn warm_and_seed(scratch: &tempfile::TempDir, crate_dir: &Path) {
    ...
    seed_cargo_home(scratch.path(), &warm_home).expect("seed cargo home");
    seed_target_dir(scratch.path(), &warm_target).expect("seed target dir");
}
