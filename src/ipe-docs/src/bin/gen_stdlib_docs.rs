//! Generator for `docs/stdlib.md` and `docs/internals/stdlib/<Module>.md`.
//!
//! Reads the `ipe_docs` index (built from the stdlib veneers) and renders two
//! doc surfaces:
//!
//! - `docs/stdlib.md` — the index: every module with a one-line summary for
//!   each documented symbol, cross-linked to its detail page.
//! - `docs/internals/stdlib/<Module>.md` — one page per module that has at
...
fn run() -> Result<(), String> {
    ...
    let content_dir = repo_root.join("docs/content");
    ...
    write_file(&repo_root.join("docs/stdlib.md"), &stdlib_md)?;

    let internals_dir = repo_root.join("docs/internals/stdlib");
    create_dir_all(&internals_dir)?;
    ...
    for (module_name, symbols) in &module_map {
        ...
        write_file(&internals_dir.join(&file_name), &detail)?;
    }
...
/// Render `docs/stdlib.md`: a module-by-module index with one-line symbol summaries.
fn render_stdlib_index(module_map: &BTreeMap<String, Vec<SymbolDoc>>) -> String {
    ...
    // Per-module sections.
    for (module_name, symbols) in module_map {
        let detail_link = format!("internals/stdlib/{}", module_file_name(module_name));
...
/// Render `docs/internals/stdlib/<Module>.md`: full doc-strings per symbol.
...
/// Convert a short module name to the file name used under `docs/internals/stdlib/`.
///
