...
/// level, but `sky_watch` is deliberately dependency-light (Task 21's own
...
impl SupervisorState {
    ...
    /// and `SKY_LIVE_STORE` injection); `readiness` and `timeouts` govern
    /// the readiness gate.
    ///
...
/// `SKY_LIVE_STORE`, …) without silently dropping that configuration is to
...
mod tests {
    ...
    fn last_good_hash_is_deterministic() {
        let dir =
            std::env::temp_dir().join(format!("sky_watch_process_hash_{}", std::process::id()));
    ...
    fn apply_green_falls_back_to_last_good_when_a_restart_candidate_fails_readiness() {
        ...
        // SAME `spawn` closure.
        let dir =
            std::env::temp_dir().join(format!("sky_watch_process_fallback_{}", std::process::id()));
        std::fs::create_dir_all(&dir).unwrap();
