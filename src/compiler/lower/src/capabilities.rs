...
mod tests {
    ...
    fn http_program_infers_network() {
        let caps = caps_of(
            "module Main exposing (main)\nmain : Task ()\nmain =\n    Task.andThen (\\_ -> Log.println \"done\") (Http.get \"http://example.com\")\n",
        );
        assert_eq!(
            caps,
    ...
    fn file_and_env_program_infers_both() {
        let caps = caps_of(
            "module Main exposing (main)\nmain : Task ()\nmain =\n    Task.andThen (\\_ -> File.writeFile \"/tmp/x\" (System.getenvOr \"HOME\" \"/\")) (Log.println \"go\")\n",
        );
        assert_eq!(
            caps,
