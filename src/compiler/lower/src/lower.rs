...
fn set_arg_bug() -> Diagnostic {
    bug("ipe_lower::ir_type", "Set applied without its element type")
}
...
/// (`ipe_runtime::json::Decoder<E, T>` holds a `Box<dyn Fn(&JsonVal) -> …>`);
...
// AFTER `ipe` exit 0 (SEAL break). Example:
//
...
fn eta_expand_leaf_to_shared(
    ...
    for (offset, (_, pty)) in params.iter().enumerate() {
        ...
        let sym = *eta_pool.get(offset).ok_or_else(|| {
            bug(
                "ipe_lower::eta_expand_leaf_to_shared",
...
/// never mention a Ipê type variable. Used by [`body_boxes_generic_callback`] to
...
/// program is well-typed to `ipe` but the emitted Rust fails `cargo build` (a
/// SEAL violation).
///
...
//
// A pure-`Fun` binding (`ipe_ir::fun_value_arc_promotable`) whose reads exceed
...
///
/// Each flag is the OR of the same [`ipe_ir::KernelFn`] family predicate the
...
/// compiler is broken"): the input is valid Ipê the supported subset has not reached.
...
pub struct Lowerer<'a> {
    ...
    /// The `Rust.*`-home unions that are TRANSPARENT FFI imports — a real
    /// record/union declaration from a driver-generated interface module, not
    /// the opaque `type N = N` placeholder (whose shape — one nullary ctor
    /// spelling the type name — the classification refuses for transparent
    /// enums, so the two are distinguishable by construction). Members lower
    ...
    /// construct and match.
    transparent_ffi_ctors: BTreeSet<(ModPath, Symbol)>,
...
impl<'a> Lowerer<'a> {
    ...
    pub fn new(
        ...
        let mut transparent_ffi_unions = BTreeSet::new();
        let mut transparent_ffi_ctors = BTreeSet::new();
    ...
    fn fresh_any_param_symbol(&self) -> DResult<Symbol> {
        ...
        let sym = *self.any_param_binders.get(i).ok_or_else(|| {
            bug(
                "ipe_lower::fresh_any_param_symbol",
    ...
    fn fresh_destructure_thunk_symbol(&self) -> DResult<Symbol> {
        ...
        let sym = *self.destructure_thunk_binders.get(i).ok_or_else(|| {
            bug(
                "ipe_lower::fresh_destructure_thunk_symbol",
    ...
    fn fresh_nested_cons_binder(&self) -> DResult<Symbol> {
        ...
        let sym = *self.nested_cons_binders.get(i).ok_or_else(|| {
            bug(
                "ipe_lower::fresh_nested_cons_binder",
    ...
    fn fresh_nested_strlit_binder(&self) -> DResult<Symbol> {
        ...
        let sym = *self.nested_strlit_binders.get(i).ok_or_else(|| {
            bug(
                "ipe_lower::fresh_nested_strlit_binder",
    ...
    pub fn run(self) -> Result<Program, (Diagnostic, Vec<Symbol>)> {
        ...
        for u in &self.m.unions {
            ...
            // via the crate's opaque-type path map instead. A TRANSPARENT
            // union (a real closed-union declaration, never the placeholder
            // shape) falls through and lowers to an app enum — its values are
            // native Ipê values, converted at the wrapper seam.
            if self.is_foreign_interface_union(u)
                && !self
                    .transparent_ffi_unions
                    .contains(&(ModPath(u.home.clone()), u.name))
            {
                continue;
            }
    ...
    /// The clone-classification context for this module.
    const fn clone_env(&self) -> CloneEnv<'_> {
        CloneEnv {
            ...
            for arg in &ctor.args {
                ...
                if let Some((carrier, found)) = self.task_arity_in_canon(arg) {
                    return Err(Diagnostic::Type {
                        span: ctor.span,
                        msg: ipe_diagnostics::TypeError::TaskArity { carrier, found },
                    });
                }
        ...
        match def {
            canon::Def::Typed {
                ...
                *self.current_poly_tvars.borrow_mut() = saved_poly_tvars;
                ...
                // loop so the Rust stack stays flat (mirrors Ipê's TailCallOpt).
    ...
    fn split_unannotated_sig(
        ...
        for pat in patterns {
            let Ty::Fun(arg, rest) = cur else {
                return Err(bug(
                    "ipe_lower::split_unannotated_sig",
        ...
        match &pat.value {
            canon::Pattern_::PRecord(fields) => {
                let ty = self.region_ty(param_span).ok_or_else(|| {
                    bug(
                        "ipe_lower::lower_param_binder_pat",
    ...
    fn ir_type_from_canon(&self, t: &canon::Type, generics: &[Symbol]) -> DResult<IrType> {
        match t {
            ...
            canon::Type::Con { home, name, args } => match self.resolve(*name)? {
                ...
                // distinct from String). Divergence from Ipê: Ipê aliases
                // Bytes = String; Ipê-Rust makes Bytes a proper byte type.
                ...
                // Ipê users write this as a structural record literal, but the
    ...
    fn apply_param_move_ownership(
        ...
        // (the single ipe_ir authority); the destructured `ps`/`r` feed the shim
    ...
    /// `Fn(i64, i64) -> i64` return slot, which cargo rejects with no Ipê
    ...
    fn lower_lambda(
        ...
        // after ipe exit 0 (SEAL break). Pad the closure with fresh trailing
    ...
    fn ir_type_from_ty(&self, t: &Ty, span: Span) -> DResult<IrType> {
        match t {
            ...
            Ty::Con { name, args, module } => match self.resolve(*name)? {
                ...
                // Zero type arguments; maps to `ipe_runtime::Db`.
                "Db" => Ok(IrType::Db),
                ...
                match self.resolve(*name)? {
                    ...
                    "Cmd" if args.len() == 1 => {
                        let inner = self.ir_type_from_ty_json(
                            args.first().ok_or_else(|| {
                                bug(
                                    "ipe_lower::ir_type_from_ty_json",
                                    "Cmd applied without its message type",
                                )
                            })?,
                            span,
    ...
    /// first-class Ipê function value, but the WRONG shape here: it does
    /// not satisfy `decode_pipeline_required`'s (etc.) `Box<dyn FnOnce>`
    /// parameter, an E0308 cargo-fail invisible to `ipe` because eta-lambda
    ...
    fn ir_type_from_ty_pipeline_decoder(&self, t: &Ty, span: Span) -> DResult<IrType> {
        ...
        let inner = args.first().ok_or_else(|| {
            bug(
                "ipe_lower::ir_type_from_ty_pipeline_decoder",
                "Decoder applied without its element type",
            )
        })?;
    ...
    /// shaped kernel parameter in `src/runtime/rust/src/{json,db}.rs`
    ...
    /// exposed as a Ipê kernel, so it needs no gate entry.
    ...
    /// first-class values and whose region type cannot be recovered from the Ipê
    ...
    fn lower_expr(&self, e: &canon::Expr) -> DResult<Expr> {
        ...
        match &e.value {
            ...
            // regardless of context — the SAME class of ipe-exit-0-then-
            ...
            canon::Expr_::VarCtor {
                ...
                if arity == 0 {
                    ...
                    // (e.g. `onInput InputAlertMetric`, `List.map Just`).  Elm / Ipê
            ...
            canon::Expr_::VarTopLevel { .. } | canon::Expr_::VarKernel { .. } => {
                ...
                if matches!(&callee, Callee::Kernel(_)) && self.callee_arity(&callee)? == 0 {
                    ...
                    // failing closed at `ipe` rather than emitting invalid Rust.
    ...
    /// with the same dedicated diagnostic. Divergence from Ipê, rationale: Rust
    /// backend capability.
    ///
    ...
    fn kernel_turbofish_pin(
        ...
        match k {
            ...
            // `Result e Decimal` — pin the error channel `e` (1st arg). Ipê's
            ...
            // so rustc reports E0283 even though Ipê knows the type. Pinning
    ...
    /// an unsatisfiable `+ Sync` → ipe-0-then-cargo-fail (E0308 / E0277).
    ///
    ...
    fn eta_expand_partial(
        ...
        // matching Ipê's pure-functional semantics.
        let result = hoisted
            .into_iter()
            .rev()
        ...
        let slot_classes: Vec<Option<CloneClass>> = arg_tys
            ...
            .map(|slot_ty| {
                self.ir_type_from_ty(slot_ty, call_span)
                    .ok()
                    .as_ref()
                    .map(|t| clone_class(self.clone_env(), t))
            })
            .collect();
        let mut hoisted: Vec<(Symbol, Expr)> = Vec::new();
        let mut cap_cursor = 0usize;
    ...
    /// Ipê-level diagnostic. (A surplus that EXCEEDS the returned closure's arity
    ...
    fn ctor_arity_of(&self, home: &ModPath, name: Symbol) -> DResult<usize> {
        ...
        // declared (the foreign type is the real `::crate::Type`). A
        // TRANSPARENT union's constructors are the one `Rust.*` set a program
        // constructs legitimately — those proceed to the ordinary lookup.
        if self.is_foreign_interface_home(&home.0)
            && !self.transparent_ffi_ctors.contains(&(home.clone(), name))
        {
            return Err(bug(
        ...
        match &callee.value {
            canon::Expr_::VarKernel { id, module, name } => {
                ...
                match (self.resolve(*module)?, self.resolve(*name)?) {
                    ...
                    // coercion. Divergence from Ipê: Ipê routes args
                    // through AsInt; Ipê-Rust follows Elm's polymorphic
                    ...
                    ("Db", "unsafeGetString") => Ok(Callee::Kernel(KernelFn::DbGetString)),
                    ("Db", "unsafeGetInt") => Ok(Callee::Kernel(KernelFn::DbGetInt)),
                    ("Db", "unsafeGetBool") => Ok(Callee::Kernel(KernelFn::DbGetBool)),
                    ("Db", "unsafeGetField") => Ok(Callee::Kernel(KernelFn::DbGetField)),
            ...
            canon::Expr_::VarTopLevel { module, name } => {
                ...
                let id = *self
                    ...
                    .ok_or_else(|| bug("ipe_lower::lower_callee", "unknown top-level binding"))?;
                Ok(Callee::Func(id))
            }
    ...
    fn lower_destructure_pat(&self, p: &canon::Pattern) -> DResult<Pat> {
        match &p.value {
            ...
            canon::Pattern_::PRecord(fields) => {
                let ty = self.region_ty(p.span).ok_or_else(|| {
                    bug(
                        "ipe_lower::lower_destructure_pat",
                        "nested record sub-pattern has no solved region type",
                    )
                })?;
                self.lower_record_pat(fields, ty, p.span)
    ...
    fn tuple_case_supported(
        ...
        let Some(elems) = literal_elems else {
            let any_col_needs_literal_tuple_path = branches.iter().any(|br| {
                ...
                // A list / cons sub-pattern needs the coerced-column (literal-tuple)
    ...
    /// depth. A slice sub-pattern (`[a, ..]`) needs `&[T]` from an owned `Vec`,
    ...
    /// path and lowers on the variable-scrutinee path.
    fn col_needs_literal_tuple_path(pat: &canon::Pattern) -> bool {
        match &pat.value {
            // List / cons leaves — the shapes that ONLY the coerced-column path can
            // lower (a slice pattern against an owned `Vec` element).
            canon::Pattern_::PList(_) | canon::Pattern_::PCons(_, _) => true,
            // Structural sub-patterns — recurse into every child. `PList` / `PCons`
            // are handled by the leaf arm above, so only the remaining nesting
            // shapes appear here.
            canon::Pattern_::PTuple(cols) => cols.iter().any(Self::col_needs_literal_tuple_path),
            canon::Pattern_::PCtor { args, .. } => {
    ...
    fn lower_case(&self, scrut: &canon::Expr, branches: &[canon::CaseBranch]) -> DResult<Expr> {
        ...
        let first = branches
            .first()
            .ok_or_else(|| bug("ipe_lower::lower_case", "empty case expression"))?;
        ...
        // Each Ipê `case` arm becomes its OWN Rust `match` arm, in source order.
        ...
        // reachable Ipê program of the repro shape (`Just (h::t) -> … ; _ -> …`)
        ...
        // (e.g. `Just (h::t)` + `Just []` + `Nothing`, exhaustive at Ipê level
        ...
        if all_ctor {
            ...
            let variants = self
                ...
                .ok_or_else(|| bug("ipe_lower::lower_case", "unknown scrutinee enum"))?;
...
mod tests {
    ...
    fn decl_equiv_legacy_match() {
        ...
        let kern_syms: Vec<(ipe_intern::Symbol, ipe_intern::Symbol)> = KernelFn::ALL
            .iter()
            .map(|sk| {
                let d = sk.decl();
    ...
    /// `KernelFn`).
    ///
    /// `constrain_var_kernel` (`ipe_types::constrain`) types a call against
    ...
    /// program can pass `ipe`'s type-check against `decl().arity`'s arrow
    /// count and still emit a Rust call with the wrong argument count,
    /// caught only by `cargo`, never by `ipe`.
    ///
    ...
    /// `ipe_ir::carrier_is_clone` (the emitter-side default-carrier authority)
    ...
    fn ffi_foreign_opaque_handle_is_non_clone() {
        ...
        assert_eq!(clone_class(env, &world_ty), CloneClass::NonClone);
        assert_eq!(
            clone_class(
                env,
        ...
        assert!(!param_is_multiuse_clonable(env, &world_ty));
        assert!(ir_type_has_ffi_foreign_handle(env, &world_ty));
        assert!(ir_type_has_ffi_foreign_handle(
            env,
            &IrType::Maybe(Box::new(world_ty.clone()))
        ));

        // A real user enum keeps deriving Clone and is multi-use clonable.
        assert_eq!(clone_class(env, &user_ty), CloneClass::CloneOk);
        assert!(param_is_multiuse_clonable(env, &user_ty));
        assert!(!ir_type_has_ffi_foreign_handle(env, &user_ty));
        ...
        assert_eq!(clone_class(env, &app_ty), CloneClass::NonClone);

        // A TRANSPARENT import under the same `Rust.*` home is a real app
        // enum: CloneOk (it derives `Clone`), multi-use clonable, and never
        // an opaque handle for the linearity gate.
        let mut transparent_world = BTreeSet::new();
        transparent_world.insert((ModPath(vec![rust, bevy]), world));
        let env_t = CloneEnv {
            interner: &interner,
        ...
        assert!(!ir_type_has_ffi_foreign_handle(env_t, &world_ty));
        // The sibling opaque name under the same home stays NonClone.
        assert_eq!(clone_class(env_t, &app_ty), CloneClass::NonClone);
    }
    ...
    fn ffi_foreign_handle_reuse_fails_closed() {
        ...
        use super::{CloneEnv, reject_foreign_handle_reuse, unsupported};
        ...
        let err = reject_foreign_handle_reuse(env, sym, &world_ty, &reused, span)
