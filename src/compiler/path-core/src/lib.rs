...
//! the SAME way, so the algorithm lives here ONCE and both crates depend on it.
//! Neither keeps its own copy. The crate is dependency-free (std only) so it can
//! sit below both `ipe_runtime` and `ipe_diagnostics` without a dependency
//! cycle, and so the compiler can validate a literal without pulling in the
