...
/// target-neutral `dom` data path, the TEA types, and the browser sink.
/// tokio-backed modules (`task`/`log`/`trace`), the reqwest client, crypto,
/// and every server/db surface are absent BY CONSTRUCTION (Layer 3 of the
/// security gate).
const WASM_RUNTIME_MOD_RS: &str = "\
// GENERATED by Sky compiler — do not edit (browser-WASM module set)
pub mod basics;
pub mod bytes;
pub mod char_kernel;
pub mod config;
...
/// prelude by [`wasm_runtime_bindings`]. Keyed on module paths (structural),
/// so a prelude drift auto-adapts.
const WASM_ABSENT_MODULE_PATHS: &[&str] = &[
    ...
    // the clock/sleep entry points inside it are tokio-gated out of the wasm
    // build — their wrappers must go with them.
    "ipe_runtime::time::",
];

/// The wasm-target kernel-wrapper prelude: [`runtime_bindings`] filtered to
/// the blocks whose runtime modules exist in [`WASM_RUNTIME_MOD_RS`]. The
/// Layer-1 gate already denies the kernels behind the dropped wrappers, so
/// no emitted call site can reference them.
fn wasm_runtime_bindings() -> DResult<String> {
    let full = runtime_bindings()?;
    let mut out = String::with_capacity(full.len());
    let mut first = true;
    for block in full.split("\npub fn ") {
        let keep = !WASM_ABSENT_MODULE_PATHS.iter().any(|p| block.contains(p));
        if first {
