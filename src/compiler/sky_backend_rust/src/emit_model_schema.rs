...
/// `sky_runtime::live::store::LIVE_MODEL_SCHEMA_WIRE_VERSION`; bumped ONLY
...
mod tests {
    ...
    fn hash_record(
        interner: &Interner,
        program: &Program,
        fields: BTreeMap<sky_intern::Symbol, IrType>,
    ) -> DResult<[u8; 32]> {
    ...
    fn wire_epoch_matches_runtime_constant() {
        let path = std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../../runtime/rust/src/sky_runtime/live/store.rs");
