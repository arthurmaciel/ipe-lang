    ...
    assert!(
        out.contains(
            "MainMaybe::Just(p0) => format!(\"Just {}\", \
             (&sky_runtime::stringify::Wrap(p0)).dispatch()),"
        ),
    ...
    assert!(
        out.contains(
            "MainShape::Rect(p0, p1) => format!(\"Rect {} {}\", \
             (&sky_runtime::stringify::Wrap(p0)).dispatch(), \
             (&sky_runtime::stringify::Wrap(p1)).dispatch()),"
        ),
...
fn end_to_end_generic_maybe_prints_five() -> DResult<()> {
    let mut interner = Interner::new();
    let prog = maybe_program(&mut interner)?;
    build_and_assert(&interner, &prog, "sky_backend_adts_maybe_e2e", "5\n")
}
...
fn end_to_end_recursive_tree_prints_twelve() -> DResult<()> {
    let mut interner = Interner::new();
    let prog = tree_program(&mut interner)?;
    build_and_assert(&interner, &prog, "sky_backend_adts_tree_e2e", "12\n")
}
...
fn build_and_assert(
    ...
    let runtime = resolve_runtime().ok_or_else(|| Diagnostic::CompilerBug {
        where_: "adts e2e",
        detail: "could not locate src/runtime/rust/src/sky_runtime".to_owned(),
    })?;
