// Postgres `sky_runtime/config.rs` template.
//
// NOT compiled into the standalone `sky-runtime-rust` crate's default build
// (the crate root always uses `config.rs`, the sqlite variant). This file
// exists purely as an `include_str!` source for `sky_backend_rust::project`
// (see `RUNTIME_CONFIG_RS_DB_POSTGRES`), which writes it verbatim to
// `<emitted-project>/src/sky_runtime/config.rs` when `sky.toml`'s
// `[database] driver = "postgres"`.
//
// Every symbol name matches `config.rs`'s sqlite variant exactly (`DbPool`,
// `DbRow`, `sky_db_url`, `db_last_insert_id`, `db_format_sql`,
