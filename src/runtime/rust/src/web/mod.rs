...
/// with a hardcoded fallback. Malformed values fall back to the default; never panics.
...
/// is threaded into `render_page_full` so the client JS prefixes
/// `/_ipe/event` + `/_ipe/sse` with it. The browser reaches this child only
/// through the parent proxy, which strips the prefix before forwarding — so
/// the child's own router stays root-relative.
...
/// Tunable via `IPE_WEB_SHUTDOWN_GRACE_MS` (default 1500 ms; 0 = exit at once).
...
/// Default preservation for the three security-critical env vars.
///
/// Each test covers two cases:
///   (a) `IPE_WEB_*` name takes effect,
///   (b) unset → unchanged default behavior.
///
...
mod security_env_tests {
    ...
    // instead test the env-read layer directly: `read_env_var` returns the value
    // when set and returns `Err` when unset.
    ...
    // layer: name takes effect, unset → empty (→ `None` in production).
    ...
    fn frame_ancestors_new_name_takes_effect() {
        ...
        // (b) unset → Err; frame_ancestors() would return None (no CSP
    ...
    #[test]
    fn web_max_body_bytes_name_and_default() {
        ...
        // (b) unset → default (5 MiB); a rename bug that silently zeros this
