...
#![cfg(feature = "regex")]
//! Parity fixtures for Ipe.Regex kernels.
//!
//! Exercises `Regex_match` / `Regex_find` / `Regex_findAll` / `Regex_replace` /
//! `Regex_split` using RE2 semantics.
//!
...
//! unvalidated pattern.
//!
//! Divergence note: the RE2 engine and Rust's `regex` crate are both
...
//! `Regex::split` disagrees on zero-width matches at position 0 (Rust emits a
//! leading empty field; RE2 drops it). `regex_split`
//! therefore reimplements the Split algorithm directly rather than
...
//
// Golden: split semantics. Two behaviours combine here.
// (1) `allMatches` (shared by FindAll and Split) NEVER delivers an empty match that
...
// adjacent to a prior match. (2) Split skips the field a match would
...
// inputs. `regex_split` reimplements the Split over `find_iter`, so each case
// below yields `["a", "b", "c"]` with NO leading empty — matching the reference behaviour.

#[test]
fn regex_split_zero_width_matches_go() {
    // Golden: Split("", "abc") — empty matches at 0,1,2,3; the position-0 field is
    // dropped → ["a", "b", "c"] (Rust's native Regex::split → ["", "a","b","c"]).
    assert_eq!(
        regex_split(re(""), "abc".to_string()),
        vec!["a".to_string(), "b".to_string(), "c".to_string()],
        "empty-pattern split drops the leading zero-width field (reference parity)"
    );
    // Golden: Split("x*", "axxbxc") — matches (0,0),(1,3),(4,5),(6,6); the empty
    // matches at the run boundaries are suppressed (adjacent to a prior match).
    assert_eq!(
        regex_split(re(r"x*"), "axxbxc".to_string()),
        vec!["a".to_string(), "b".to_string(), "c".to_string()],
        r"x* split must match reference zero-width handling"
    );
    // Golden: Split(`\d*`, "a1b22c") — matches (0,0),(1,2),(3,5),(6,6) → ["a","b","c"].
    assert_eq!(
        regex_split(re(r"\d*"), "a1b22c".to_string()),
        vec!["a".to_string(), "b".to_string(), "c".to_string()],
        r"\d* split must match reference zero-width handling"
    );
    // Golden: Split(`,?`, "a,b,c") — matches (0,0),(1,2),(3,4),(5,5) → ["a","b","c"].
    assert_eq!(
        regex_split(re(r",?"), "a,b,c".to_string()),
        vec!["a".to_string(), "b".to_string(), "c".to_string()],
        r",? split must match reference zero-width handling"
    );
}
...
fn regex_handles_empty_string_inputs() {
    // golden-verified: operations on empty string must not panic
...
fn regex_match_anchors_work_like_go() {
    // regexp does partial matching by default (no implicit ^...$).
