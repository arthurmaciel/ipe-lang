...
| Define surface — `foreign` declarations in `src/Ffi/<Crate>.ipe` (struct/enum/closure); async closures, closure→`run` handoff, trait-impl exposure, `#[define_in_ipe]`; legacy `[[rust.define.*]]` toml retired (migrate via `ipe migrate config`) | shipped | `src/compiler/ffi/src/{wrapper,instance,bindings}.rs`, seals `define_*_seal.rs`, `closure_handoff_seal.rs` |
