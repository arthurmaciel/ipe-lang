...
| `IPE_WEB_STORE` | memory | Session-store backend for the web server: `memory` (per-process, lost on restart), `file` (persisted to `IPE_WEB_STORE_PATH`, no database dependency), or `sqlite` (persisted, requires the `db` feature). `ipe watch` selects `file` so a rebuild preserves live sessions even for a plain web app; a `sqlite` request degrades to `file` when the build has no `db` feature. | `Tunable` |
