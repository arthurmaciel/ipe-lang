...
43 kernels in the Std.Html / Std.Ui / Sky.Live rendering family were typed as
...
record of 9 fields under one Sky-level arity — and needs an explicit decision
...
- **Invariant that must keep holding:** `arrow-count == decl().arity ==
  ...
  `Ty::Var(MAX)` hole that passes `skyc` and fails `cargo`.
