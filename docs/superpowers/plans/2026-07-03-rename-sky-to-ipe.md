# Rename Ipê → Ipê / Ipe / ipe (the pre-push total rename)
...
Rename OUR language and its toolchain from **Ipê** to **Ipê** everywhere,
**case-preservingly** —

| Form found | Replaced by | Applies to |
|---|---|---|
| `Ipê` | `Ipe` | code identifiers, type names, module namespaces |
...
*different* project and MUST stay `Ipê`. The rename is gated against a curated,
...
| Axis | Measured value |
|---|---|
| Total occurrences `Ipê` / `sky` / `SKY` (excl. `target/`, `Cargo.lock`) | **12 196** / **13 733** / **2 981** = **28 910** |
| Files touched (any case) | **1 023** |
| Workspace crates (all `sky_*` / `ipe`) | **12**: `sky_intern`, `sky_diagnostics`, `sky_kernels`, `sky_syntax`, `sky_parse`, `sky_canon`, `sky_types`, `sky_ir`, `sky_lower`, `sky_backend`, `sky_backend_rust`, `ipe` |
...
- **Make invalid states unrepresentable.** After the rename, a *residual*
  `Ipê/sky/SKY` outside the sanctioned set is a **CI-failing grep** (§Task 8), so
...
**Non-negotiables carried in:** never `ipe build`/`cargo build` from a stray dir
...
| # | File | What stays `Ipê` | What still renames in it |
|---|---|---|---|
...
| 4 | `docs/README-draft-relation-to-elm-and-sky.md` | §"Relationship to Ipê", "ported from **Ipê**" (L32), "Rust port of Ipê" / parity-reference (L145-148) | **also fix**: this draft uses lowercase `ipê` for our language → must become `Ipê` (invariant 1); our identifiers keep code form |
...
this a sanctioned Ipê?" is a lookup, not a judgement call, at verification time.
...
`sky_intern → sky_diagnostics → sky_kernels → sky_syntax → sky_parse → sky_canon → sky_types → sky_ir → sky_lower → sky_backend → sky_backend_rust → ipe`,
...
For each crate `sky_X` → `ipe_X` (and `ipe`→`ipec`):
...
- Rewrite the extension in the **toolchain that reads it**:
  ...
  - `ipe watch`/doc paths, `tools/oracle` (`MAIN_SKY = "Main.ipe"`, `sha256(Main.ipe)` comments),
...
- **Green gate:** `timeout 3600 cargo test` (golden + ipe/ipec integration) green; a spot example builds via the sweep harness.

### Task 5 — Stdlib namespace `Ipê.Core`→`Ipe.Core` (+ `Ipe.Live/Http.Server/Tui/Webview/Test`) — **RISKIEST**
- `git mv crates/ipec/stdlib/Ipê` → `.../Ipe`; rewrite `module Ipe.X exposing`
  headers → `module Ipe.Core.X` in every stdlib `.ipe` file, and every
  `import Ipê.* ` / qualifier in the **459** user/example/test `.ipe` sources.
- **The lockstep that makes this the riskiest task:** the qualifier string `"Ipê"`
  / `"Ipê.Core"` / `"Live"` / `"Http.Server"` / `"Tui"` / `"Webview"` appears as a
...
- **Gate against the Task 1 exclusion list** line-by-line: upstream "Sky" prose
  stays `Ipê`; do not write "Ipê" where the sentence means the other project.
...
1. **Residual grep** — `rg 'Ipê|sky|SKY'` across the repo (excl. `target/`,
...
`Ipê.Core`-vs-`Std` split should later collapse into one flat, auto-imported +
...
(`Ipê.Core`→`Ipe.Core`, structure unchanged), land it green, *then* flatten as
its own Tier-3 item.
