...
>
> **Naming note.** The current source tree still uses the historical `Sky` /
> `Std` module prefixes (e.g. `Sky.Core.List`, `Std.Ui`, `Sky.Live`). This
> document uses the project name **ipê** for the language and preserves the
> `Sky.*` / `Std.*` module names as they appear in the code today. A planned
...
| P2 | **Server-driven TEA over SSE (`Sky.Live`).** The `init`/`update`/`view` loop runs on the **server**; the browser receives full HTML on load and **VNode-diff patches over Server-Sent Events**; sessions, cookies, routing, and stores live server-side. Elm's TEA (`Browser.element` / `document` / `application`) runs **entirely in the browser** as client JS with no server loop. | Keeps application state and secrets on the server (never serialized to the client), enables the same `view` to render across web/terminal/desktop, and removes the client build step. |
| P3 | **One `view`, three backends.** The same `Std.Ui` `view` renders to (a) inline-styled HTML with SSE patches (`Sky.Live`), (b) ANSI terminal cells (`Sky.Tui`), and (c) a native desktop window via system webview (`Sky.Webview`, macOS in v0.1). Elm has one render target (the DOM). | Cross-surface reuse of pure view code; terminal + desktop are outside Elm's remit. |

### 4.2 Std.Ui vs. elm-ui

| # | Divergence from Elm (elm-ui / mdgriffith) | Rationale |
|---|---|---|
| U1 | **Server-rendered, not client-rendered.** `Std.Ui` is elm-ui-derived (`row`/`column`/`el`, `Background`/`Border`/`Font`/`Region`, `Input.*`) but renders to **inline-styled HTML on the server**. elm-ui builds the layout in the browser at runtime. | Follows from server-driven TEA (P2). |
...
- **Persistence & data:** `Std.Db` (SQLite/Postgres, typed `SqlValue`/`SqlField`
  ADTs, migrations, tenant-prefix SQL enforcement), `Std.Csv`, `Std.Config`
  (TOML/YAML/JSON decoders), `Std.Compression` (gzip/zstd).
- **Security:** `Std.Auth` (bcrypt + HS256/RS256 JWT cookies), `Sky.Core.Crypto`
  (sha/hmac/rsa/AEAD/entropy), `Sky.Core.Jwt`, `Sky.Core.Uuid`,
  `Sky.Core.Encoding`, `Sky.Core.Bytes`. Elm has no crypto in core (browser
  code would use SubtleCrypto via ports).
- **Money/precision:** `Std.Decimal` (arbitrary precision) + `Std.Money`
  (currency-typed on Decimal, ISO 4217). Elm core has only `Float`; there is
  no decimal/money type in core.
- **Servers & networking:** `Sky.Http.Server` (routes, middleware, cookies,
  streaming), `Sky.Http.Server.WebSocket` + `Sky.Core.WebSocket` (client),
  `Sky.Http.Server.Stream` (SSE / chunked). Elm cannot open a server socket.
- **Observability:** `Std.Log`, `Std.Trace`, auto-mounted `/_sky/console`,
...
- **Runtime services:** `Std.Cache` (LRU+TTL), `Std.Email` (Resend/SES/
  SendGrid/SMTP), `Std.Time` (IANA zones, calendar math), `Sky.Core.File`,
  `Sky.Core.Io`, `Sky.Core.System`, `Sky.Core.Process`, `Sky.Core.Path`,
  `Sky.Core.Regex` (built-in — Elm 0.19 removed `elm/regex` from the blessed
...
| Observability | `Debug.log` (dev-only) | `Std.Log`, `Std.Trace`, console, metrics, OTLP | Production operability |
...
- **R4** — RESOLVED. No `Never`-error task form exists; the `Task` surface
  (`crates/skyc/stdlib/Sky/Core/Task.ipe`) fixes every error slot to `Error`,
