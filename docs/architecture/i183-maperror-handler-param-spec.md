...
1. **Hand-written `oracle.meta` sha256 drift.** Attempt #2 hand-authored
   `tests/golden/m88_result_map_error_wildcard/oracle.meta` with a literal
...
- **Only fires for an inline `Expr::Lambda` handler at `args[0]`.** A
  **named-function** handler (`tag : String -> String`, the existing
  `l0114_map_error_arity1_accepted` fixture) is not an `Expr::Lambda`, so the
...
### Fixture: `tests/golden/m88_result_map_error_wildcard/Main.sky`
...
CARGO_TARGET_DIR=/tmp/i183-target cargo run -p refresh-oracle -- m88_result_map_error_wildcard
```
...
`tests/golden/m88_result_map_error_wildcard/expected_go.txt` contains exactly
...
| `tests/golden/m88_result_map_error_wildcard/Main.sky` | New fixture (above). |
| `tests/golden/m88_result_map_error_wildcard/{expected_go.txt,oracle.meta}` | Generated by `refresh-oracle`, NOT hand-written. |
