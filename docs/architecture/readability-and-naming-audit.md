...
| **CONTRACT-TOUCHING** | Crosses a crate's public API, OR is a stringly key (kernel qualifier `String`/`Db.Decode`/`idiv`, error code `SKY-L0119`, runtime symbol name, JSON wire field) OR is otherwise golden-oracle-observable. Can silently break correctness/soundness — **coordinate-with-tests**, do not rename in isolation. |
...
- **Error-code wire forms** — `Code::as_str()` → `"SKY-T0001"` etc. are
...
3. **H1d/H2d — `Code`-as-string forces non-exhaustive dispatch + a drifted
   ...
   (`SKY-P0016,P0017,T0014,L0114,L0115,L0116,L0117,L0119`), so
   `skyc explain SKY-L0117` returns `UnknownCode` today. **H2d must be fixed on
