...
| **CPS rewrites of non-tail list ops (a0b63e4e..222a4a25, Limitation #8)** | `map/filter/foldr/concat/take/append/range/zip/length/indexedMap/concatMap` + `Maybe.combine`/`Result.combine` rewritten to constant-stack CPS/accumulator `*Help` form, each with a `*Spec` stack-bound gate | soundness-hardening | soundness (unbounded stack = reachable overflow from well-typed code on 200k+ lists) | **partial/lack** — VERIFIED: `stdlib/Sky/Core/List.sky` still ships naive body recursion; ipê TCO (task #49) never fires on non-tail bodies → Rust stack overflow/abort | **YES, high priority** — mechanical 1:1 `.sky` port into `List.sky`/`Maybe.sky`/`Result.sky`; ipê TCO picks it up. Fold into task #49 or a new "stdlib stack-safety" slice, before example sweep signs off |
...
| **String.dropLeft/dropRight (861d425a, #544/#132)** | Elm-shaped rune-based | new-feature | completeness | **HAVE** — `String.sky:184-191`, rune-based, parity tests present | no |
...
| **insertFieldsReturning (18f58267, #586)** | `INSERT … RETURNING <projection>` decoded via `Std.Db.Decode` | new-feature | completeness | **lack** — Db un-ported at stdlib level | **YES**, bundled with eventual `Std.Db` port (not standalone); pair with the `SqlFragment` param-newtype hardening queued (task #61) so RETURNING projections can't reopen injection |
...
| **Property-based well-typed fuzzer (9e170314, `fuzz-well-typed.sh`)** | Generates random well-typed Sky programs; asserts `build && run` doesn't panic; 6 HM-valid templates, deterministic LCG seeding, ≥10k iters target | tooling/DX (soundness-verification infra) | soundness (the "real-world leg" of the three-leg no-panic proof) | **partial/lack** — ipê has WellTypedFuzzer reference (M7 gate) + golden oracle, no random-generator harness | **YES** — after 5-shape/CI-sweep parity (tasks #35/#37), before FFI. Cheap to port the 6 templates; deterministic-seed reproduction fits the log-and-re-read discipline |
...
| **sky fmt header-comment fix (889bf01f, #572)** | Formatter idempotence regression fix | tooling/DX | readability/correctness | **N/A** — ipê formatter not at this surface | no |
| **Std.Ui.dispatchTag sig + Haddock batches (64ea7e7d, ed89eda2, 93817c90, 2873eb79)** | Signature annotation + docstrings | tooling/DX | readability | **lack** (modules un-ported) | opportunistic, no standalone action |
...
2. **[SECURITY] withCsrf + Middleware module** (78faa349) → NEW task:
   "port `Sky.Http.Middleware` (withCors/withLogging/withBasicAuth/withRateLimit
...
3. **[SOUNDNESS] CPS stack-safety rewrites** (a0b63e4e..222a4a25) → fold into
   **task #49** (TCO) or a NEW "stdlib stack-safety" slice. Mechanical 1:1 `.sky`
...
10. **[COMPLETENESS] insertFieldsReturning** (18f58267) → adopt bundled with the
    `Std.Db` stdlib port; pair with the `SqlFragment` newtype (task #61).
