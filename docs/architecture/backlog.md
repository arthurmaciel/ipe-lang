...
- **#66-N — Negative / rejection fuzzer (the inverse of #66)** — the other half of "if it compiles, it works": #66 checks *good programs are accepted + run clean*; #66-N checks **broken programs are REJECTED** (false-acceptance is where soundness dies — an accepted ill-typed program → exit-0-cargo-fail or miscompile).
  - **First half — guaranteed-breaking mutation tier (🔨 IMPLEMENTING):** take a well-typed base (the #66 templates), inject a mutation that is ill-typed BY CONSTRUCTION (undefined field access `model.<typo>`, undefined var, unknown qualified member `String.<typo>`, forced type mismatch `String.length <int>`, wrong arity), and assert `skyc` REJECTS (exit ≠ 0) — and where the expected code is known, assert the right diagnostic CODE + that the span points at the mutation site (leverages the just-landed home-module span discriminant). A mutant skyc ACCEPTS is a real false-acceptance bug (a finding). Trap avoided: only GUARANTEED-breaking mutations (a random mutation may still be well-typed, so "mutant ⟹ reject" would false-alarm). Self-validates: all mutants rejected + the un-mutated base accepted.
