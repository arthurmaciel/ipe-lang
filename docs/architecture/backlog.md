...
- **ex37** `37-composite-live-shop`: ~~SKY-L0108 `Border.shadow`~~ **LANDED** — `BorderShadow` kernel wired across all 8 sites (kernel/constrain/lower/naming/pretty/emit_expr + runtime helper/render via `AttrStyle` box-shadow). Autonomously produced by a `progressive-development --once` run, **gate-green** (workspace test + clippy) and verified to clear the L0108. **Next blocker:** SKY-T0001 at `src/Main.sky:33` — `cart = []` expected `{ count : Int, name : String }`, found `Dict String String` (a record/type-inference issue, likely a fixture or Dict-literal-vs-record shape — investigate).
