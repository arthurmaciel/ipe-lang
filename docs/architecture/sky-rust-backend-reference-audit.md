...
| 15 | Crate-version single source + drift guard | `crate-specs.toml` embedded SSOT + `crate_specs_sync.rs` drift test | scattered version literals in `project.rs`; no drift test | **T+** | invalid-states + correctness | SSOT + drift test — **before-push (NEW task)** |
