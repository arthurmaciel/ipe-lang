...
| L23 | Single-module type-parameter anchoring keeps `Element msg`/`Attribute msg` from canonicaliser type-stripping | Ipe.Ui/Html (#76) | (3) soundness | **ADOPT** | Keep Ui `Element`/`Attribute` in one module; add wrapper fns in the parent so sub-modules call fns not ctors. Test: cross-module ctor import fails at compile time. **#76** |
| L24 | Phased kernel registration; ~160 members intentionally unbacked (`id=None`) as pure-Ipê | Ipe.Ui/Html (#76) | (5) COMPLETENESS, (2) correctness | **ADAPT** | Build a Pure-vs-Kernel registry-status tracker; audit all 160, implement kernels for the high-value 30-40 blocking examples, mark the rest explicitly. **#76** |
| L25 | 8-category kernel taxonomy makes incompleteness visible | Ipe.Ui/Html (#76) | (5) COMPLETENESS, (4) efficiency | **ADOPT** | Reorganize `sky_kernels/src/lib.rs` into 8 semantic groups with per-group expected counts + count-assert tests. Turns #76 into a structured audit. **#76** |
| L26 | Dual-path attribute compilation: pure-Ipê construction, kernel-backed CSS emission | Ipe.Ui/Html (#76) | (3) soundness, (4) efficiency, (2) correctness | **ADOPT** | Route `Attribute → kernel-call` for CSS emission during the Element tree walk; don't inline-string from ADT destructuring in Ipê. **#76** |
| L27 | Unbacked members should be a **type-level** fact: registry status `Kernel \| PureSky \| Todo`, missing = compile error (no `id=None` silent fallback) | Ipe.Ui/Html (#76) | (3) soundness (**MAKE INVALID STATES UNREPRESENTABLE**) | **ADOPT** | Add mandatory `status` field to the Ui/Html kernel registry; referenced-but-unregistered member = hard canon error. **#76** |
...
### P2 — #76 Ipe.Ui / Ipe.Html member breadth (160 unbacked canonicals)
