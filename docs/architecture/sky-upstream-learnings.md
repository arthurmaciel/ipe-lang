...
| L5 | `Error = Error ErrorKind ErrorInfo` emitted via standard tuple-ADT lowering; 2-field layout must be preserved | Error module | (2) correctness, (3) soundness | **ADOPT** | Register `Error` in canon QUALIFIERS; wire `ErrorKind`/`ErrorInfo` ctors with correct arity; codegen the explicit `Error` constructor. Validate line-by-line vs `Error.sky:33-34`. **#78** |
...
- **L30/L31** Enforce explicit `exposing` on every ported stdlib `.sky`; run `build_module_exports` before downstream import; per-module public-API test.
...
- **L7 / L15** Add `Stringify` (`SHOW`) obligation to `constrain.rs`; gate `errorToString`, `toString`, `Debug.toString`, `Log.*With`. Regression `SKY-T0014`.
