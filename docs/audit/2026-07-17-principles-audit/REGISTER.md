...
- **T1 FFI trust-boundary hardening** — parse-don't-validate type/path strings at decode (newtypes); no-egress-while-executing two-phase sandbox; stop mounting `~/.cargo` secrets; cache-root stops at `ipe.toml` + ownership check; hard-fail on missing timeout/prlimit caps. *(co-ffi-001, CLI-001/002/007/008, co-ffi-002/003/004)*
