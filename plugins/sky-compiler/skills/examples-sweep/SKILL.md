...
`scripts/equivalence-checks/examples-sweep.sh` — that for every in-scope example (`build_set` from
...
## EQUIVALENCE modes — DERIVED from shape, overrides on top

`equivalence_mode` (`lib/examples.sh`) derives the mode from `example_shape` so a new
example auto-classifies: cli→`stdout`, server→`body`, live→`scenario`, tui→`pty`,
webview/fyne/Go-FFI→`none`. `equivalence-classification.tsv` is **overrides-only** (a
...
2. **Relay the table + verdict** — quote the rendered BUILD·RUN·EQUIVALENCE table, the
...
3. **Triage RED rows.** A `skyc-fail`/`cargo-fail` is a real compiler/codegen gap
   ...
   `panic`/`hang`/`noserve` is a real runtime regression. A `DIFFER` (once EQUIVALENCE is
...
`lib/examples.sh` (the Go-FFI exclusion / `build_set` / `equivalence_mode` — SSOT),
`lib/checks.sh` (per-shape `exercise_*` + `PANIC_RE` + browser-stack probe), and
the two `equivalence_*.py` normalisers are the SINGLE SOURCE OF TRUTH. RUN exercises the
Rust binary; EQUIVALENCE (phased) exercises BOTH backends and compares.
