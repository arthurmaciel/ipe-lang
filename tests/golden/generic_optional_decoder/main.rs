    ...
    task_and_then(
        db_open("sqlite".to_string(), "sqlite::memory:".to_string()),
        {
            let __ipe_fn: Box<dyn Fn(Db) -> IpeTask<()> + Send + Sync + 'static> = Box::new(
                move |conn: Db| -> IpeTask<()> {
                    db_with_transaction(conn.clone(), {
                        let __ipe_fn: Box<dyn Fn(Db) -> IpeTask<()> + Send + Sync + 'static> =
                            Box::new(move |txconn: Db| -> IpeTask<()> {
                                ({
                                    let txconn = txconn.clone();
                                    {
                                        let __ipe_fn: Box<
                                            dyn Fn(IpeTask<i64>) -> IpeTask<()>
                                                + Send
                                                + Sync
                                                + 'static,
                                        > = Box::new(move |eta_0: IpeTask<i64>| -> IpeTask<()> {
                                            task_and_then(
                                                eta_0,
                                                ({
                                                    let txconn = txconn.clone();
