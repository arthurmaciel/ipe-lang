...
impl<T1: SkyStringify + std::fmt::Debug> SkyStringify for MainOpt<T1> {
    fn sky_show(&self) -> String {
        match self {
            MainOpt::Som(p0) => format!("Som {}", (&sky_runtime::stringify::Wrap(p0)).dispatch()),
...
pub fn main_f(oo: MainOpt<MainOpt<i64>>) -> i64 {
    match oo {
        MainOpt::Som(MainOpt::Som(x)) => x,
        MainOpt::Som(MainOpt::Non) => 7,
        MainOpt::Non => 3,
    }
}
pub fn sky_main() -> SkyTask<()> {
    log_println(string_from_int(((main_f(MainOpt::Som(MainOpt::Som(42))) + main_f(MainOpt::Som(MainOpt::Non))) + main_f(MainOpt::Non))))
}
