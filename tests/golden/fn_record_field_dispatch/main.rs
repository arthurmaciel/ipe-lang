...
pub struct RecAddMul {
    add: ::std::sync::Arc<dyn Fn(i64) -> i64 + Send + Sync + 'static>,
    mul: ::std::sync::Arc<dyn Fn(i64) -> i64 + Send + Sync + 'static>,
}
impl Clone for RecAddMul {
    fn clone(&self) -> Self {
        Self {
            add: self.add.clone(),
            mul: self.mul.clone(),
