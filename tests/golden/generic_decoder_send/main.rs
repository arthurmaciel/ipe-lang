...
pub fn main_with_default<T1: 'static + Send + Sync + Clone>(
    fallback: T1,
    dec: Decoder<T1>,
) -> Decoder<T1> {
    ...
    decode_one_of(vec![
        dec,
        decode_succeed::<IpeError, _>({ let __ipe_succeed = fallback; Box::new(move || __ipe_succeed.clone()) }),
    ])
}
pub fn ipe_main() -> IpeTask<()> {
    ...
    ({
        ...
        ({
            ...
            ({
                let r1 = decode_from_json_string(intDec.clone(), "99".to_string());
                ({
                    let r2 = decode_from_json_string(intDec, "\"bad\"".to_string());
                    ({
                        let r3 = decode_from_json_string(strDec.clone(), "\"hello\"".to_string());
                        ({
