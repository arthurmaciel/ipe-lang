...
pub fn ipe_main() -> IpeTask<()> {
    log_println(string_from_int(maybe_with_default(0, ipe_maybe_and_then(ipe_maybe_map(IpeMaybe::Just(5), { let __sky_fn: Box<dyn Fn(i64) -> i64 + Send + Sync + 'static> = Box::new(move |x: i64| -> i64 { (x * 10) }); __sky_fn }), { let __sky_fn: Box<dyn Fn(i64) -> IpeMaybe<i64> + Send + Sync + 'static> = Box::new(move |x: i64| -> IpeMaybe<i64> { IpeMaybe::Just((x + 1)) }); __sky_fn }))))
}
