...
pub struct RecCountFormatLabelOnChange {
    count: i64,
    format: ::std::sync::Arc<dyn Fn(i64) -> String + Send + Sync + 'static>,
    label: String,
    onChange: ::std::sync::Arc<dyn Fn(String) -> i64 + Send + Sync + 'static>,
}
impl Clone for RecCountFormatLabelOnChange {
    fn clone(&self) -> Self {
        Self {
...
impl IpeStringify for RecCountFormatLabelOnChange {
...
pub fn main_apply(c: RecCountFormatLabelOnChange) -> String {
    format!(
        "{}{}",
        (c.clone()).label.clone(),
        format!(
...
pub fn ipe_main() -> IpeTask<()> {
    io_println(crate::main_apply(RecCountFormatLabelOnChange {
        ...
        label: "len".to_string(),
        onChange: {
            let __ipe_fn: ::std::sync::Arc<dyn Fn(String) -> i64 + Send + Sync + 'static> =
