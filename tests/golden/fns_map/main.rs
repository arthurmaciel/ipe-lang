...
pub fn sky_main() -> SkyTask<()> {
    log_println(string_from_int(list_foldl({ let __sky_fn: Box<dyn Fn(i64, i64) -> i64 + Send + Sync + 'static> = Box::new(move |a: i64, x: i64| -> i64 { (a + x) }); __sky_fn }, 0, list_map_consume({ let __sky_fn: Box<dyn Fn(i64) -> i64 + Send + Sync + 'static> = Box::new(move |x: i64| -> i64 { (x * 2) }); __sky_fn }, vec![1, 2, 3]))))
}
