...
impl SkyStringify for MainTree {
    fn sky_show(&self) -> String {
        match self {
            MainTree::Leaf => "Leaf".to_string(),
            MainTree::Node(p0, p1, p2) => format!("Node {} {} {}", (&sky_runtime::stringify::Wrap(p0)).dispatch(), (&sky_runtime::stringify::Wrap(p1)).dispatch(), (&sky_runtime::stringify::Wrap(p2)).dispatch()),
        }
    }
}
