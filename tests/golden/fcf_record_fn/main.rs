...
pub struct RecShouldRetry {
    shouldRetry: ::std::sync::Arc<dyn Fn(i64) -> i64 + Send + Sync + 'static>,
}
impl Clone for RecShouldRetry {
    fn clone(&self) -> Self {
        Self {
            shouldRetry: self.shouldRetry.clone(),
        }
    }
}
impl IpeStringify for RecShouldRetry {
    fn ipe_show(&self) -> String {
        format!("{{{}}}", "<fn>")
    }
}
...
pub fn main_wrap(
    f: Box<dyn Fn(i64) -> i64 + Send + Sync + 'static>,
) -> Box<dyn Fn(i64) -> i64 + Send + Sync + 'static> {
    let _ipe_recursion_guard = crate::recursion_guard();
    {
        let __ipe_fn: Box<dyn Fn(i64) -> i64 + Send + Sync + 'static> =
            Box::new(move |n: i64| -> i64 { ((f)(n) + 1) });
        __ipe_fn
    }
}
...
pub fn main_guard() -> RecShouldRetry {
    let _ipe_recursion_guard = crate::recursion_guard();
    RecShouldRetry {
        shouldRetry: {
            let __ipe_fn: ::std::sync::Arc<dyn Fn(i64) -> i64 + Send + Sync + 'static> =
