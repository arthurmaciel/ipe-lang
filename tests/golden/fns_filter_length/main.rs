...
pub fn sky_main() -> SkyTask<()> {
    log_println(string_from_int(list_length(list_filter({ let __sky_fn: Box<dyn Fn(i64) -> bool + Send + Sync + 'static> = Box::new(move |x: i64| -> bool { (x > 2) }); __sky_fn }, vec![1, 2, 3, 4]))))
}
