            ...
            ({
                ...
                ({
                    ...
                    ({
                        ...
                        ({
                            let got = ipe_maybe_map(maybeG, {
                                let __ipe_fn: Box<
                                    dyn Fn(Box<dyn Fn(i64) -> i64 + Send + Sync + 'static>) -> i64
                                        + Send
                                        + Sync
                                        + 'static,
                                > = Box::new(
                                    move |fn_: Box<dyn Fn(i64) -> i64 + Send + Sync + 'static>| -> i64 {
                                        (fn_)(100)
                                    },
                                );
                                __ipe_fn
                            ...
                            ({
                                ...
                                io_println(string_from_int(
                                    ((applied + list_sum(mapped)) + maybe_with_default(0, got)),
                                ))
                            })
                        })
                    })
