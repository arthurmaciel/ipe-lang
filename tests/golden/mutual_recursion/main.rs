...
impl SkyStringify for MainEven {
    fn sky_show(&self) -> String {
        match self {
            MainEven::EZero => "EZero".to_string(),
            MainEven::ESucc(p0) => format!("ESucc {}", (&sky_runtime::stringify::Wrap(p0)).dispatch()),
        }
    }
}
#[derive(Clone, Debug, PartialEq)]
...
impl SkyStringify for MainOdd {
    fn sky_show(&self) -> String {
        match self {
            MainOdd::OSucc(p0) => format!("OSucc {}", (&sky_runtime::stringify::Wrap(p0)).dispatch()),
        }
    }
}
