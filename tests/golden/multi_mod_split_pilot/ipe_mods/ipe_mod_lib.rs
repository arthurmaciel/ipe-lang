                                ...
                                task_and_then(
                                    ...
                                    Box::new(move |_| {
                                        task_and_then(
                                            ...
                                            Box::new(move |_| {
                                                task_and_then(db_exec_params(txconn.clone().clone(), "INSERT INTO widgets (name, qty) VALUES (?, ?)".to_string(), (vec![MainSqlValue::SqlString("cog".to_string()), MainSqlValue::SqlInt(2)]).into_iter().map(::core::convert::Into::into).collect::<Vec<ipe_runtime::db::SqlParam>>()), Box::new(move |_| { task_map({ let __ipe_fn: Box<dyn Fn(Vec<HashMap<String, String>>) -> i64 + Send + Sync + 'static> = Box::new(move |rows: Vec<HashMap<String, String>>| -> i64 { list_length(rows) }); __ipe_fn }, db_query_params(txconn.clone(), "SELECT name, qty FROM widgets".to_string(), (Vec::<MainSqlValue>::new()).into_iter().map(::core::convert::Into::into).collect::<Vec<ipe_runtime::db::SqlParam>>())) }))
                                            }),
                                        )
                                    }),
                                )
