...
pub struct RecInnerName {
    inner: RecOpTag,
    name: String,
}
impl Clone for RecInnerName {
    fn clone(&self) -> Self {
        Self {
...
impl IpeStringify for RecInnerName {
...
pub struct RecOpTag {
    op: ::std::sync::Arc<dyn Fn(i64) -> i64 + Send + Sync + 'static>,
    tag: i64,
}
impl Clone for RecOpTag {
    fn clone(&self) -> Self {
        Self {
            op: self.op.clone(),
            tag: self.tag.clone(),
...
pub fn main_outer() -> RecInnerName {
...
pub fn main_run(o: RecInnerName, x: i64) -> i64 {
    let _ipe_recursion_guard = crate::recursion_guard();
    ({
        let dup = o.clone();
