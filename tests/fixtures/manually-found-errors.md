...
18-job-queue    Gross error: error[E0507]: cannot move out of `insertRow`, a captured variable in an `Fn` closure
   --> src/main.rs:424:682
    |
424 | ...et insertRow = { let __ipe_fn: ::std::sync::Arc<dyn Fn(Db, i64) -...<Vec<sky_runtime::db::SqlParam>>()) }); __ipe_fn }; ({ let writeAll = { let __ipe_fn: Box<dyn Fn(Db) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |db: Db| -> SkyTask<i64> { (({ let db = db.clone(); { let __ipe_fn: Box<dyn Fn(SkyTask<i64>) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<i64>| -> SkyTask<i64> { task_and_then(eta_0, ({ let db = db.clone(); ({ let insertRow = insertRow.cl...
...
help: consider cloning the value before moving it into the closure
    |
424 ~     ({ let insertRow = { let __ipe_fn: ::std::sync::Arc<dyn Fn(Db, i64) -> SkyTask<i64> + Send + Sync + 'static> = ::std::sync::Arc::new(move |db: Db, ts: i64| -> SkyTask<i64> { db_exec_params(db.clone(), "INSERT INTO snapshots (ok, failed, total, ts) VALUES (?, ?, ?, ?)".to_string(), (vec![okCount, failCount, total, ts]).into_iter().map(::core::convert::Into::into).collect::<Vec<sky_runtime::db::SqlParam>>()) }); __ipe_fn }; ({ let writeAll = { let __ipe_fn: Box<dyn Fn(Db) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |db: Db| -> SkyTask<i64> { (({ let db = db.clone(); { let value = insertRow.clone();
425 ~     let __ipe_fn: Box<dyn Fn(SkyTask<i64>) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<i64>| -> SkyTask<i64> { task_and_then(eta_0, ({ let db = db.clone(); ({ let insertRow = value.clone(); { let __ipe_fn: Box<dyn Fn(i64) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |ts: i64| -> SkyTask<i64> { (insertRow)(db.clone(), ts) }); __ipe_fn } }) })) }); __ipe_fn } }))(({ let eta_0: SkyTask<i64> = db_exec_raw(db.clone(), main_create_snapshot_table()); task_and_then(eta_0, { let __ipe_fn: Box<dyn Fn(i64) -> SkyTask<i64> + Send + Sync + 'static> = Box::new(move |arg_5: i64| -> SkyTask<i64> { time_now(()) }); __ipe_fn }) })) }); __ipe_fn }; ({ let successMsg = format!("{}{}", "Snapshot saved (ok=".to_string(), format!("{}{}", string_from_int(okCount), format!("{}{}", " fail=".to_string(), format!("{}{}", string_from_int(failCount), ")".to_string())))); (({ let cap_0 = "snapshot.save".to_string(); { let __ipe_fn: Box<dyn Fn(SkyTask<String>) -> SkyTask<String> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<String>| -> SkyTask<String> { main_with_error_reporting(cap_0.clone(), eta_0) }); __ipe_fn } }))(({ let eta_0: SkyTask<i64> = ({ let eta_0: SkyTask<Db> = db_open("sqlite".to_string(), main_db_path()); task_and_then(eta_0, writeAll) }); task_map({ let __ipe_fn: Box<dyn Fn(i64) -> String + Send + Sync + 'static> = Box::new(move |arg_4: i64| -> String { successMsg.clone() }); __ipe_fn }, eta_0) })) }) }) })
    |

error[E0507]: cannot move out of `selectRecent`, a captured variable in an `Fn` closure
   --> src/main.rs:427:790
    |
427 | ...et selectRecent = { let __ipe_fn: ::std::sync::Arc<dyn Fn(Db) -> Ipê...<Vec<sky_runtime::db::SqlParam>>()) }); __ipe_fn }; ({ let readAll = { let __ipe_fn: Box<dyn Fn(Db) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = Box::new(move |db: Db| -> SkyTask<Vec<HashMap<String, String>>> { (({ let db = db.clone(); { let __ipe_fn: Box<dyn Fn(SkyTask<i64>) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<i64>| -> SkyTask<Vec<HashMap<String, String>>> { task_and_then(eta_0, ({ let db = db.clone(); ({ let selectRecent = selectRecent.cl...
...
help: consider cloning the value before moving it into the closure
    |
427 ~     ({ let selectRecent = { let __ipe_fn: ::std::sync::Arc<dyn Fn(Db) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = ::std::sync::Arc::new(move |db: Db| -> SkyTask<Vec<HashMap<String, String>>> { db_query_params(db.clone(), "SELECT ok, failed, total, ts FROM snapshots ORDER BY ts DESC LIMIT 5".to_string(), (Vec::<MainSqlValue>::new()).into_iter().map(::core::convert::Into::into).collect::<Vec<sky_runtime::db::SqlParam>>()) }); __ipe_fn }; ({ let readAll = { let __ipe_fn: Box<dyn Fn(Db) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = Box::new(move |db: Db| -> SkyTask<Vec<HashMap<String, String>>> { (({ let db = db.clone(); { let value = selectRecent.clone();
428 ~     let __ipe_fn: Box<dyn Fn(SkyTask<i64>) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<i64>| -> SkyTask<Vec<HashMap<String, String>>> { task_and_then(eta_0, ({ let db = db.clone(); ({ let selectRecent = value.clone(); { let __ipe_fn: Box<dyn Fn(i64) -> SkyTask<Vec<HashMap<String, String>>> + Send + Sync + 'static> = Box::new(move |arg_6: i64| -> SkyTask<Vec<HashMap<String, String>>> { (selectRecent)(db.clone()) }); __ipe_fn } }) })) }); __ipe_fn } }))(db_exec_raw(db.clone(), main_create_snapshot_table())) }); __ipe_fn }; (({ let cap_0 = "snapshot.load".to_string(); { let __ipe_fn: Box<dyn Fn(SkyTask<Vec<RecFailedOkTotalTs>>) -> SkyTask<Vec<RecFailedOkTotalTs>> + Send + Sync + 'static> = Box::new(move |eta_0: SkyTask<Vec<RecFailedOkTotalTs>>| -> SkyTask<Vec<RecFailedOkTotalTs>> { main_with_error_reporting(cap_0.clone(), eta_0) }); __ipe_fn } }))(({ let eta_0: SkyTask<Vec<HashMap<String, String>>> = ({ let eta_0: SkyTask<Db> = db_open("sqlite".to_string(), main_db_path()); task_and_then(eta_0, readAll) }); task_map({ let __ipe_fn: Box<dyn Fn(Vec<HashMap<String, String>>) -> Vec<RecFailedOkTotalTs> + Send + Sync + 'static> = Box::new(move |rows: Vec<HashMap<String, String>>| -> Vec<RecFailedOkTotalTs> { list_map_consume({ let __ipe_fn: Box<dyn Fn(HashMap<String, String>) -> RecFailedOkTotalTs + Send + Sync + 'static> = Box::new(main_parse_snapshot); __ipe_fn }, rows) }); __ipe_fn }, eta_0) })) }) })
    |
...
2026/07/15 23:39:36 [sky.live] session store: memory (ttl=30m0s)
[sky.console] inline console mounted as Ipe.Live sub-app at /_ipe/console mode=dev-open
