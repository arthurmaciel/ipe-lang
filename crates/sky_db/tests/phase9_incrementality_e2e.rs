...
fn body_edit_reexecutes_only_the_edited_module_file() {
    ...
    let warm = sky_db::emit_manifest(&db, root, main, config).expect("warm build");
    ...
    let spine_before = sky_db::emit_spine_file(&db, root, main, config).expect("spine before");
    assert!(
        warm.files
            .keys()
    ...
    let _rebuilt = sky_db::emit_manifest(&db, root, main, config).expect("incremental rebuild");
