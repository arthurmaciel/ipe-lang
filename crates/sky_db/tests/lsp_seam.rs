...
use sky_db::{ImportResolution, ModuleOrigin, SkyDatabase, SourceFile, SourceRoot};
...
fn lsp_shaped_buffer_edit_drives_diagnostics_and_navigation_in_memory() {
    ...
    let parsed = sky_db::parse(&db, entry).expect("buffer must parse");
    ...
    assert!(
        sky_db::set_text_if_changed(&mut db, entry, ENTRY_TYPE_ERROR),
    ...
    // text.
    assert!(sky_db::set_text_if_changed(&mut db, entry, ENTRY_OK));
    sky_db::typecheck(&db, root, entry)
        .expect("buffer fixed by a second keystroke must type-check clean again");
    assert!(sky_db::parse(&db, entry).is_ok());
    assert!(sky_db::resolve_imports(&db, root, entry).is_ok());
}
...
fn lsp_diagnostics_query_is_cancelled_by_the_next_keystroke_and_converges_to_latest_state() {
    ...
    let worker = thread::spawn(move || {
        salsa::Cancelled::catch(AssertUnwindSafe(|| {
            sky_db::typecheck(&db_worker, root, entry)
        }))
    });
