...
fn program_metadata_memoized_coarse_floor() {
    ...
    let meta = sky_db::program_metadata(&db, root, b).expect("trivial int program must lower");
    assert!(
    ...
    log.clear();
    assert!(sky_db::program_metadata(&db, root, b).is_ok());
    assert_eq!(
...
fn program_metadata_excludes_unreached_function() {
    ...
    let program = sky_db::lower_program(&db, root, entry).expect("must lower");
    let meta = sky_db::program_metadata(&db, root, entry).expect("must compute metadata");
...
fn find_func_id(db: &sky_db::SkyDatabase, module: &sky_ir::Module, name: &str) -> sky_ir::FuncId {
    let interner = sky_db::Db::interner(db).lock();
