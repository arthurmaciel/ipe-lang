...
//!   emits SKY-L0125 for NonClone captures outside callee position.
...
/// NonClone in CALLEE position.  T3 rule 3: bare `Var` (no clone, no L0125).
...
// ── F6 — gate: fn-typed capture forwarded (non-callee) → SKY-L0125 ───────────

/// `compose f = \x -> applyTwice f x` — `f : Int -> Int` in non-callee
/// position.  T3 must surface SKY-L0125 at skyc time (never cargo-fail).
#[test]
fn f6_capture_fn_forwarded_gate_l0125() {
    assert_skyc_gate(
        "i121_capture_fn_forwarded",
        "i121_capture_fn_forwarded_gate",
        sky_diagnostics::SKY_L0125,
    );
}
...
// ── F10 — generic curried fn → SKY-L0125 (T5 deferred) ───────────────────────
...
/// `SKY-L0125` if the clone bound is still absent).  For now the expected
