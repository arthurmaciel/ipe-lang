//! Milestone-1 record gate: `skyc` must emit `main.rs` byte-identical to the
...
fn end_to_end_builds_and_prints_forty_two() {
    ...
    let outcome = support::build_and_run_emitted("m1_records", &out);
    support::assert_go_parity(
        "m1_records",
        &repo_root().join("tests").join("golden").join("m1_records"),
        &outcome.stdout,
    );
