...
fn end_to_end_builds_and_prints_forty_six() {
    ...
    let root = repo_root();
    let entry = example_entry(&root);
    let out = std::env::temp_dir().join("skyc_m3b1_tuple_e2e");
    let _ = std::fs::remove_dir_all(&out);
    ...
    let outcome = support::build_and_run_emitted("m3b1_tuple", &out);
    support::assert_go_parity(
        "m3b1_tuple",
        &repo_root().join("tests").join("golden").join("m3b1_tuple"),
        &outcome.stdout,
    );
