...
fn i225_depth0_no_overclone_skyc_accepts_lean() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i225_depth0_no_overclone: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i225_depth0_no_overclone: {:?}",
        built.err()
    );
...
fn i225_depth0_no_overclone_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i225_depth0_no_overclone", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i225_depth0_no_overclone must exit 0; stdout: {:?}",
        outcome.stdout
    );
