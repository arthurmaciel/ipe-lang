...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i193_asymmetric_arms_cloneok")
        .join("Main.sky")
}
...
fn i193_skyc_accepts_asymmetric_arms() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i193_asymmetric_arms_cloneok: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i193_asymmetric_arms_cloneok: {:?}",
        built.err()
    );
...
fn i193_idempotent() {
    ...
    assert_eq!(
        main1, main2,
        "two independent builds of i193_asymmetric_arms_cloneok must produce \
...
fn i193_cargo_builds_and_runs() {
    ...
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i193_asymmetric_arms_cloneok must exit 0 (no E0382); stdout: {:?}",
        outcome.stdout
    );
