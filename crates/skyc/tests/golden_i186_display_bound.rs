...
fn i186_skyc_accepts_and_bounds_fn_display() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i186_display_bound: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i186_display_bound: {:?}",
        built.err()
    );
...
fn i186_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i186_display_bound", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i186_display_bound binary must exit 0 (no E0277); got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
    assert_eq!(
