...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i218_clone_relay_intermediate_eta")
        .join("Main.sky")
}
...
fn i218_clone_relay_skyc_accepts() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i218_clone_relay_intermediate_eta: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i218_clone_relay_intermediate_eta: {:?}",
        built.err()
    );
...
fn i218_clone_relay_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i218_clone_relay_intermediate_eta", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i218_clone_relay_intermediate_eta must exit 0 (no E0507); stdout: {:?}",
        outcome.stdout
    );
