...
//! … ; _ -> …`). Coverage is proven UPSTREAM by the SKY-T0010 usefulness check
...
//! or record product `case` stays fail-closed as SKY-L0115 (the m3b1/m3b2 gates).
//!
...
/// Fast gate: a multi-arm LITERAL-tuple `case` builds cleanly — the SKY-L0115
...
fn multi_arm_tuple_case_builds() {
    ...
    assert!(
        built.is_ok(),
        "multi-arm tuple `case` must build (SKY-L0115 close); got {:?}",
        built.err()
    );
}
...
fn multi_arm_tuple_case_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i_tuple_multiarm_case", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
