...
fn i193_taskseq_skyc_accepts() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i193_taskseq_reuse: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i193_taskseq_reuse: {:?}",
        built.err()
    );
...
fn i193_taskseq_cargo_builds_and_runs() {
    ...
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i193_taskseq_reuse must exit 0 (no E0382); stdout: {:?}",
        outcome.stdout
    );
