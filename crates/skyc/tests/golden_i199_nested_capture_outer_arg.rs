...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i199_nested_capture_outer_arg")
        .join("Main.sky")
}
...
fn i199_skyc_accepts_and_hoists() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i199_nested_capture_outer_arg: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i199_nested_capture_outer_arg: {:?}",
        built.err()
    );
...
fn i199_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i199_nested_capture_outer_arg", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i199_nested_capture_outer_arg must exit 0 (no E0507/E0382); stdout: {:?}",
        outcome.stdout
    );
