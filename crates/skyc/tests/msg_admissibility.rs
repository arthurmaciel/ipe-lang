...
/// `Sky.Live` app: Msg variant carries a `Cmd`. Must be rejected with SKY-L0122.
...
/// code is L0114 not L0122 because function-in-constructor is a separate,
/// earlier constraint.
...
/// `Sky.Tui` app: Msg variant carries a `Cmd`. Must be rejected with SKY-L0122.
...
fn live_msg_with_cmd_is_rejected() -> Result<(), BoxError> {
    assert_rejected_with("live_cmd_msg", LIVE_CMD_MSG, "SKY-L0122")
}
...
fn live_lambda_update_with_cmd_msg_is_rejected() -> Result<(), BoxError> {
    assert_rejected_with("live_lambda_update_cmd_msg", LIVE_LAMBDA_UPDATE_CMD_MSG, "SKY-L0122")
}

#[test]
fn tui_msg_with_cmd_is_rejected() -> Result<(), BoxError> {
    assert_rejected_with("tui_cmd_msg", TUI_CMD_MSG, "SKY-L0122")
}
