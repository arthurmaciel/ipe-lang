...
fn lazy_emit_seal_skyc_cargo_and_run_zero() {
    ...
    let root = repo_root();
    let dir = root.join("tests").join("golden").join("i146_lazy_emit_seal");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i146_lazy_emit_seal (SKY-I0001 must be gone): {:?}",
        built.err()
    );
    ...
    let outcome = support::build_and_run_emitted("i146_lazy_emit_seal", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
