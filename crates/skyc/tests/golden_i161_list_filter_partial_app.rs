...
fn list_filter_partial_app_compiles() {
    ...
    let entry = root
        .join("tests")
        .join("golden")
        .join("i161_list_filter_partial_app")
        .join("Main.sky");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i161_list_filter_partial_app: {:?}",
        built.err()
    );
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i161_list_filter_partial_app (E2E leg): {:?}",
        built.err()
    );

    let outcome =
        support::build_and_run_emitted("i161_list_filter_partial_app", &e2e_out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
