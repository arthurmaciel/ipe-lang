...
fn cli_program_skyc_cargo_and_run_zero() {
    ...
    let root = repo_root();
    let dir = root.join("tests").join("golden").join("i111_cli_program_seal");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i111_cli_program_seal: {:?}",
        built.err()
    );
    ...
    let outcome = support::build_and_run_emitted("i111_cli_program_seal", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
