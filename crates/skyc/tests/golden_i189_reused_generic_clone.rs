...
fn golden_dir(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i189_reused_generic_clone")
}
...
fn i189_skyc_accepts_and_clones_reused_generic() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i189_reused_generic_clone: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i189_reused_generic_clone: {:?}",
        built.err()
    );
...
fn i189_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i189_reused_generic_clone", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i189_reused_generic_clone binary must exit 0 (no E0382); got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
    assert_eq!(
    ...
    support::assert_go_parity("i189_reused_generic_clone", &gdir, &outcome.stdout);
}
