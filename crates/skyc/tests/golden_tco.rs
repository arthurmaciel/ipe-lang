...
fn tco_count_runs_to_completion_constant_stack() {
    ...
    let out = support::build_and_run_stack_limited("tco_count", &dir, 512);
    assert_eq!(
        out.exit_code,
        Some(0),
...
fn tco_arg_swap_uses_temporaries_first() {
    ...
    let out = support::build_and_run_emitted("tco_swap", &dir);
...
fn tco_swap_matches_go_oracle() {
    ...
    let out = support::build_and_run_emitted("tco_swap", &dir);
    assert_eq!(out.exit_code, Some(0));
    support::assert_go_parity("tco_swap", &golden_dir(&root, "tco_swap"), &out.stdout);
}
