...
//!
//! Root cause (see `docs/architecture/class4-pattern-lowering-fix-spec-2026-07-09.md`,
...
fn m102_local_type_shadows_dep_fails_n0012() {
    let err = try_build("m102_local_type_shadows_dep").expect_err(
        "m102_local_type_shadows_dep must fail (local `type Color` shadows the imported Dep.Color)",
    );
    assert!(
