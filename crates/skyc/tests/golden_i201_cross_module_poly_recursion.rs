...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i201_cross_module_poly_recursion")
        .join("src")
        .join("Main.sky")
}
...
fn i201_skyc_accepts_and_emits_clone_bounded_generic() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i201_cross_module_poly_recursion: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i201_cross_module_poly_recursion (no SKY-L0102): {:?}",
        built.err()
    );
...
fn i201_cargo_builds_and_runs() {
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i201_cross_module_poly_recursion: {:?}",
        built.err()
    );

    let outcome = support::build_and_run_emitted("i201_cross_module_poly_recursion", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i201_cross_module_poly_recursion binary must exit 0 (no SKY-L0102, no cargo error); \
         got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
