...
fn any_ctor_payload_skyc_and_cargo_zero() {
    let root = repo_root();
    let entry = fixture_entry(&root, "l0102_any_ctor_payload");
    ...
    let outcome = support::build_and_run_emitted("l0102_any_ctor_payload", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
...
fn any_ctor_payload_fail_closed() {
    let root = repo_root();
    let entry = fixture_entry(&root, "l0102_any_ctor_fail_closed");
...
fn ctor_span_attr_dep_module() {
    let root = repo_root();
    let entry = fixture_src_entry(&root, "l0102_ctor_span_attr");
