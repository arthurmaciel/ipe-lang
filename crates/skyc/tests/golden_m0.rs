...
fn emits_byte_identical_main_rs_and_vendors_runtime() {
    let root = repo_root();
    let entry = root.join("tests").join("golden").join("m0").join("Main.sky");
    ...
    assert_eq!(emitted.ok(), want.ok(), "emitted main.rs must equal the golden byte-for-byte");

    assert!(
        out.join("src").join("sky_runtime").join("core.rs").is_file(),
...
fn end_to_end_builds_and_prints_one() {
    ...
    let status = Command::new("cargo").arg("build").current_dir(&out).status();
    assert!(matches!(&status, Ok(s) if s.success()), "emitted project must build: {status:?}");

    let bin = out.join("target").join("debug").join("sky-app");
    let output = Command::new(&bin).output();
    assert!(output.is_ok(), "emitted binary must run: {:?}", output.as_ref().err());
