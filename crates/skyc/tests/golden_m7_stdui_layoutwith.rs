...
//!
//! The golden compiles `tests/golden/m7_stdui_layoutwith/Main.sky` through
...
//! (`tests/golden/m7_stdui_layoutwith/oracle.meta` + `expected_go.txt`).
...
//! `tests/golden/m7_stdui_layoutwith/sanctioned.divergence`.
//!
//! ## What is tested
//!
...
/// Compile / build / run `tests/golden/m7_stdui_layoutwith/Main.sky` and
...
fn build_run_layoutwith() -> (PathBuf, support::RunOutcome) {
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for m7_stdui_layoutwith: {:?}",
        built.err()
    );

    let outcome = support::build_and_run_emitted("m7_stdui_layoutwith", &out);
    (dir, outcome)
}
...
fn layoutwith_inline_cfg_applies_wrapper_and_root_attrs() {
    ...
    assert!(
        ...
        "m7_stdui_layoutwith: wrapperAttrs padding must be present in output\n--- actual ---\n{html}"
    );
    // rootAttrs = [ Ui.spacing 6 ] must appear on the inner root div.
    assert!(
        html.contains("gap:6px"),
        "m7_stdui_layoutwith: rootAttrs spacing must be present in output\n--- actual ---\n{html}"
    );
    // Font.bold on the el must be present.
    assert!(
        html.contains("font-weight:700"),
        "m7_stdui_layoutwith: Font.bold must render font-weight:700\n--- actual ---\n{html}"
    );
    // The text node must be present.
    assert!(
        html.contains("lw"),
        "m7_stdui_layoutwith: text content 'lw' must be present\n--- actual ---\n{html}"
    );

    support::assert_go_parity("m7_stdui_layoutwith", &dir, html);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "m7_stdui_layoutwith: must exit 0"
    );
}
