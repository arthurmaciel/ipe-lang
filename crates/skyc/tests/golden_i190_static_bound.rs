...
//! `<T1: Clone>`.
//!
//! This is EXACTLY the `Std.Live.Head.link` shape (the two examples
...
fn i190_skyc_accepts_and_bounds_fn_static() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i190_static_bound: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i190_static_bound: {:?}",
        built.err()
    );
...
fn i190_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i190_static_bound", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i190_static_bound binary must exit 0 (no E0310); got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
    assert_eq!(
