//! #117 seal — all 8 `Std.Ui.Region` kernel members exercised inside
...
fn region_all_members_skyc_and_cargo_zero() {
    ...
    let root = repo_root();
    let dir = root.join("tests").join("golden").join("i117_region_seal");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i117_region_seal: {:?}",
        built.err()
    );

    // Verify the emitted source calls the Region helpers.
    ...
    let outcome = support::build_and_run_emitted("i117_region_seal", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
