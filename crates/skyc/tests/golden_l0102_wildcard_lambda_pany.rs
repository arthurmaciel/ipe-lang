...
fn wildcard_lambda_pany_skyc_cargo_and_run_zero() {
    ...
    let dir = root
        .join("tests")
        .join("golden")
        .join("l0102_wildcard_lambda_pany");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for l0102_wildcard_lambda_pany (was: SKY-L0102 Polymorphism): {:?}",
        built.err()
    );

    // cargo-0 ∧ run: the emitted binary must build, run, and produce the
    // expected output showing all three shapes executed correctly.
    let outcome = support::build_and_run_emitted("l0102_wildcard_lambda_pany", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
