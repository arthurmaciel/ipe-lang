...
fn end_to_end_builds_and_prints_forty_two() {
    ...
    let root = repo_root();
    let entry = example_entry(&root);
    let out = std::env::temp_dir().join("skyc_m2b_parametric_aliases_e2e");
    let _ = std::fs::remove_dir_all(&out);
    ...
    let outcome = support::build_and_run_emitted("m2b_parametric_aliases", &out);
    support::assert_go_parity(
        "m2b_parametric_aliases",
        &repo_root()
            .join("tests")
            .join("golden")
            .join("m2b_parametric_aliases"),
