...
fn record_ctor_end_to_end_field_order() {
    ...
    let outcome = support::build_and_run_emitted("m82_record_ctor", &out);
    assert_eq!(
...
fn seal_fn_field_alias_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("m82_record_ctor_fn_field", &out);
    assert_eq!(
        outcome.stdout, "ok\n",
...
fn seal_opaque_field_alias_emits_no_struct() {
    ...
    let entry = root
        .join("tests")
        .join("golden")
        .join("m82_record_ctor_opaque_field")
        .join("Main.sky");
...
fn seal_opaque_field_alias_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("m82_record_ctor_opaque_field", &out);
    assert_eq!(
        outcome.stdout, "ok\n",
...
fn seal_opaque_field_used_as_ctor_fails_closed() {
    ...
    let entry = root
        .join("tests")
        .join("golden")
        .join("m82_record_ctor_opaque_ctor")
        .join("Main.sky");
