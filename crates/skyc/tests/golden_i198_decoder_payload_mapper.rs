...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i198_decoder_payload_mapper")
        .join("Main.sky")
}
...
fn i198_skyc_accepts_and_renders_send_only_fnonce_param() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i198_decoder_payload_mapper: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i198_decoder_payload_mapper: {:?}",
        built.err()
    );
...
fn i198_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i198_decoder_payload_mapper", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i198_decoder_payload_mapper binary must exit 0 (no E0308/E0277); got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
