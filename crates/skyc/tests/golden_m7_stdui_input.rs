...
fn ui_input_and_describe_skyc_and_cargo_zero() {
    ...
    let root = repo_root();
    let dir = root.join("tests").join("golden").join("m7_stdui_input");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for m7_stdui_input: {:?}",
        built.err()
    );
    ...
    let outcome = support::build_and_run_emitted("m7_stdui_input", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
