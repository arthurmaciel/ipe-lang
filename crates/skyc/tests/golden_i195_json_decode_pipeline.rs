...
//! to — see `runtime/src/sky_runtime/json.rs`). Two mismatches fired: the wrong
...
fn entry_path(root: &Path) -> PathBuf {
    root.join("tests")
        .join("golden")
        .join("i195_json_decode_pipeline")
        .join("Main.sky")
}
...
fn i195_skyc_accepts_and_renders_send_only_fnonce_payload() {
    ...
    let Ok(runtime) = skyc::resolve_runtime() else {
        eprintln!("SKIP i195_json_decode_pipeline: runtime not available");
        return;
    };
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i195_json_decode_pipeline: {:?}",
        built.err()
    );
...
fn i195_cargo_builds_and_runs() {
    ...
    let outcome = support::build_and_run_emitted("i195_json_decode_pipeline", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "i195_json_decode_pipeline binary must exit 0 (no E0308/E0277); got {:?} (stdout: {:?})",
        outcome.exit_code,
        outcome.stdout
    );
