...
//! `i151_poly_task_on_error` c02 fixture, which goes through
...
fn poly_task_on_error_nested_green() {
    ...
    let entry = root
        .join("tests")
        .join("golden")
        .join("i164_poly_task_on_error_nested")
        .join("Main.sky");
    let out = PathBuf::from(env!("CARGO_TARGET_TMPDIR")).join("i164_poly_task_on_error_nested");
    ...
    assert!(
        built.is_ok(),
        "skyc build must succeed for i164_poly_task_on_error_nested: {:?}",
        built.err()
    );
    ...
    let outcome = support::build_and_run_emitted("i164_poly_task_on_error_nested", &out);
    assert_eq!(
        outcome.exit_code,
        Some(0),
        "must exit 0; got:\n{}",
