...
//!   `sky_runtime::cli_program(init, update, view, subscriptions, on_line)`.
...
pub fn emit_cli_call(
    ...
    match k {
        ...
        //
        // view : Model -> String
        // Runtime entry: `sky_runtime::cli_program(init, update, view, subs, on_line)`
        KernelFn::CliProgram => {
            let [cfg_e] = args else {
                return Err(Diagnostic::CompilerBug {
                    where_: "sky_backend_rust::emit_cli_call::CliProgram",
...
/// Emit `sky_runtime::cli_program(init, update, view, subs, on_line)`.
///
...
fn lookup_field<'f>(
    ...
    Err(Diagnostic::CompilerBug {
        where_: "sky_backend_rust::emit_cli_call",
        detail: format!(
