# SkyMon

Ipe.Live monitoring dashboard with uptime monitors, custom metrics, alerting, and SVG charts. 20 modules, approximately 3300 lines of pure Ipê code with proper ADT types throughout.

## Build & Run

```bash
ipe install
ipe build src/Main.ipe
./sky-out/app
```

Open `http://localhost:8000` in your browser. Configure monitors and alert thresholds from the dashboard.

<img width="1024" height="666" alt="17-skymon mp4" src="https://github.com/user-attachments/assets/add828cf-e94a-4582-9570-57a1f811c53f" />
