...
impl IpeStringify for MainMsg {
    fn ipe_show(&self) -> String {
        match self {
            ...
            MainMsg::Reset => "Reset".to_string(),
            MainMsg::Quit => "Quit".to_string(),
            MainMsg::NoOp => "NoOp".to_string(),
        }
    }
}
#[derive(Clone, Debug, PartialEq)]
...
impl IpeStringify for RecCount {
    fn ipe_show(&self) -> String {
        format!("{{{}}}", (&ipe_runtime::stringify::Wrap(&self.count)).dispatch())
    }
}
...
pub fn main_init(arg_0: ()) -> (RecCount, IpeCmd<MainMsg>) {
    (RecCount { count: 0 }, cmd_none())
}
pub fn main_update(msg: MainMsg, model: RecCount) -> (RecCount, IpeCmd<MainMsg>) {
    match msg {
        MainMsg::Increment => ({ let mut __sky_rec = (model.clone()).clone(); __sky_rec.count = ((model).count + 1); __sky_rec }, cmd_none()),
        MainMsg::Decrement => ({ let mut __sky_rec = (model.clone()).clone(); __sky_rec.count = ((model).count - 1); __sky_rec }, cmd_none()),
        MainMsg::Reset => (RecCount { count: 0 }, cmd_none()),
        MainMsg::Quit => (model, cmd_perform(system_exit(0), { let __sky_fn: Box<dyn Fn(IpeResult<ipe_runtime::error::IpeError, JsonVal>) -> MainMsg + Send + Sync + 'static> = Box::new(move |arg_1: IpeResult<ipe_runtime::error::IpeError, JsonVal>| -> MainMsg { MainMsg::NoOp }); __sky_fn })),
        MainMsg::NoOp => (model, cmd_none()),
    }
}
pub fn main_view(model: RecCount) -> String {
    format!("{}{}", "count=".to_string(), format!("{}{}", string_from_int((model).count), "  (+, -, r, q) > ".to_string()))
}
pub fn main_subscriptions(arg_2: RecCount) -> IpeSub<MainMsg> {
    sub_none()
}
pub fn main_on_line(line: String) -> MainMsg {
    match (string_trim(line)).as_str() {
        "+" => MainMsg::Increment,
        "-" => MainMsg::Decrement,
        "r" => MainMsg::Reset,
        "q" => MainMsg::Quit,
...
pub fn ipe_main() -> IpeTask<()> {
    ipe_runtime::cli_program(main_init, main_update, main_view, main_subscriptions, main_on_line)
}
pub fn main_Model(count: i64) -> RecCount {
