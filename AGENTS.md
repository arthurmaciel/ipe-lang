...
Every unit of work is its own feature branch merged straight into `main`, the
default branch. Branch off `main` → fast gate → fix until green → PR **into
`main`** (`gh pr create --base main`) → `gh pr merge <N> --auto --squash` (merges
when green + current). `main` stays green by construction: the fast gate plus
each PR's required CI is the guarantee, so a red PR never lands. One PR per unit;
